...
from runOptions import doNoL1Topo, do3bunches, add2kHxPixel, doAFP2017
...
# on/off the L1 Topo items used in L1Topo commissioning, also disable L1Topo item set in Monitoring_pp_v7_rules.py
NoL1Topo=False ## Keep it at False ATR-16530
# for 2017 rate prediction L1Topo2017_rules
L1Topo2017=True # switch on this one
peakLumi=24001
# 2.2e34 study
...
L1_list=['L1_MU20', 'L1_MU21', 'L1_2MU10', 'L1_3MU6', 'L1_MU11_2MU6',]
...
HLT_list=[ 
    ...
    'HLT_mu26_mu8noL1',
    "HLT_mu26_mu10noL1", 
    'HLT_mu24_2mu4noL1',    'HLT_mu22_2mu4noL1', 'HLT_mu20_2mu4noL1',
    'HLT_3mu6',
    ]
muon_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
HLT_list=[
    ...
    "HLT_mu24_mu10noL1_calotag_0eta010",
    "HLT_mu26_mu8noL1_calotag_0eta010",
    "HLT_mu26_mu10noL1_calotag_0eta010",
    "HLT_mu28_mu8noL1_calotag_0eta010",
    ]
muon_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary 2017 ATR-15878'), 
                    }])))
...
muon_Rules_1p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary'), 
                    18001 : TriggerRule(PS=-1, comment='Disabled'),
                    }])))

HLT_list=['HLT_mu22_mu8noL1',]
muon_Rules_1p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary'), 
                    18001 : TriggerRule(rate=0.1, comment='Support'),
                    }])))

HLT_list=['HLT_mu22_mu8noL1_calotag_0eta010',]
muon_Rules_1p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary'),
                    18001 : TriggerRule(PS=-1, comment='Disabled'),
                    }])))
...
muon_Rules_supporting.update({
    ...
    # low pT used for supporting (also for bJpsi_Trkloose, TrkPEB)
        'L1_MU4'                                   : {  1000  : TriggerRule(rate=930, maxRate=930, comment='Support L1, also for bjets and bphys TrkPEB (ATR-16689)')	,
                                                        10001 : TriggerRule(rate=50, maxRate=100, comment='Support L1, also for bjets and bphys TrkPEB (ATR-16689)'), 
                                                        },
        'L1_MU6'                                   : {  1000  : TriggerRule(rate=2250, maxRate=2250, comment='Support L1, 2500Hz for TrkPEB chains (ATR-16689)'),
                                                        10001 : TriggerRule(rate=625, maxRate=625, comment='Support L1, Maxrate for CPU-heavy Trkloose chains, 500Hz for TrkPEB'),
                                                        },
})
...
for Rules in RulesList:
    for newrule in Rules.keys():
        if newrule in muon_Rules:
            print 'FATAL     Physics_pp_v7_rules     Duplicated rule inside physics rule. Cannot be added:',newrule
...
bphys_Rules_2e34=RulebookDict(tag,'bphys_Rules_2e34')
...
if NoHLTTopo:
    bphys_Rules_2e34.update(dict(map(None,L1_list,len(L1_list)*[{
                        1000 : TriggerRule(PS=1, comment='Primary L1'),
                        7001 : TriggerRule(PS=100, comment='Support, ATr-16576'),
                        }])))
else:
    bphys_Rules_2e34.update(dict(map(None,L1_list,len(L1_list)*[{
...
# support triggers in B-physics stream 
HLT_list=[
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
    1000 : TriggerRule(rate=0.2, maxRate=0.2, comment='Support, ATR-15503'),
   15001 : TriggerRule(PS=-1, comment='Disable, ATR-16576'),
    }])))

# support triggers in Main
HLT_list=[
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
    10000 : TriggerRule(rate=0.2, comment='Support, ATR-15503'),
    }])))
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
    10000 : TriggerRule(rate=0.2, ESValue=1, comment='Support, ATR-15503, express'),
    }])))
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
    ...
    10001: TriggerRule(PS=50, ESRate=0.1, comment='Support ATR-15503 express')  ,
    15001: TriggerRule(PS=-1, ESRate=0.1, comment='Support ATR-15503, 122.5 Hz unPSed at 2E34, express'),
    }])))
...
if L1Topo2017: 
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
        1 : TriggerRule(PS=1, ESRate=0.1, comment='Primary L1Topo Bphys, EndOfFill, ATR-14108 express'),
     4001 : TriggerRule(PS=1,  ESRate=0.1, comment='Primary L1Topo Bphys, EndOfFill, ~2Hz at 1e34 PS=1 express'),
        }])))
else:
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{ 
        1000 : TriggerRule(PS=-1, ESValue=0, comment='Disabled L1Topo B-phys, ATR-15503'),
        }])))
...
if L1Topo2017: 
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
         1 : TriggerRule(PS=1, ESRate=0.1, comment='Primary L1Topo Bphys, EndOfFill, ATR-14108 express')  ,
      4001 : TriggerRule(PS=1, ESRate=0.1, comment='Primary L1Topo Bphys, EndOfFill, 1Hz at 1e34 PS=1 express'),
        }])))
else:
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{ 
        1000 : TriggerRule(PS=-1, ESValue=0, comment='Disabled L1Topo B-phys, ATR-15503'),
...
if L1Topo2017: 
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
        1 : TriggerRule(PS=1, ESRate=0.1, comment='Primary L1Topo Bphys, EndOfFill, ATR-14108 express')  ,
        4001 : TriggerRule(PS=20, ESRate=0.1, comment='Primary L1Topo Bphys, EndOfFill, 3Hz at 1e34 PS=1 express'),
        7001 : TriggerRule(PS=20, ESRate=0.1, comment='Support L1Topo Bphys, ATR-15503 express'),
        }])))
else:
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{ 
        1000 : TriggerRule(PS=-1, ESValue=0, comment='Disabled L1Topo B-phys, ATR-15503'),
...
#     4001 : TriggerRule(PS=5, comment='L1Topo Bphys, EndOfFill, 21Hz at 1e34'),
#    10001 : TriggerRule(PS=10, comment='PS=10 for 1.0-1.5E34, ATR-15503'),
#    15001 : TriggerRule(PS=20, comment='PS=20 above 1.5E34, ATR-15503')
        }])))
else:
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{ 
        1000 : TriggerRule(PS=-1, comment='Disabled L1Topo B-phys, ATR-15503'),
        }])))
...
if L1Topo2017:
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
        1 : TriggerRule(PS=1, ESRate=0.2, comment='Primary, L1Topo Bphys, EndOfFill, ATR-15503, express')  ,
        12001 : TriggerRule(PS=1, ESRate=0.2, comment='Primary, ATR-15503, EndOfFill, express'),
        # unprescale in scenario 2, ATR-15503
        18001 : TriggerRule(PS=1, ESRate=0.2, comment='Primary, ATR-15503, EndOfFill, express'),
        }])))
else:
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{ 
        1000 : TriggerRule(PS=-1, ESValue=0, comment='Disabled L1Topo B-phys, ATR-15503'),
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='high-lumi backup, ATR-17014'),
                    }])))
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
   ...
   10000 : TriggerRule(PS=-1, comment='Disable, ATR-15503'),
    # NB: has PS=-1 at 1E34
    }])))

HLT_list=[
    'HLT_2mu4_bJpsimumu_L1BPH-2M9-2MU4_BPH-0DR15-2MU4', # 230.8 Hz @2E34
]
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
   ...
   10001 : TriggerRule(PS=-1, comment='Disable, ATR-15503'),
    }])))


HLT_list=[
    'HLT_mu6_mu4_bBmumu_L1BPH-2M9-MU6MU4_BPH-0DR15-MU6MU4', # 41.5 Hz @2E34
]
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
    1000 : TriggerRule(PS=20, comment='Support, ATR-15503'),
   15001 : TriggerRule(PS=-1, comment='Disable, ATR-15503'),
    }])))
...
HLT_list=[
    'HLT_mu6_mu4_bJpsimumu_L1BPH-2M9-MU6MU4_BPH-0DR15-MU6MU4', # 175 Hz@2E34
]
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
# PS=20 in scenario 3b, ATR-15503
        1 : TriggerRule(PS=100, comment='Support, ATR-15503'),
    15001 : TriggerRule(PS=-1, comment='Support, ATR-15503'),
    }])))
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
# has L1 PS=20 at 2E34
    1000 : TriggerRule(PS=1, comment='Primary, EndOfFill, ATR-15503'),
   15001 : TriggerRule(PS=-1, comment='Disable, ATR-16576'),
    }])))

# Triggers for B-physics stream
HLT_list=[
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
# unprescale in scenario 2a, ATR-15503
                    1000 : TriggerRule(PS=1, comment='Primary, EndOfFill, ATR-15503'),
                    }])))
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
# unprescale in scenario 2a, ATR-15503
                    1000 : TriggerRule(PS=1, ESRate=0.2, comment='Primary, EndOfFill, Express, ATR-15503, approved 2017-06-03'),
                    }])))

# Triggers for Main (3mu4)
HLT_list=[
...
HLT_list=[
    'HLT_2mu6_bBmumuxv2_L1LFV-MU6', # 87.7 Hz at 2E34
]
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary, EndOfFill, ATR-15503'),
# unprescale in scenario 2b, prescale again in scenario 3 & above - ATR-15503
                   12001 : TriggerRule(PS=-1, comment='Disabled, ATR-15503'),
                    }])))
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                   ...
                   peakLumi : TriggerRule(PS=-1, comment='Disabled, ATR-15503'),
                    }])))
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                   10001 : TriggerRule(PS=-1, comment='Disabled, ATR-15503'),
                    }])))
...
HLT_list = ['HLT_2mu6_bDimu_L1BPH-8M15-2MU6', 'HLT_2mu6_bBmumu']
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
    1000 : TriggerRule(PS=-1, comment='Disabled',ESValue=0 )
}])))
HLT_list=[
...
if L1Topo2017:
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
        1000 : TriggerRule(rate=0.2,maxRate=0.2, comment='Support', ESValue=0), 
        }])))
else:
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{ 
        1000 : TriggerRule(PS=-1, comment='Disabled L1Topo B-phys, ATR-15503'),
...
if L1Topo2017:
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
        1000 : TriggerRule(PS=1, ESRate=0.2, comment='Primary, EndOfFill, express'),
        # unprescale in scenario 2, ATR-15503
        15001 : TriggerRule(PS=1, ESRate=0.2, comment='Primary, ATR-15503, EndOfFill, express'),
        }])))
else:
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{ 
        1000 : TriggerRule(PS=-1, ESValue=0, comment='Disabled L1Topo B-phys, ATR-15503'),
...
bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
    1000 : TriggerRule(rate=0.2,maxRate=0.2, comment='Support'), 
    }])))
...
HLT_list=[
    ...
    'HLT_3mu4_mu4noL1', 'HLT_3mu4_mu2noL1', # requested by James initially
    # calotag ATR-16142
...
if NoHLTTopo:
    bphys_Rules_1p5e34.update(dict(map(None,L1_list,len(L1_list)*[{
                        1000 : TriggerRule(PS=100, comment='Support L1, ATR-16576'),
                        }])))
else:        
    bphys_Rules_1p5e34.update(dict(map(None,L1_list,len(L1_list)*[{
                        1000 : TriggerRule(PS=1, comment='Primary L1, EndOfFill'),                        
...
bphys_Rules_1p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
    1000 : TriggerRule(PS=1, comment='Primary, EndOfFill'),
    5001 : TriggerRule(PS=-1, comment='temporarily disable'),
    }])))
...
bphys_Rules_1p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
    # unprescale in scenario 3, ATR-15503
    1000 : TriggerRule(PS=-1, ESValue=0, comment='Disabled express 0.1?'), 
    }])))

########################################
# 0.4e34 

bphys_Rules_0p4e34=RulebookDict(tag,'bphys_Rules_0p4e34')
L1_list=['L1_2MU4']
if NoHLTTopo :
...
bphys_Rules_0p5e34=RulebookDict(tag,'bphys_Rules_0p5e34') 
L1_list=['L1_3MU4']
bphys_Rules_0p5e34.update(dict(map(None,L1_list,len(L1_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary L1'),
                    peakLumi : TriggerRule(PS=-1, comment='Disabled YU'),
                    }])))
...
# special requests for 4mu analysis
HLT_list=[
    'HLT_3mu4_bTau',
    'HLT_3mu4_bUpsi',
    'HLT_2mu6_mu4_bUpsi',
...
bphys_Rules_0p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=-1, comment='Disable, ATR-15503'),
                    }])))

bphys_Rules_delayed=RulebookDict(tag,'bphys_Rules_delayed')

HLT_list=[
    # new chains as discussed in ATR-15503
...
bphys_Rules_delayed.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, ESRate=0.2, comment='Primary, EndOfFill, express, ATR-15503'), 
                    }])))
...
bphys_Rules_delayed.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, ESRate=0.1, comment='Primary, EndOfFill, express, ATR-15503'), 
                    }])))
...
bphys_Rules_delayed.update(dict(map(None,HLT_list,len(HLT_list)*[{                    
                    1000 : TriggerRule(PS=1, ESRate=0.2, comment='Primary, Baseline, express, ATR-15503, ATR-16717'),
                    }])))
...
bphys_Rules_delayed.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=-1, comment='Disabled, ATR-16576', ESValue=0),
                    }])))
...
bphys_Rules_delayed.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=-1, comment='temporarily disabled'),
...
if L1Topo2017:
    bphys_Rules_delayed.update(dict(map(None,HLT_list,len(HLT_list)*[{
        1000 : TriggerRule(PS=1, comment='Primary, EndOfFill, ATR-15503'),
#        15001 : TriggerRule(PS=-1, comment='ATR-15503'),
        }])))
else:
    bphys_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{ 
        1000 : TriggerRule(PS=-1, comment='Disabled L1Topo B-phys, ATR-15503'),
...
bphys_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
    1000 : TriggerRule(rate=0.2, ESValue=1, comment='Support, express'), 
    }])))

HLT_list=[
    'HLT_mu11_mu6_bDimu_noinvm_novtx_ss',
    ]
bphys_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
HLT_list=['HLT_2mu6_bDimu', 
          ]
bphys_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                        1000 : TriggerRule(rate=0.5, comment='Support'),
                        }])))

HLT_list=['HLT_2mu6_bDimu_novtx_noos_L1BPH-2M9-2MU6_BPH-2DR15-2MU6',
          ]
bphys_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                        1000 : TriggerRule(rate=0.2, comment='Support'),
                        }])))
...
#                   15001 : TriggerRule(PS=-1, comment='Support'),
                    }])))
...
HLT_list=['HLT_mu6_mu4_bUpsimumu_L1BPH-8M15-MU6MU4_BPH-0DR22-MU6MU4-BO']
if L1Topo2017:
    bphys_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Support, ATR-16219'),
                   12001 : TriggerRule(PS=-1, comment='Disable, ATR-16219'),
        }])))
else:
    bphys_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{ 
        1000 : TriggerRule(PS=-1, comment='Disabled L1Topo B-phys, ATR-15503'),
...
bphys_Rules_supporting.update({
    ...
    'HLT_mu6_bJpsi_Trkloose'             : {  1000 : TriggerRule(rate=0.1, ESValue=1, comment='Support, Jpsi idperf')},
    ...
    'HLT_mu20_bJpsi_Trkloose'            : {  1000 : TriggerRule(rate=0.1, ESValue=1, comment='Support, Jpsi idperf')},
...
bphys_Rules_supporting.update({ 
    ...
    'HLT_mu10_bJpsi_TrkPEBmon'   : {  1000 : TriggerRule(rate=0.2, ESRate=0.2,  comment='Support, ATR-16689, ATR-17203, Jpsi TrkPEB for low-lumi') },
...
RulesList=[bphys_Rules_2p5e34, bphys_Rules_2e34, bphys_Rules_1p5e34, bphys_Rules_0p5e34, bphys_Rules_0p4e34, bphys_Rules_supporting, bphys_Rules_delayed]
...
HLT_list=["HLT_xe110_pufit_L1XE70", "HLT_xe120_pufit_L1XE70",]
...
HLT_list=[
    ...
    "HLT_xe90_mht_xe65_L1XE60",
    "HLT_xe90_mht_xe70_L1XE60",
    "HLT_xe90_mht_xe75_L1XE60",
    "HLT_xe90_mht_xe80_L1XE60",
    
    "HLT_xe100_mht_xe65_L1XE60",
    "HLT_xe100_mht_xe70_L1XE60",
    "HLT_xe100_mht_xe75_L1XE60",
    "HLT_xe100_mht_xe80_L1XE60",

    "HLT_xe110_mht_xe65_L1XE60",
    "HLT_xe110_mht_xe70_L1XE60",    
    "HLT_xe110_mht_xe75_L1XE60",
    "HLT_xe110_mht_xe80_L1XE60",
    ]
met_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Support for commissioning'),
                   10001 : TriggerRule(PS=-1, comment='Disable'),
                    }])))

HLT_list=[
    'HLT_xe110_L1XE50','HLT_xe110_wEFMu_L1XE50', 
    'HLT_xe120_L1XE60', 'HLT_xe130_L1XE60',
    ]
met_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Support Backup'), 
                   10001 : TriggerRule(PS=-1, comment='Disabled'),
                    }])))
...
met_Rules_1p5e34.update(dict(map(None,L1_list,len(L1_list)*[{
                   1000 : TriggerRule(PS=1, comment='Primary L1'), 
                   18001 : TriggerRule(rate=100, maxRate=100, comment='Support'),
                   }])))

HLT_list=[
    #primary
    'HLT_xe130_mht_L1XE50',
    'HLT_xe130_tc_em_L1XE50',"HLT_xe120_mht_L1XE60",
]
...
HLT_list=[
    "HLT_xe100_pufit_wEFMu_L1XE50", "HLT_xe100_pufit_wEFMu_L1XE55", "HLT_xe100_pufit_wEFMu_L1XE60",
    ]
...
HLT_list = [
    ...
    # 5-jet central GSC
    #'HLT_5j50_gsc70_boffperf_split_0eta240_L14J150ETA25',## disable YU
    #'HLT_5j50_gsc70_boffperf_split_0eta240', #L1_4J20## disable YU
    ...
    'HLT_7j25_gsc45_boffperf_split_L15J150ETA25',
    ...
    # 7-jet central GSC
    'HLT_7j25_gsc45_boffperf_split_0eta240_L14J150ETA25',
    'HLT_7j25_gsc45_boffperf_split_0eta240_L14J20',
    'HLT_7j25_gsc45_boffperf_split_0eta240_L15J150ETA25',
...
HLT_list = [
    ...
    'HLT_5j60_gsc100_boffperf_split_L14J150ETA25',
    'HLT_5j60_gsc100_boffperf_split', #L1_4J20
    ...
    # 7-jet GSC
    'HLT_7j25_gsc50_boffperf_split_L14J150ETA25',
    'HLT_7j25_gsc50_boffperf_split_L14J20',
    'HLT_7j25_gsc50_boffperf_split_L15J150ETA25',

    # 7-jet central
    # None foreseen, fall back on the GSC 7j central trigger below instead

    # 7-jet central GSC
    'HLT_7j25_gsc50_boffperf_split_0eta240_L14J150ETA25',
    'HLT_7j25_gsc50_boffperf_split_0eta240_L14J20',
    'HLT_7j25_gsc50_boffperf_split_0eta240_L15J150ETA25',
...
HLT_list = ['HLT_j460']
jet_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary backup, Express',ESRate=0.2), 
                    }])))
...
if L1Topo2017:
    jet_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                       1000 : TriggerRule(rate=1, comment='Support, L1Topo')
                       }])))
else:
    jet_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                       1000 : TriggerRule(PS=-1, comment='Disabled, L1Topo')
...
HLT_list=[
    'HLT_2j225_gsc250_boffperf_split_0eta240_j70_gsc120_boffperf_split_0eta240',
    'HLT_2j225_gsc250_boffperf_split_0eta240_j70_gsc140_boffperf_split_0eta240',
]
jet_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
jet_Rules_1p7e34.update(dict(map(None,L1_list,len(L1_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary L1'),
                   18001 : TriggerRule(PS=-1, comment='Disabled L1'), 
                     }])))
...
HLT_list = [
    ...
    'HLT_6j25_gsc50_boffperf_split_0eta240_L15J150ETA25',
]
...
# HLT primaries for 1.5e34 which become support triggers
HLT_list = ['HLT_j440_a10t_lcw_jes_L1J100',]
jet_Rules_1p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                      1000 : TriggerRule(PS=1, comment='Primary 1.5e34'), 
                     15001 : TriggerRule(rate=1.0, comment='Support for j440 SMC'),
                      }])))
...
HLT_list = [
    ...
    'HLT_6j25_gsc45_boffperf_split_0eta240_L15J150ETA25',
]
...
HLT_list=['HLT_6j25_gsc45_boffperf_split_0eta240_L14J150ETA25',]
jet_Rules_1p2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary 1.2e34 jet'), 
                    12001 : TriggerRule(PS=15, comment='Support for SUSY multijets, ATR-16728'), 
                    }])))
...
# HLT primaries for 1.4e34 which become support triggers
HLT_list = ['HLT_6j45_0eta240_L14J20',]
jet_Rules_1p2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
if not NoL1Topo:
    jet_Rules_1p2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                        1000 : TriggerRule(PS=1, comment='Primary J4-MATCH seeded multi jet'),
                        14001 : TriggerRule(rate=1.0, comment='Support ATR-15236, ATR-14353'),
                        }])))
else:
    jet_Rules_1p2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
jet_Rules_supporting = {
        'L1_RD0_FILLED'  : { 1000 : TriggerRule(rate=rate_L1RD0_FILLED, maxRate=rate_L1RD0_FILLED, scaling='bunches', comment='Support L1') },
        }
...
HLT_list = ['HLT_j25']
jet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(rate=4.0, comment='Support, Express',ESRate=0.8), # 3Hz
                    15001 : TriggerRule(rate=2.0, comment='Support, Express',ESRate=0.8), 
                    }])))
HLT_list = ['HLT_j60']
jet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(rate=4.0, comment='Support, Express',ESRate=0.2), # 3Hz
                    15001 : TriggerRule(rate=2.0, comment='Support, Express',ESRate=0.2),
                    }])))


# Standard single-jet support triggers for jet calibration, pass-through
# NOTE these should vary during a single fill
HLT_list = ['HLT_j15',]
jet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(rate=4.0, comment='Support', PTRate=0.5), # 3Hz
                    15001 : TriggerRule(rate=2.0, comment='Support', PTRate=0.5),
                   }])))
...
HLT_list = [
    ...
    # HLT_j35_320eta490 (L1_RD0_FILLED)
...
jet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                   1000 : TriggerRule(PS=-1, comment='Disabled, currently under validation')
                   }])))
...
if not NoL1Topo:
    jet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                       1000 : TriggerRule(rate=1.0, comment='Support L1Topo'), 
                       }])))
else:
    jet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                       1000 : TriggerRule(PS=-1, comment='Disabled L1Topo'), 
...
HLT_list=['HLT_j440_a10t_lcw_jes_35smcINF_L1J100',
...
HLT_list=[
    # 2b+2j
    "HLT_2j15_gsc35_bmv2c1040_split_2j15_gsc35_boffperf_split_L14J15.0ETA25", #==>BACKUP
    "HLT_2j25_gsc45_bmv2c1050_split_2j25_gsc45_boffperf_split_L14J15.0ETA25", #==>BACKUP
    "HLT_2j35_gsc55_bmv2c1060_split_2j35_gsc55_boffperf_split_L14J15.0ETA25", #==>BACKUP
    # 2b+3j
    "HLT_2j15_gsc35_bmv2c1050_split_3j15_gsc35_boffperf_split",     #==>BACKUP
    "HLT_2j25_gsc45_bmv2c1060_split_3j25_gsc45_boffperf_split",     #==>BACKUP
    # 3b+1j
    "HLT_3j15_gsc35_bmv2c1070_split_j15_gsc35_boffperf_split_L14J15.0ETA25",  #==>BACKUP
    # 4b
    "HLT_2j15_gsc35_bmv2c1070_split_2j15_gsc35_bmv2c1085_split_L14J15.0ETA25", #==>BACKUP
    "HLT_4j15_gsc35_bmv2c1077_split_L14J15.0ETA25", #==>BACKUP    
    ]
if NoGSCMBjets:
    bjet_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
HLT_list=[
    # 2b+2j
    "HLT_2j35_bmv2c1040_split_2j35_boffperf_split_L14J15.0ETA25", #OK
    "HLT_2j35_bmv2c1050_split_2j35_boffperf_split_L14J15.0ETA25", #OK
    "HLT_2j35_bmv2c1060_split_2j35_boffperf_split_L14J15.0ETA25", #OK
    # 2b+3j
    "HLT_2j35_bmv2c1050_split_3j35_boffperf_split", #OK
    "HLT_2j35_bmv2c1060_split_3j35_boffperf_split", #OK
    # 4b
    "HLT_2j35_bmv2c1070_split_2j35_bmv2c1085_split_L14J15.0ETA25",  #OK   
    "HLT_4j35_bmv2c1077_split_L14J15.0ETA25", #OK 
    # 3b+1j 
    "HLT_3j35_bmv2c1070_split_j35_boffperf_split_L14J15.0ETA25", # OK
]
bjet_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
HLT_list=[
    ...
    "HLT_2j15_gsc35_bmv2c1060_split_3j15_gsc35_boffperf_split", # new 16Hz
...
HLT_list=[
    ...
    "HLT_mu26_ivarmedium_2j15_gsc35_boffperf_split",
    
    'HLT_e26_lhtight_nod0_ivarloose_L1EM22VHIM_2j35_boffperf',
    'HLT_e26_lhtight_nod0_ivarloose_L1EM22VHIM_2j35_boffperf_split',
    'HLT_e26_lhtight_nod0_ivarloose_L1EM22VHIM_2j15_gsc35_boffperf_split',

    'HLT_e28_lhtight_nod0_ivarloose_L1EM24VHIM_2j35_boffperf_split',
    'HLT_e28_lhtight_nod0_ivarloose_L1EM24VHIM_2j35_boffperf',
    'HLT_e28_lhtight_nod0_ivarloose_L1EM24VHIM_2j15_gsc35_boffperf_split',
    ]
...
HLT_list=[
    ...
    'HLT_2e24_lhvloose_nod0_2j35_boffperf',
    ...
    'HLT_e28_lhtight_nod0_ivarloose_j150_boffperf_j35_boffperf',
    ...
    'HLT_mu26_ivarmedium_j150_boffperf_j35_boffperf',
    ...
    'HLT_e28_lhmedium_nod0_L1EM24VHI_mu8noL1_2j35_boffperf',
...
HLT_list=[
    ...
    # 1j+2b
    "HLT_j110_gsc150_boffperf_split_2j35_gsc55_boffperf_split_L1J85_3J30",
    # 2b+2j
    "HLT_2j15_gsc35_boffperf_split_2j15_gsc35_boffperf_split_L14J15.0ETA25",
    # 3b
    "HLT_3j50_gsc65_boffperf_split_L13J35.0ETA23",
    # 2b+HT
    "HLT_2j35_gsc55_boffperf_split_ht300_L1HT190-J15s5.ETA21",
...
HLT_list=[
    'HLT_2j15_gsc35_bmv2c1060_split_2j15_gsc35_boffperf_split_L14J15.0ETA25',
    ]
bjet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=5, comment='Support for 3b+1j, all-hadronic ttH Hbb (ATR-16309) EndOfFill'),
                   10001 : TriggerRule(PS=10, comment='Support for 3b+1j, all-hadronic ttH Hbb (ATR-16309) EndOfFill'), 
                            }])))
...
HLT_list=['HLT_j35_gsc55_boffperf_split',]
bjet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                           1000 : TriggerRule(rate=0.6, comment='Support Express', ESRate=0.4), 
                           }])))
HLT_list=['HLT_j60_gsc110_boffperf_split',]
bjet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                           1000 : TriggerRule(rate=0.6, comment='Support Express', ESRate=0.2), 
                           }])))
HLT_list=['HLT_j15_gsc35_boffperf_split',]
bjet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                           1000 : TriggerRule(rate=0.6, comment='Support', ESValue=0), 
                           }])))
HLT_list=['HLT_j55_boffperf_split']
bjet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
HLT_list=[
    ...
    'HLT_mu4_j15_gsc35_boffperf_split',
    'HLT_mu4_j15_gsc35_boffperf_split_dr05_dz02', 
    'HLT_mu4_j15_gsc35_boffperf_split_dr05_dz99', 
    ...
    'HLT_mu6_j60_gsc110_boffperf_split_dr05_dz02',
    # MU6_J40
...
# MU4_J12
HLT_list=['HLT_mu4_j35_gsc55_boffperf_split_dr05_dz02',
    ]
bjet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
HLT_list=[
    ...
    # tau + mu iso (high et, w/ and w/o jet)
...
tau_Rules_1p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                            1000 : TriggerRule(PS=1, comment='Primary'), 
                            18001 : TriggerRule(PS=-1, comment='Disable, huge L1 rate, 5kHz at 2e34'),
                            }])))
...
tau_Rules_1p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                            1000 : TriggerRule(PS=1, comment='Primary'), 
                            18001 : TriggerRule(rate=0.1, comment='Support, ATR-12219', rerun=1),
                            }])))
...
tau_Rules_1p5e34.update(dict(map(None,L1_list,len(L1_list)*[{
                              1000 : TriggerRule(PS=1, comment='Primary L1'), 
                              12001 : TriggerRule(rate=100, maxRate=100, comment='Support'),
                              }])))
...
tau_Rules_1p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                              ...
                              18001 : TriggerRule(rate=0.2, comment='Support, ATR-12219', ESValue=1, rerun=1),
                              }])))
...
tau_Rules_1p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                              ...
                              18001 : TriggerRule(rate=0.2, comment='Support, ATR-12219', ESValue=0),
                              }])))
...
if NoHLTTopo:
    ...
    tau_Rules_0p3e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                       ...
                       18001 : TriggerRule(PS=5, comment='Support Commission L1Topo, ATR-13366'),
                        }])))
...
# --------------------------------------
# 1.5e34 menu
L1_list=['L1_EM22VHI', 'L1_EM22VHIM']
egamma_Rules_2e34.update(dict(map(None,L1_list,len(L1_list)*[{
                   1000 : TriggerRule(PS=1, comment='Primary L1'), 
                  18001 : TriggerRule(PS=-1, comment='Disable'), 
                   }])))
...
HLT_list=['HLT_e26_lhtight_nod0_ivarloose', 'HLT_e26_lhtight_nod0_ivarloose_L1EM22VHIM',
          ]
egamma_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                   1000 : TriggerRule(PS=1, comment='Primary, EM22VHI(M) seed'), 
                  18001 : TriggerRule(PS=-1, comment='Support')}])))
...
egamma_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    ...
                    18001 : TriggerRule(rate=0.5, comment='Support'),
                    }])))


# --------------------------------------
HLT_list=['HLT_e26_lhtight_idperf', 'HLT_e26_lhtight_idperf_L1EM22VHIM', ]
egamma_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
egamma_Rules_1e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary, EndOfFill', ESValue=0),
                    10001 : TriggerRule(PS=-1, comment='Disabled', ESValue=0),
                    }])))
...
HLT_list=[
    ...
    'HLT_e26_lhvloose_nod0', 'HLT_e26_lhvloose_nod0_L1EM22VHIM', 'HLT_e26_lhvloose_nod0_L1EM22VHIM',
...
egamma_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
    1000 : TriggerRule(PS=0, comment='Support Express', rerun=1, ESRate=0.2), 
}])))
...
egamma_Rules_supporting.update({
     ...
     "HLT_e26_lhtight_nod0_L1EM22VHIM_e15_etcut_L1EM7_Zee"       : { 1000 : TriggerRule(PS=1, comment='Primary, ATR-16089') },
...
egamma_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Support, Jpsi ee, express',ESRate=0.4), 
                    }])))

HLT_list=['HLT_e5_lhtight_nod0_e4_etcut_Jpsiee_L1RD0_FILLED']
egamma_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(rate=0.5, comment='Support, Jpsi ee ATR-16855',),
                   15001 : TriggerRule(PS=-1, comment='Disable, Jpsi ee ATR-16855',), 
                    }])))
...
egamma_Rules_supporting.update({
    ...
    # 1.2e34
        'HLT_e26_lhvloose_nod0_L1EM20VH_3j20noL1'              : {  1000 : TriggerRule(PS=5, comment='Support, SM W+jets, 1 Hz target at 1.2e34'),
                                                                   18001 : TriggerRule(PS=-1, comment='Disabled, SM W+jets'),
                                                                    },
...
HLT_list=[
    ...
    'HLT_e26_lhmedium_nod0_L1EM22VHIM_mu8noL1',
...
combined_Rules_other={
    ...
    'HLT_g45_tight_L1EM22VHI_xe45noL1'   : {  1000 : TriggerRule(PS=1, comment='Primary, ATR-12901 EndOfFill'),
                                             ...
                                             18001 : TriggerRule(PS=-1, comment='Disabled, ATR-12901'),
                                              },
    ...
    'HLT_j60_gsc100_bmv2c1050_split_xe80_mht_L1XE60' : {  1000 : TriggerRule(PS=1, comment='Primary, SUSY 3bjet+MET analysis ATR-15232'),
                                                      }, 
    'HLT_2j25_gsc45_bmv2c1070_split_xe80_mht_L12J15_XE55' : {  1000 : TriggerRule(PS=1, comment='Primary, SUSY 3bjet+MET analysis ATR-15232'), 
                                                        },
    'HLT_3j15_gsc35_bmv2c1077_split_xe60_mht_L13J15.0ETA25_XE40' : {  1000 : TriggerRule(PS=1, comment='Primary, SUSY 3bjet+MET analysis ATR-15232'), #6Hz
                                                                      },
...
if L1Topo2017:
    ...
    vbf_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                        1000 : TriggerRule(PS=-1, comment='Disable, VBF H->bb , ATR-13526, ATR-15044'), 
                        }])))
...
if L1Topo2017:
    ...
    vbf_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                        1000 : TriggerRule(PS=-1, comment='Disable, Support, VBF H->bb , ATR-13526, ATR-15044'), 
                        }])))
...
HLT_list=['HLT_g20_tight_icaloloose_j15_gsc35_bmv2c1077_split_3j35_0eta490_invm500', #==>BACKUP
          'HLT_g27_medium_L1EM24VHI_j15_gsc35_bmv2c1077_split_3j35_0eta490_invm700', #==>BACKUP
          'HLT_g27_medium_L1EM24VHI_2j15_gsc35_bmv2c1077_split_2j35_0eta490', #==>BACKUP
          ]
if NoGSCMBjets:
    vbf_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=-1, comment='Disable, VBF H->bb + photon , ATR-15062. To be replaced with non-gsc backup (NoGSCMBjets)'), 
                    }])))
else:
    vbf_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary, VBF H->bb + photon , ATR-15062'), 
                    }])))
...
HLT_list=['HLT_j80_0eta240_j60_j45_320eta490_AND_2j25_gsc45_bmv2c1070_split', ]         
vbf_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
...
dedicated_Rules_2e34 = {
    ...
    'HLT_j30_jes_PS_llp_L1TAU60'                                : {  1000 : TriggerRule(PS=1, comment='Primary, LLP, ATR-10933'),
                                                                    18001 : TriggerRule(PS=-1, comment='Disable, LLP, ATR-10933'),
                                                                     },
    ...
    'HLT_j30_jes_cleanLLP_PS_llp_pufix_L1TAU60'                 : {  1000 : TriggerRule(PS=1, comment='Primary, LLP, ATR-15255'),
                                                                    18001 : TriggerRule(PS=-1, comment='Disable, LLP, ATR-15255'), 
                                                                     },
...
if L1Topo2017:
    ...
    dedicated_Rules_2e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    1000 : TriggerRule(PS=-1, comment='Disabled L1Topo, SUSY Higgsino, ATR-15243')
                    }])))
...
datascouting_Rules = {
    ...
    {  100 : TriggerRule(PS=2, comment='DS, ATR-16764)'), 
     10001 : TriggerRule(PS=-1, comment='Disabled') },    
    }
...
if NoL1Topo:
    ...
    topo_Rules.update(dict(map(None,topo_unPS_items,len(topo_unPS_items)*[{
       ...
       peakLumi : TriggerRule(PS=-1, comment='Disable'),
                        }])))
...
l1_topo_bphys_rules = {
#  'L1_BPH-2M9-2MU6_BPH-2DR15-2MU6'      : { 1: TriggerRule(PS=1, comment='L1Topo Bphys, EndOfFill, ATR-14108') ,  18001 : TriggerRule(PS=-1,  comment='Disabled if 2MU6 is disabled')},
...
if L1Topo2017:    
    ...
    topo_Rules.update(dict(map(None,HLT_LLP_list,len(HLT_LLP_list)*[{
                        1 : TriggerRule(PS=-1, comment='Disable, ATR-14428, ATR-15236'),
                        }])))
...
#        "L1_BPH-8M15-MU6MU4_BPH-0DR22-MU6MU4-BO", defined below
        "L1_LAR-J",
        "L1_LAR-EM",
        "L1_SC111-CJ15",
#        "L1_BPH-2M9-MU6MU4_BPH-0DR15-MU6MU4", defined below
#        "L1_BPH-8M15-MU6MU4_BPH-0DR22-MU6MU4", defined below
        'L1_EM18VHI_MJJ-300',
        'L1_LATE-MU10_XE40',
        'L1_LATE-MU10_XE50',
        'L1_LATE-MU10_J50',
...
if L1Topo2017:
    ...
    topo_Rules.update(dict(map(None,L1Topo_list,len(L1Topo_list)*[{
                        1000 : TriggerRule(PS=1, comment='Primary L1Topo'),
                        12001 : TriggerRule(PS=-1, comment='Disable, ATR-16576'),
                        }])))
...
if L1Topo2017:
    ...
    topo_Rules.update(dict(map(None,L1Topo_list,len(L1Topo_list)*[{
                        1000 : TriggerRule(PS=1, comment='Primary L1Topo'),
                        10001 : TriggerRule(PS=-1, comment='Disable, ATR-16576'),
                        }])))
...
if L1Topo2017:
    L1Topo_list=[
        'L1_DR-TAU20ITAU12I', 'L1_HT150-JJ15.ETA49', 'L1_MJJ-100', 'L1_HT150-JJ15.ETA49_MJJ-400', 'L1_MJJ-800', 'L1_KF-XE60', 'L1_KF-XE65', 'L1_DPHI-J20XE50', 'L1_DPHI-CJ20XE50', 'L1_J4-MATCH', 'L1_KF-XE55', 'L1_BPH-8M15-2MU6', 'L1_DY-DR-2MU4', 'L1_BPH-8M15-MU6MU4', 'L1_DY-BOX-2MU6', 'L1_EM15TAU12I-J25', 'L1_DPHI-J20s2XE50', 'L1_MJJ-400', 'L1_TAU20ITAU12I-J25', 'L1_DR-MU10TAU12I', 'L1_SC85-CJ15', 'L1_MJJ-400-CF', 'L1_30M-EM20ITAU12',      ]
...
if L1Topo2017:
    topo_Rules.update({
    'L1_JPSI-1M5-EM7': {  1000 : TriggerRule(rate=1200, maxRate=1200, comment='Support L1'),
                          12001: TriggerRule(rate=60,   maxRate=60,   comment='Support L1')},
    'L1_JPSI-1M5-EM12': { 1000 : TriggerRule(rate=2600, maxRate=2600, comment='Support L1'),
                          12001: TriggerRule(rate=130,  maxRate=130,  comment='Support L1')}  
     })
    ...
    topo_Rules.update(dict(map(None,hlt_jpsiee_list,len(hlt_jpsiee_list)*[{
                    1000 : TriggerRule(PS=-1, comment='Disable L1Topo'),
                    }])))
...
l1topo_streamer_Rules = {
    'HLT_noalg_l1topo_L1DR-TAU20ITAU12I'    : { 1000 : TriggerRule(rate=0.5, comment='YUU')},
    ...
    'HLT_noalg_l1topo_L1BPH-8M15-2MU6'    : { 1000 : TriggerRule(rate=0.5, comment='')},
    'HLT_noalg_l1topo_L1DY-DR-2MU4'    : { 1000 : TriggerRule(rate=0.5, comment='')},
    'HLT_noalg_l1topo_L1BPH-8M15-MU6MU4'    : { 1000 : TriggerRule(rate=0.5, comment='')},
...
HLT_list=[
    'HLT_mu4_j15_gsc35_boffperf_split_dr05_dz02_L1BTAG-MU4J15', 'HLT_mu6_j60_gsc85_boffperf_split_dr05_dz02_L1BTAG-MU6J20',
    "HLT_mu4_j35_boffperf_split_dr05_dz02_L1BTAG-MU4J15",
    ]
...
for Rules in RulesList:
    for newrule in Rules.keys():
        if newrule in rules:
            print 'FATAL     Physics_pp_v7_rules     Duplicated rule inside calibration rule. Cannot be added:',newrule
    rules.update(Rules)
...
for Rules in RulesList:
    for newrule in Rules.keys():
        if newrule in rules:
            print 'FATAL     Physics_pp_v7_rules     Duplicated rule inside calibration rule. Cannot be added:',newrule, "CHECK"
    rules.update(Rules)
...
for Rules in RulesList:
    for newrule in Rules.keys():
        if newrule in rules:
            if 0: print 'FATAL     Physics_pp_v7_rules     Duplicated rule inside calibration rule. Cannot be added:',newrule, ". For L1Topo, overwritten intentionally."
    rules.update(Rules)
...
RulesList=[Monitoring_pp_v7_rules.physics_rules]
...
if NoL1Topo:
    for mon_rules in RulesList:
        for newrule in mon_rules.keys():
            if 'L1_' in newrule and '-' in newrule and not 'LAR' in newrule:
                print  "NoL1Topo set to True: Disabling ", newrule
                mon_rules[newrule]={1 : TriggerRule(PS=-1, comment='disable L1 Topo')}

# disable LAR is L1Topo2017=False

if not L1Topo2017:
    for mon_rules in RulesList:
        for newrule in mon_rules.keys():
            if 'LAR' in newrule:
                print  "L1Topo2017 set to false, dissable ", newrule
                mon_rules[newrule]={1 : TriggerRule(PS=-1, comment='disable L1 Topo')}
...
for Rules in RulesList:
    for newrule in Rules.keys():
        if newrule in rules and not 'LAR' in newrule: 
            print 'FATAL     Physics_pp_v7_rules     Duplicated rule in monitoring rule. Cannot be added from monitoring rule for:',newrule
    rules.update(Rules)

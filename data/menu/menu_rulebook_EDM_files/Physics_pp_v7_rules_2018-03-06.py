...
# These flags can be removed only for past L1Topo commissioning efforts
...
disable3MU4=False # Default: False
...
if NoHLTTopo :
    ...
    bphys_Rules_2p5e34.update(dict(map(None,L1_list,len(L1_list)*[{
                        1000 : TriggerRule( rate = 500, reoptimize=False, comment='Support L1'),
                        }])))
#To check: is this L1 Rate optimized?
...
if NoHLTTopo:
    ...
    #Likely this rate at >1e34 is too high for HLT items... this needs to be checked!
    bphys_Rules_2p5e34.update(dict(map(None,L1_list,len(L1_list)*[{
                        1000 : TriggerRule(PS=1, reoptimize=False, comment='Primary L1, EndOfFill'), 
                        10001 : TriggerRule(rate=500, reoptimize=False, comment='Support L1 topo, EndOfFill'),
                        }])))
...
if NoHLTTopo:
    ...
    bphys_Rules_2p5e34.update(dict(map(None,L1_list,len(L1_list)*[{
                        ...
                        5001 : TriggerRule(rate=1000, reoptimize=False, comment='Support L1 topo, EndOfFill'),
                        }])))
...
bphys_Rules_supporting.update({
    ...
    'HLT_mu6_mu2noL1_msonly_bJpsimumu_noid'  : {  1000 : TriggerRule(rate=1.5, comment='Support Express, Jpsi idperf, ATR-17600') },
...
for Rules in RulesList:
    for newrule in Rules.keys():
        if newrule in bphys_Rules:
            print 'FATAL  Physics_pp_v7_rules   Duplicated rule inside bphys_Rules. Cannot add rule: ',newrule
...
met_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                   1000 : TriggerRule(inputRate=3, comment='Support, ATR-17754,  Express', ESRate=0.5), 
                   }])))
...
jet_Rules_supporting.update(dict(map(None,HLT_list,len(HLT_list)*[{
                    ...
                    15001 : TriggerRule(rate=4.0, comment='Support, Express',ESRate=0.8), 
                    }])))
HLT_list = [
    'HLT_j35',
...
tau_Rules_1p5e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                              ...
                              Primary2017Oct1p7E34 : TriggerRule(rate=0.2, comment='Support, ATR-12219', ESValue=0),
                              }])))
...
if NoHLTTopo:
    ...
    tau_Rules_0p3e34.update(dict(map(None,HLT_list,len(HLT_list)*[{
                       ...
                       Primary2017Oct1p7E34 : TriggerRule(PS=5, comment='Support Commission L1Topo, ATR-13366'),
                        }])))
...
egamma_Rules_supporting.update({
    'HLT_e24_lhvloose_nod0_L1EM20VH'   : {  1000 : TriggerRule(PS=-1,comment='Disable') },
...
L1_list=['L1_MJJ-500-NFF']
topo_Rules.update(dict(map(None,L1_list,len(L1_list)*[{
                    1000 : TriggerRule(PS=1, comment='Primary L1'), 
                    }])))
...
if disable3MU4:
    HLT_list=['HLT_3mu4_bNocut', 'HLT_2mu4_mu3_mu2noL1_bNocut_L13MU4', 'HLT_3mu4', 'HLT_3mu4_mu2noL1', 'HLT_3mu4_bUpsi', 'HLT_3mu3_mu3noL1_L13MU4', 'HLT_3mu4_bVertex2', 'HLT_3mu4_bVertex3', 'HLT_3mu4_mu4noL1', 'HLT_3mu4_bDimu', 'HLT_mu6_2mu4_bDimu', 'HLT_3mu3_mu3noL1_bNocut_L13MU4', 'HLT_mu6_2mu4_bUpsi', 'HLT_mu6_2mu4_bNocut_L1MU6_3MU4', 'HLT_2mu6_mu4_bUpsi', 'HLT_2mu6_mu4_bNocut_L12MU6_3MU4', 'HLT_3mu6_bDimu', 'HLT_3mu6_bUpsi', 'HLT_4mu4_bNocut']  

...
EL::StatusCode JetCalibrator::initialize() {
    ...
    for(unsigned int i = 0; i < m_systList.size(); i++) m_systType.insert(m_systType.begin(), 0);  // Push systType nominal for this case
    ...
    if( !m_JESUncertConfig.empty() && !m_systName.empty()  && (m_systName != "None") ) {
        ...
        if(m_systValVector.size() == 0) {
            if(m_debug) Info("initialize()", "Pushing the following systVal to m_systValVector: %f", m_systVal); m_systValVector.push_back(m_systVal);
        }
    ...
    if( !m_JERUncertConfig.empty() ) {
        ...
        if( !m_JERSmearingTool_handle.isUserConfigured() ) {
            ...
            if(m_JERFullSys) RETURN_CHECK("JetCalibrator::initialize()", m_JERSmearingTool_handle.setProperty("SystematicMode", "Full"), "Failed to set SystematicMode");
            else RETURN_CHECK("JetCalibrator::initialize()", m_JERSmearingTool_handle.setProperty("SystematicMode", "Simple"), "Failed to set SystematicMode");
    ...
    for(const auto& syst_it : m_systList) {
        ...
        if(m_runSysts) {
            if(thisSysType == 1) {
                ...
                if(m_debug) std::cout << "Configure JES for systematic variation : " << syst_it.name() << std::endl; if(m_JetUncertaintiesTool_handle->applySystematicVariation(syst_it) != CP::SystematicCode::Ok) {
            ...
            if( (thisSysType == 2) || m_JERApplyNominal ) {
                if(m_debug) std::cout << "Configure JER for systematic variation : " << syst_it.name() << std::endl; if(thisSysType == 2) { // apply this systematic

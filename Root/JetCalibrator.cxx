...
*
* G.Facini (gabriel.facini@cern.ch), M. Milesi (marco.milesi@cern.ch), J. Dandoy
*(jeff.dandoy@cern.ch)
*
*
******************************************/
...
JetCalibrator::JetCalibrator(std::string className) :
  ...
  m_JetCalibrationTool_handle("JetCalibrationTool/JetCalibrationTool_" +
                              className),
  m_JetUncertaintiesTool_handle(
    "JetUncertaintiesTool/JetUncertaintiesTool_" + className),
  ...
  m_JetTileCorrectionTool_handle(
    "JetTileCorrectionTool/JetTileCorrectionTool_" + className)
{
  ...
  m_calibConfig        = "";
  m_forceInsitu        = true; // when running data "_Insitu" is appended to
                               // this string
...
EL::StatusCode JetCalibrator::initialize()
{
  ...
  RETURN_CHECK("JetCalibrator::execute()",
  ...
  // Insitu should not be applied to the trimmed jets, per Jet/Etmiss
  // recommendation
  if (m_forceInsitu && !m_isMC &&
      (m_calibSequence.find("Insitu") == std::string::npos) &&
      (m_inContainerName.find("AntiKt10LCTopoTrimmedPtFrac5SmallR20") ==
       std::string::npos)) m_calibSequence += "_Insitu";

  if (m_isMC && (m_calibSequence.find("Insitu") != std::string::npos)) {
    Error("initialize()",
          "Attempting to use an Insitu calibration sequence on MC.  Exiting.");
    return EL::StatusCode::FAILURE;
  }
  ...
  if (!m_isMC) {
    ...
    // Check simulation flavour for calibration config - cannot directly read
    // metadata in xAOD otside of Athena!
    //
    // N.B.: With SampleHandler, you can define sample metadata in job steering
    // macro!
    //
    //       They will be passed to the EL:;Worker automatically and can be
    // retrieved anywhere in the EL::Algorithm
    //       I reasonably suppose everyone will use SH...
    //
    //       IMPORTANT! the metadata name set in SH *must* be "AFII" (if not
    // set, name will be *empty_string*)
    ...
    if (m_setAFII) {
      ...
      Warning("initialize()",
              "Could not access simulation flavour from EL::Worker. Treating MC as FullSim by default!");
    } else {
  ...
  m_JetCalibrationTool_handle.setTypeAndName(
    "JetCalibrationTool/JetCalibrationTool_" + m_name);

  if (!m_JetCalibrationTool_handle.isUserConfigured()) {
    RETURN_CHECK("JetCalibrator::initialize()",
                 ASG_MAKE_ANA_TOOL(m_JetCalibrationTool_handle,
                                   JetCalibrationTool),
                 "Could not make JetCalibrationTool");

    RETURN_CHECK("JetCalibrator::initialize()",
                 m_JetCalibrationTool_handle.setProperty("JetCollection",
                                                         m_jetAlgo),
                 "Failed to set JetCollection");
    RETURN_CHECK("JetCalibrator::initialize()",
                 m_JetCalibrationTool_handle.setProperty("ConfigFile",
                                                         m_calibConfig),
                 "Failed to set ConfigFile");
    RETURN_CHECK("JetCalibrator::initialize()",
                 m_JetCalibrationTool_handle.setProperty("CalibSequence",
                                                         m_calibSequence),
                 "Failed to set CalibSequence");
    RETURN_CHECK("JetCalibrator::initialize()",
                 m_JetCalibrationTool_handle.setProperty("IsData", !m_isMC),
                 "Failed to set IsData");

    RETURN_CHECK("JetCalibrator::initialize()",
                 m_JetCalibrationTool_handle.retrieve(),
                 "Failed to retrieve JetCalibrationTool");
  }

  // initialize jet tile correction tool
  if (m_doJetTileCorr && !m_isMC) { // Jet Tile Correction should only be
                                    // applied to data
    m_JetTileCorrectionTool_handle.setTypeAndName(
      "CP::JetTileCorrectionTool/JetTileCorrectionTool_" + m_name);
    RETURN_CHECK("JetCalibrator::initialize()",
                 ASG_MAKE_ANA_TOOL(m_JetTileCorrectionTool_handle,
                                   CP::JetTileCorrectionTool),
                 "Could not make JetTileCorrectionTool");
    RETURN_CHECK("JetCalibrator::initialize()",
                 m_JetTileCorrectionTool_handle.retrieve(),
                 "Failed to retrieve JetTileCorrectionTool");
  }
  ...
  if (m_doCleaning) {
    ...
    m_JetCleaningTool_handle.setTypeAndName(
      "JetCleaningTool/JetCleaningTool_" + m_name);

    if (!m_JetCleaningTool_handle.isUserConfigured()) {
      RETURN_CHECK("JetCalibrator::initialize()",
                   ASG_MAKE_ANA_TOOL(m_JetCleaningTool_handle,
                                     JetCleaningTool),
                   "Could not make JetCleaningTool");

      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JetCleaningTool_handle.setProperty("CutLevel",
                                                        m_jetCleanCutLevel),
                   "Failed to set CutLevel");

      if (m_jetCleanUgly) {
        RETURN_CHECK("JetCalibrator::initialize()",
                     m_JetCleaningTool_handle.setProperty("DoUgly", true),
                     "Failed to set DoUgly");
      }

      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JetCleaningTool_handle.retrieve(),
                   "Failed to retrieve JetCleaningTool");
    }
    ...
    if (m_saveAllCleanDecisions) {
      ...
      for (unsigned int iD = 0; iD < m_decisionNames.size(); ++iD) {
        ...
        this_JetCleaningTool_handle.setTypeAndName("JetCleaningTool/JetCleaningTool_" + m_decisionNames.at(
                                                     iD) + "_" + m_name);

        if (!this_JetCleaningTool_handle.isUserConfigured()) {
          RETURN_CHECK("JetCalibrator::initialize()",
                       ASG_MAKE_ANA_TOOL(this_JetCleaningTool_handle,
                                         JetCleaningTool),
                       "Could not make JetCleaningTool");

          if (m_decisionNames.at(iD).find("Ugly") != std::string::npos) {
            RETURN_CHECK("JetCalibrator::initialize()",
                         this_JetCleaningTool_handle.setProperty("CutLevel",
                                                                 m_decisionNames.
                                                                 at(iD).substr(0,
                                                                               m_decisionNames
                                                                               .at(
                         ...
                         "");
            RETURN_CHECK("JetCalibrator::initialize()",
                         this_JetCleaningTool_handle.setProperty("DoUgly", true),
                         "");
          } else {
            RETURN_CHECK("JetCalibrator::initialize()",
                         this_JetCleaningTool_handle.setProperty("CutLevel",
          ...
          RETURN_CHECK("JetCalibrator::initialize()",
                       this_JetCleaningTool_handle.retrieve(),
                       "Failed to retrieve JetCleaningTool");
        }
  ...
  for (unsigned int i = 0; i < m_systList.size(); i++) m_systType.insert(
      m_systType.begin(),
      0);  // Push systType nominal for this case
  ...
  if (!m_JESUncertConfig.empty() && !m_systName.empty()  &&
      (m_systName != "None")) {
    m_JetUncertaintiesTool_handle.setTypeAndName(
      "JetUncertaintiesTool/JetUncertaintiesTool_" + m_name);

    if (!m_JetUncertaintiesTool_handle.isUserConfigured()) {
      RETURN_CHECK("JetCalibrator::initialize()",
                   ASG_MAKE_ANA_TOOL(m_JetUncertaintiesTool_handle,
                                     JetUncertaintiesTool),
                   "Could not make JetUncertaintiesTool");

      m_JESUncertConfig = gSystem->ExpandPathName(m_JESUncertConfig.c_str());
      Info("JetCalibrator::initialize()",
           "Initialize JES UNCERT with %s",
           m_JESUncertConfig.c_str());
      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JetUncertaintiesTool_handle.setProperty("JetDefinition",
                                                             m_jetAlgo),
                   "Failed to set JetDefinition");
      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JetUncertaintiesTool_handle.setProperty("MCType",
                                                             m_JESUncertMCType),
                   "Failed to set MCType");
      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JetUncertaintiesTool_handle.setProperty("ConfigFile",
                                                             m_JESUncertConfig),
                   "Failed to set ConfigFile");

      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JetUncertaintiesTool_handle.retrieve(),
                   "Failed to retrieve JetUncertaintiesTool");

      //      m_JetUncertaintiesTool_handle->msg().setLevel( MSG::ERROR ); //
      // VERBOSE, INFO, DEBUG
    }


    Info("initialize()", " Initializing Jet Systematics :");
    const CP::SystematicSet recSysts =
      m_JetUncertaintiesTool_handle->recommendedSystematics();

    // If just one systVal, then push it to the vector
    RETURN_CHECK("JetCalibrator::initialize()",
                 this->parseSystValVector(),
                 "Failed to parse vector of systematic sigma values.");

    if (m_systValVector.size() == 0) {
      if (m_debug) Info("initialize()",
                        "Pushing the following systVal to m_systValVector: %f",
                        m_systVal); m_systValVector.push_back(m_systVal);
    }

    for (unsigned int iSyst = 0; iSyst < m_systValVector.size(); ++iSyst) {
      m_systVal = m_systValVector.at(iSyst);
      std::vector<CP::SystematicSet> JESSysList =
        HelperFunctions::getListofSystematics(recSysts,
                                              m_systName,
                                              m_systVal,
                                              m_debug);
      ...
      for (unsigned int i = 0; i < JESSysList.size(); ++i) {
        ...
        // CP::SystematicSet() creates an empty systematic set, compared to the
        // set at index i
        if (JESSysList.at(i).empty() ||
            (JESSysList.at(i) == CP::SystematicSet())) { Info("initialize()",
                                                              "JESSysList Empty at index %d.",
                                                              i); continue; }
    ...
    if (!m_systList.empty()) {
      ...
      if (m_JetUncertaintiesTool_handle->applySystematicVariation(m_systList.at(0))
          != CP::SystematicCode::Ok) {
        Error("JetCalibrator:initialize()",
              "Cannot configure JetUncertaintiesTool for systematic %s",
              m_systName.c_str());
        return EL::StatusCode::FAILURE;
      }
    }
  ...
  if (!m_JERUncertConfig.empty()) {
    ...
    if (!m_JERTool_handle.isUserConfigured()) {
      RETURN_CHECK("JetCalibrator::initialize()",
                   ASG_MAKE_ANA_TOOL(m_JERTool_handle,
                                     JERTool), "Could not make JERTool");

      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JERTool_handle.setProperty("PlotFileName",
                                                m_JERUncertConfig.c_str()),
                   "");
      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JERTool_handle.setProperty("CollectionName", m_jetAlgo),
                   "");

      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JERTool_handle.retrieve(),
                   "Failed to retrieve JERTool");
    }

    // Instantiate the JER Smearing tool
    m_JERSmearingTool_handle.setTypeAndName(
      "JERSmearingTool/JERSmearingTool_" + m_name);

    if (!m_JERSmearingTool_handle.isUserConfigured()) {
      RETURN_CHECK("JetCalibrator::initialize()",
                   ASG_MAKE_ANA_TOOL(m_JERSmearingTool_handle,
                                     JERSmearingTool),
                   "Could not make JERSmearingTool");

      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JERSmearingTool_handle.setProperty("JERTool",
                                                        m_JERTool_handle.getHandle()),
                   "Failed to set JERTool");
      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JERSmearingTool_handle.setProperty("isMC", m_isMC),
                   "Failed to set isMC");
      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JERSmearingTool_handle.setProperty("ApplyNominalSmearing",
                                                        m_JERApplyNominal),
                   "Failed to set ApplyNominalSmearing");

      if (m_JERFullSys) RETURN_CHECK("JetCalibrator::initialize()",
                                     m_JERSmearingTool_handle.setProperty(
                                       "SystematicMode", "Full"),
                                     "Failed to set SystematicMode");
      else RETURN_CHECK("JetCalibrator::initialize()",
                        m_JERSmearingTool_handle.setProperty("SystematicMode",
                                                             "Simple"),
                        "Failed to set SystematicMode");

      RETURN_CHECK("JetCalibrator::initialize()",
                   m_JERSmearingTool_handle.retrieve(),
                   "Failed to retrieve JERSmearingTool");
    }


    const CP::SystematicSet recSysts =
      m_JERSmearingTool_handle->recommendedSystematics();
    Info("initialize()", " Initializing JER Systematics :");

    std::vector<CP::SystematicSet> JERSysList =
      HelperFunctions::getListofSystematics(recSysts, m_systName, 1, m_debug); //
                                                                               //
                                                                               //
                                                                               // Only
                                                                               //
    ...
    for (unsigned int i = 0; i < JERSysList.size(); ++i) {
      ...
      // CP::SystematicSet() creates an empty systematic set, compared to the
      // set at index i
      if (JERSysList.at(i).empty() ||
          (JERSysList.at(i) == CP::SystematicSet())) { Info("initialize()",
                                                            "JERSysList Empty at index %d.",
                                                            i); continue; }
  ...
  m_JVTUpdateTool_handle.setTypeAndName(
    "JetVertexTaggerTool/JVTUpdateTool_" + m_name);

  if (m_redoJVT && !m_JVTUpdateTool_handle.isUserConfigured()) {
    RETURN_CHECK("JetCalibrator::initialize()",
                 ASG_MAKE_ANA_TOOL(m_JVTUpdateTool_handle,
                                   JetVertexTaggerTool),
                 "Could not make the tool");
    RETURN_CHECK("JetCalibrator::initialize()",
                 m_JVTUpdateTool_handle.setProperty("JVTFileName",
                                                    "JetMomentTools/JVTlikelihood_20140805.root"),
                 "");
    RETURN_CHECK("JetCalibrator::initialize()",
                 m_JVTUpdateTool_handle.retrieve(),
                 "Failed to retrieve JVTUpdateTool");
  }
...
EL::StatusCode JetCalibrator::execute()
{
  ...
  RETURN_CHECK("JetCalibrator::execute()",
               HelperFunctions::retrieve(inJets,
                                         m_inContainerName,
                                         m_event,
                                         m_store,
                                         m_verbose), "");
  ...
  std::pair<xAOD::JetContainer *,
            xAOD::ShallowAuxContainer *> calibJetsSC = xAOD::shallowCopyContainer(
    *inJets);

  // ConstDataVector<xAOD::JetContainer>* calibJetsCDV = new
  // ConstDataVector<xAOD::JetContainer>(SG::VIEW_ELEMENTS);
  ...
  RETURN_CHECK("JetCalibrator::execute()",
               m_store->record(calibJetsSC.first,
                               outSCContainerName),
               "Failed to record shallow copy container.");
  RETURN_CHECK("JetCalibrator::execute()",
               m_store->record(calibJetsSC.second,
                               outSCAuxContainerName),
               "Failed to record shallow copy aux container.");
  ...
  for (auto jet_itr : *(calibJetsSC.first)) {
    ...
    if (m_isMC && m_runSysts) {
      ...
      static SG::AuxElement::Decorator<char> accIsBjet("IsBjet"); // char due to
                                                                  // limitations
                                                                  // of ROOT
                                                                  // I/O, still
                                                                  // treat it as
    ...
    if (m_JetCalibrationTool_handle->applyCorrection(*jet_itr) ==
        CP::CorrectionCode::Error) {
      Error("execute()",
            "JetCalibration tool reported a CP::CorrectionCode::Error");
      Error("execute()", "%s", m_name.c_str());
      return StatusCode::FAILURE;
    }

    if (m_doJetTileCorr && !m_isMC) {
      if (m_JetTileCorrectionTool_handle->applyCorrection(*jet_itr) ==
          CP::CorrectionCode::Error) {
        Error("execute()",
              "JetTileCorrection tool reported a CP::CorrectionCode::Error");
      }
    }
  } // for jets
  ...
  for (const auto& syst_it : m_systList) {
    ...
    outSCAuxContainerName = m_outContainerName + syst_it.name() +
                            "ShallowCopyAux.";
    ...
    // create shallow copy;
    std::pair<xAOD::JetContainer *,
              xAOD::ShallowAuxContainer *> uncertCalibJetsSC =
      (thisSysType == 0) ? calibJetsSC : xAOD::shallowCopyContainer(
        *calibJetsSC.first);
    ConstDataVector<xAOD::JetContainer> *uncertCalibJetsCDV
      = new ConstDataVector<xAOD::JetContainer>(SG::VIEW_ELEMENTS);
    ...
    if (m_runSysts) {
      if (thisSysType == 1) {
        ...
        if (m_debug) std::cout << "Configure JES for systematic variation : " <<
            syst_it.name() << std::endl;

        if (m_JetUncertaintiesTool_handle->applySystematicVariation(syst_it) !=
            CP::SystematicCode::Ok) {
          Error("execute()",
                "Cannot configure JetUncertaintiesTool for systematic %s",
                m_systName.c_str());
        ...
        for (auto jet_itr : *(uncertCalibJetsSC.first)) {
          if (m_applyFatJetPreSel) {
            ...
            validForJES &=
              (jet_itr->m() / jet_itr->pt() >= 0 &&
               jet_itr->m() / jet_itr->pt() < 1);
            validForJES &= (fabs(jet_itr->eta()) < 2);

            if (!validForJES) continue;
          }

          if (m_JetUncertaintiesTool_handle->applyCorrection(*jet_itr) ==
              CP::CorrectionCode::Error) {
            Error("execute()",
                  "JetUncertaintiesTool reported a CP::CorrectionCode::Error");
            Error("execute()", "%s", m_name.c_str());
          }
        } // for jets
      }   // JES

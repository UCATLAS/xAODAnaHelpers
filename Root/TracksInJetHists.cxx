...
StatusCode TracksInJetHists::initialize() {
  ...
  // TrackHists
  //
  m_trkPlots = new TrackHists(m_name,
                              "IPDetails HitCounts TPErrors Chi2Details Debugging vsLumiBlock");
  m_trkPlots->initialize();

  //
  //  d0
  //
  m_trk_d0 = book(m_name,
                  "d0_signed",
                  "d0[mm] (signed)",
                  100,
                  -2.0,
  ...
  //  z0
  //
  m_trk_z0_signed = book(m_name,
                         "z0_signed",
                         "z0[mm] (signed)",
                         100,
                         -5.0,
                         5.0);
  m_trk_z0sinT_signed = book(m_name,
                             "z0sinT_signed",
                             "z0xsin(#theta)[mm] (signed)",
                             100,
                             -2.0,
                             2.0);
  m_trk_z0Sig_signed = book(m_name,
                            "z0Sig_signed",
                            "z0 significance (signed)",
                            100,
                            -25.0,
                            25.0);
  m_trk_z0Sig_signed_pdf = book(m_name,
                                "z0Sig_signed_pdf",
                                "z0 significance (signed)",
                                42,
                                -21.0,
                                20.0);
  m_trk_z0SigsinT_signed = book(m_name,
                                "z0SigsinT_signed",
...
StatusCode TracksInJetHists::execute(const xAOD::TrackParticle *trk,
                                     const xAOD::Jet           *jet,
  ...
  //  Fill track hists
  //
  RETURN_CHECK("TracksInJetHists::execute()",
               m_trkPlots->execute(trk, pvx, eventWeight, eventInfo), "");
  ...
  m_trk_jetdPhi->Fill(HelperFunctions::dPhi(trk->phi(),
                                            jet->p4().Phi()), eventWeight);
...
float TracksInJetHists::getD0Sign(const xAOD::TrackParticle *trk,
                                  const xAOD::Jet           *jet) {
...
float TracksInJetHists::getZ0Sign(const xAOD::TrackParticle *trk,
                                  const xAOD::Jet           *jet,
                                  const xAOD::Vertex        *pvx) {
  ...
  float trk_z0_wrtPV = trk->z0() + trk->vz() - HelperFunctions::getPrimaryVertexZ(
    pvx);

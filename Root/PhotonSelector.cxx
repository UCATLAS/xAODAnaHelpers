...
EL::StatusCode PhotonSelector::initialize() {
    ...
    if(m_debug) Info( "initialize()", "Adding isolation WP %s to IsolationSelectionTool", ( m_IsoKeys.at(0) ).c_str() ); RETURN_CHECK("PhotonSelector::initialize()", m_IsolationSelectionTool->setProperty( "PhotonWP", ( m_IsoKeys.at(0) ).c_str() ), "Failed to configure base WP");
    ...
    for(auto WP_itr = std::next( m_IsoKeys.begin() ); WP_itr != m_IsoKeys.end(); ++WP_itr) {
        if(m_debug) Info( "initialize()", "Adding extra isolation WP %s to IsolationSelectionTool", (*WP_itr).c_str() ); RETURN_CHECK("PhotonSelector::initialize()", m_IsolationSelectionTool->addPhotonWP( (*WP_itr).c_str() ), "Failed to add isolation WP");
    }
...
bool PhotonSelector::executeSelection(const xAOD::PhotonContainer* inPhotons,
  ...
  ConstDataVector<xAOD::PhotonContainer>* selectedPhotons) {
    ...
    return true;
} // PhotonSelector::executeSelection
...
bool PhotonSelector::passCuts(const xAOD::Photon* photon) {
    ...
    if(m_doAuthorCut) {
        if( !( (photon->author() & xAOD::EgammaParameters::AuthorPhoton) || (photon->author() & xAOD::EgammaParameters::AuthorAmbiguous) ) ) {
            if(m_debug) Info("PassCuts()", "Photon failed author kinematic cut."); return false;
        }
    }
    ...
    if(m_doOQCut) {
        if( ( (oq & 134217728) != 0 ) && ( (reta > 0.98) || (rphi > 1.0) || ( (oq & 67108864) != 0 ) ) ) {
            if(m_debug) Info("PassCuts()", "Electron failed Object Quality cut."); return 0;
        }
    }
    ...
    if(m_eta_max != 1e8) {
        if(fabs(eta) > m_eta_max) {
            if(m_debug) Info("PassCuts()", "Photon failed |eta| max cut."); return false;
        }
    }
    // |eta| crack veto
    //
    if(m_vetoCrack) {
        if( (fabs(eta) > 1.37) && (fabs(eta) < 1.52) ) {
            if(m_debug) Info("PassCuts()", "Photon failed |eta| crack veto cut."); return false;
        }
    }
    ...
    for(auto WP_itr : m_IsoKeys) {
        ...
        if(m_debug) Info( "PassCuts()", "Decorate photon with %s - accept() ? %i", decorWP.c_str(), accept_list.getCutResult( WP_itr.c_str() ) ); photon->auxdecor<char>(decorWP) = static_cast<char>( accept_list.getCutResult( WP_itr.c_str() ) );
    }

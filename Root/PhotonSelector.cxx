...
EL::StatusCode PhotonSelector::initialize()
{
  ...
  if (m_useCutFlow) {
    ...
    m_ph_cutflow_author_cut =
      m_ph_cutflowHist_1->GetXaxis()->FindBin("author_cut");
    ...
    m_ph_cutflow_ptmax_cut =
      m_ph_cutflowHist_1->GetXaxis()->FindBin("ptmax_cut");
    m_ph_cutflow_ptmin_cut =
      m_ph_cutflowHist_1->GetXaxis()->FindBin("ptmin_cut");
    m_ph_cutflow_eta_cut = m_ph_cutflowHist_1->GetXaxis()->FindBin("eta_cut"); //
                                                                               //
                                                                               //
                                                                               // including
                                                                               //
  ...
  if (asg::ToolStore::contains<CP::IsolationSelectionTool>(isoToolName.c_str())) {
    m_IsolationSelectionTool = asg::ToolStore::get<CP::IsolationSelectionTool>(
      isoToolName.c_str());
  } else {
    m_IsolationSelectionTool =
      new CP::IsolationSelectionTool(isoToolName.c_str());
  }

  if (m_debug) Info("initialize()",
                    "Adding isolation WP %s to IsolationSelectionTool",
                    (m_IsoKeys.at(0)).c_str()); RETURN_CHECK(
    "PhotonSelector::initialize()",
    m_IsolationSelectionTool->setProperty(
      "PhotonWP", (m_IsoKeys.at(
                     0)).c_str()),
    "Failed to configure base WP");
  RETURN_CHECK("PhotonSelector::initialize()",
  ...
  for (auto WP_itr = std::next(m_IsoKeys.begin()); WP_itr != m_IsoKeys.end();
    ...
    if (m_debug) Info("initialize()",
                      ...
                      (*WP_itr).c_str()); RETURN_CHECK(
      "PhotonSelector::initialize()",
      m_IsolationSelectionTool->addPhotonWP(
        (*WP_itr).c_str()),
...
EL::StatusCode PhotonSelector::execute()
{
  ...
  RETURN_CHECK("PhotonSelector::execute()",
               HelperFunctions::retrieve(eventInfo, m_eventInfoContainerName,
                                         m_event, m_store,
                                         m_verbose),
               Form("Failed in retrieving %s in %s",
                    m_eventInfoContainerName.c_str(), m_name.c_str()));
  ...
  if (m_inputAlgoSystNames.empty()) {
    ...
    if (m_createSelectedContainer) selectedPhotons =
    ...
    //
    eventPass = executeSelection(inPhotons,
                                 mcEvtWeight,
                                 countPass,
                                 selectedPhotons);
    ...
    if (m_createSelectedContainer) {
      if (eventPass) {
        ...
        //
        RETURN_CHECK("PhotonSelector::execute()",
                     m_store->record(selectedPhotons,
                                     m_outContainerName),
    ...
    for (auto systName : *systNames) {
      ...
      // find the selected photons, and return if event passes object selection
      //
      eventPassThisSyst = executeSelection(inPhotons,
                                           mcEvtWeight,
                                           countPass,
                                           selectedPhotons);
      ...
      if (m_debug) Info("execute()",
                        " syst name: %s  output container name: %s ",
                        systName.c_str(),
                        (m_outContainerName + systName).c_str());
    ...
    if (m_debug) Info("execute()", " output list of syst size: %i ",
                      static_cast<int>(vecOutContainerNames->size()));
...
bool PhotonSelector::executeSelection(
...
bool PhotonSelector::passCuts(const xAOD::Photon *photon)
{
  ...
  if ((!(photon->isAvailable<bool>(photonIDKeyName)))and(m_photonIdCut !=
                                                         "None")) {
    Error("passCuts()",
          Form(
    "Please call PhotonCalibrator before calling PhotonSelector, or check the quality requirement (should be either of Tight/Medium/Loose) [%s %s]",
               m_name.c_str(), photonIDKeyName.c_str()));
  }
  ...
  if (m_doOQCut) {
    if (((oq & 134217728) != 0) &&
        ((reta > 0.98) || (rphi > 1.0) || ((oq & 67108864) != 0))) {
  ...
  if (m_photonIdCut != "None") {
    ...
    if (!photon->auxdecor<bool>(photonIDKeyName)) {
      if (m_debug) Info("PassCuts()", "Photon failed ID cut."); return false;
    }
  }
  ...
  if (m_pT_max != 1e8) {
    if (et > m_pT_max) {
      if (m_debug) Info("PassCuts()", "Photon failed pT max cut."); return false;
    }
  }
  ...
  if (m_pT_min != 1e8) {
    if (et < m_pT_min) {
      if (m_debug) Info("PassCuts()", "Photon failed pT min cut."); return false;
    }
  }
  ...
  for (auto WP_itr : m_IsoKeys) {
    ...
    if (m_debug) Info("PassCuts()",
      "Decorate photon with %s - accept() ? %i",
      decorWP.c_str(),
      accept_list.getCutResult(WP_itr.c_str()));
    photon->auxdecor<char>(decorWP) =
    static_cast<char>(accept_list.getCutResult(WP_itr.c_str()));
  }

  // Apply the cut if needed
  //
  if (!m_MinIsoWPCut.empty() &&
  !accept_list.getCutResult(m_MinIsoWPCut.c_str())) {
    if (m_debug) Info("PassCuts()", "Photon failed isolation cut %s ",  m_MinIsoWPCut.c_str()); return false;
  }

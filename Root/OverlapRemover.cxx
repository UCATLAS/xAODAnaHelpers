...
EL::StatusCode OverlapRemover::initialize() {
    ...
    if( !m_inContainerName_Electrons.empty() ) m_useElectrons = true; if( !m_inContainerName_Muons.empty() ) m_useMuons = true; if( !m_inContainerName_Taus.empty() ) m_useTaus = true; if( !m_inContainerName_Photons.empty() ) m_usePhotons = true;
...
EL::StatusCode OverlapRemover::fillObjectCutflow(const xAOD::IParticleContainer* objCont, const std::string& overlapFlag, const std::string& selectFlag) {
    ...
    for( auto obj_itr : *(objCont) ) {
        ...
        switch( obj_itr->type() ) {
            ...
            case xAOD::Type::Jet:
                if( !overlapAcc(*obj_itr) ) m_jet_cutflowHist_1->Fill(m_jet_cutflow_OR_cut, 1);
                type = "jet";
                break;
            ...
            case xAOD::Type::Tau:
                if( !overlapAcc(*obj_itr) ) m_tau_cutflowHist_1->Fill(m_tau_cutflow_OR_cut, 1);
                type = "tau";
                break;
        ...
        if(m_debug) {
            ...
            if( objLinkAcc.isAvailable(*obj_itr) && objLinkAcc(*obj_itr).isValid() ) {
                ...
                std::stringstream ss_or;
                ss_or << overlapObj->type();
    ...
    return EL::StatusCode::SUCCESS;
} // OverlapRemover::fillObjectCutflow
...
EL::StatusCode OverlapRemover::executeOR(const xAOD::ElectronContainer* inElectrons, const xAOD::MuonContainer* inMuons, const xAOD::JetContainer* inJets,
  ...
  SystType syst_type, std::vector<std::string>* sysVec) {
    ...
    switch( static_cast<int>(syst_type) ) {
        ...
        {
            if(m_debug) Info("execute()", "Doing nominal case"); bool nomContainerNotFound(false);
            ...
            if(m_debug) {
                if(m_useElectrons) Info( "execute()", "inElectrons : %lu", inElectrons->size() ); if(m_useMuons) Info( "execute()", "inMuons : %lu", inMuons->size() ); Info( "execute()", "inJets : %lu", inJets->size() );
                if(m_usePhotons) Info( "execute()", "inPhotons : %lu", inPhotons->size() ); if(m_useTaus) Info( "execute()", "inTaus : %lu", inTaus->size() ); }

            // do the actual OR
            //
            if(m_debug) Info("execute()", "Calling removeOverlaps()"); RETURN_CHECK("OverlapRemover::execute()", m_ORToolbox.masterTool->removeOverlaps(inElectrons, inMuons, inJets, inTaus, inPhotons), "");
            if(m_debug) Info("execute()", "Done Calling removeOverlaps()");
            std::string ORdecor("passOR");
            if(m_useCutFlow) {
                // fill cutflow histograms
                //
                if(m_debug) Info("execute()", "Filling Cut Flow Histograms"); if(m_useElectrons) fillObjectCutflow(inElectrons);
            ...
            if(m_createSelectedContainers) {
                if(m_debug) Info("execute()", "Creating selected Containers"); if(m_useElectrons) selectedElectrons = new ConstDataVector<xAOD::ElectronContainer>(SG::VIEW_ELEMENTS);
            ...
            //
            if(m_debug) Info("execute()", "Resizing"); if(m_useElectrons) RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inElectrons, selectedElectrons, ORdecor.c_str(), ToolName::SELECTOR), ""); if(m_useMuons) RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inMuons, selectedMuons, ORdecor.c_str(), ToolName::SELECTOR), ""); RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inJets, selectedJets, ORdecor.c_str(), ToolName::SELECTOR), "");
            if(m_usePhotons) RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inPhotons, selectedPhotons, ORdecor.c_str(), ToolName::SELECTOR), ""); if(m_useTaus) RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inTaus, selectedTaus, ORdecor.c_str(), ToolName::SELECTOR), "");
            if(m_debug) {
                if(m_useElectrons) Info( "execute()", "selectedElectrons : %lu", selectedElectrons->size() ); if(m_useMuons) Info( "execute()", "selectedMuons : %lu", selectedMuons->size() ); Info( "execute()", "selectedJets : %lu", selectedJets->size() );
                if(m_usePhotons) Info( "execute()", "selectedPhotons : %lu", selectedPhotons->size() ); if(m_useTaus) Info( "execute()", "selectedTaus : %lu", selectedTaus->size() ); }

            // add ConstDataVector to TStore
            //
            if(m_createSelectedContainers) {
                if(m_debug) Info("execute()", "Recording"); if(m_useElectrons) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedElectrons, m_outContainerName_Electrons), "Failed to store const data container"); if(m_useMuons) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedMuons, m_outContainerName_Muons), "Failed to store const data container"); RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedJets, m_outContainerName_Jets), "Failed to store const data container");
                if(m_usePhotons) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedPhotons, m_outContainerName_Photons), "Failed to store const data container"); if(m_useTaus) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedTaus, m_outContainerName_Taus), "Failed to store const data container"); }
        ...
        case ELSYST: // electron syst
        {
            ...
            for(auto systName : *sysVec) {
                ...
                if(m_createSelectedContainers) {
                    ...
                    if(m_usePhotons) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedPhotons, m_outContainerName_Photons + systName), "Failed to store const data container"); if(m_useTaus) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedTaus, m_outContainerName_Taus + systName), "Failed to store const data container"); }
        ...
        case JETSYST: // jet systematics
        {
            ...
            for(auto systName : *sysVec) {
                ...
                // resize containers basd on OR decision
                //
                if(m_useElectrons) RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inElectrons, selectedElectrons, ORdecor.c_str(), ToolName::SELECTOR), ""); if(m_useMuons) RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inMuons, selectedMuons, ORdecor.c_str(), ToolName::SELECTOR), ""); RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inJets, selectedJets, ORdecor.c_str(), ToolName::SELECTOR), "");
                if(m_usePhotons) RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inPhotons, selectedPhotons, ORdecor.c_str(), ToolName::SELECTOR), ""); if(m_useTaus) RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inTaus, selectedTaus, ORdecor.c_str(), ToolName::SELECTOR), "");
                // add ConstDataVector to TStore
                //
                if(m_createSelectedContainers) {
                    // a different syst varied container will be stored for each syst variation
                    //
                    if(m_useElectrons) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedElectrons, m_outContainerName_Electrons + systName), "Failed to store const data container"); if(m_useMuons) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedMuons, m_outContainerName_Muons + systName), "Failed to store const data container"); RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedJets, m_outContainerName_Jets + systName), "Failed to store const data container");
                    if(m_usePhotons) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedPhotons, m_outContainerName_Photons + systName), "Failed to store const data container"); if(m_useTaus) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedTaus, m_outContainerName_Taus + systName), "Failed to store const data container"); }

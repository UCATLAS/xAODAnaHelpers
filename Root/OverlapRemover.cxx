  ...
  m_tau_cutflowHist_1(nullptr)
{
  ...
  m_outputAlgoSystNames = "ORAlgo_Syst"; // name of vector<string> of combined
                                         // syst pushed in TStore.

  /* Muons */
  m_inContainerName_Muons = "";
  m_inputAlgoMuons        = ""; // name of vector<string> of syst retrieved from
                                // TStore

  /* Electrons */
  m_inContainerName_Electrons = "";
  m_inputAlgoElectrons        = ""; // name of vector<string> of syst retrieved
                                    // from TStore

  /* Jets */
  m_inContainerName_Jets = "";
  m_inputAlgoJets        = ""; // name of vector<string> of syst retrieved from
                               // TStore

  /* Photons */
  m_inContainerName_Photons = "";
  m_inputAlgoPhotons        = ""; // name of vector<string> of syst retrieved
                                  // from TStore

  /* Taus */
  m_inContainerName_Taus = "";
  m_inputAlgoTaus        = ""; // name of vector<string> of syst retrieved from
                               // TStore
  ...
  m_useSelected = false;
  m_bTagWP      = ""; // Decoration name like "BTag", potentially from
...
EL::StatusCode OverlapRemover::initialize()
{
  ...
  if (m_inContainerName_Jets.empty()) {
    Error("initialize()",
          "InputContainerJets is empty! Must have it to perform Overlap Removal! Exiting.");
    return EL::StatusCode::FAILURE;
  }
  ...
  m_outAuxContainerName_Electrons = m_outContainerName_Electrons + "Aux."; // the
                                                                           // period
                                                                           // is
                                                                           // very
                                                                           // important!
  m_outAuxContainerName_Muons = m_outContainerName_Muons + "Aux.";         // the
                                                                           // period
                                                                           // is
                                                                           // very
                                                                           // important!
  m_outAuxContainerName_Jets = m_outContainerName_Jets + "Aux.";           // the
                                                                           // period
                                                                           // is
                                                                           // very
                                                                           // important!
  m_outAuxContainerName_Photons = m_outContainerName_Photons + "Aux.";     // the
                                                                           // period
                                                                           // is
                                                                           // very
                                                                           // important!
  m_outAuxContainerName_Taus = m_outContainerName_Taus + "Aux.";           // the
                                                                           // period
                                                                           // is
                                                                           // very
                                                                           // important!
  ...
  if (setCounters() == EL::StatusCode::FAILURE) {
    Error("initialize()",
          "Failed to properly set event/object counters. Exiting.");
    return EL::StatusCode::FAILURE;
  }

  // initialize ASG overlap removal tool
  const std::string selected_label = (m_useSelected) ? "passSel" : ""; // set
                                                                       // with
                                                                       // decoration
                                                                       // flag
                                                                       // you
  ...
  RETURN_CHECK("OverlapRemover::initialize()",
               ORUtils::recommendedTools(orFlags,
                                         m_ORToolbox),
               "Failed to get recommended tools in Overlap Removal Tool.");
  RETURN_CHECK("OverlapRemover::initialize()",
               m_ORToolbox.initialize(),
               "Failed to initialize Overlap Removal Tool.");
...
EL::StatusCode OverlapRemover::execute()
{
  ...
  // -----------------------------------------------------------------------------------------------
  //
  // if at least one of the m_inputAlgo* is not empty, and there's at least one
  // non-empty syst name,
  ...
  if (!m_inputAlgoElectrons.empty()) {
    ...
    // get vector of string giving the syst names (rememeber: 1st element is a
    // blank string: nominal case!)
    std::vector<std::string> *systNames_el(nullptr);
    RETURN_CHECK("OverlapRemover::execute()",
                 HelperFunctions::retrieve(systNames_el, m_inputAlgoElectrons, 0,
                                           m_store,
                                           m_verbose), "");

    if (HelperFunctions::found_non_dummy_sys(systNames_el)) {
      executeOR(inElectrons,
                inMuons,
                inJets,
                inPhotons,
                inTaus,
  ...
  if (!m_inputAlgoMuons.empty()) {
    ...
    RETURN_CHECK("OverlapRemover::execute()",
                 HelperFunctions::retrieve(systNames_mu, m_inputAlgoMuons, 0,
                                           m_store,
                                           m_verbose), "");
  ...
  if (!m_inputAlgoJets.empty()) {
    ...
    RETURN_CHECK("OverlapRemover::execute()",
                 HelperFunctions::retrieve(systNames_jet, m_inputAlgoJets, 0,
                                           m_store,
                                           m_verbose), "");
  ...
  if (!m_inputAlgoPhotons.empty()) {
    ...
    RETURN_CHECK("OverlapRemover::execute()",
                 HelperFunctions::retrieve(systNames_photon, m_inputAlgoPhotons,
                                           0, m_store,
                                           m_verbose), "");
  ...
  if (!m_inputAlgoTaus.empty()) {
    ...
    RETURN_CHECK("OverlapRemover::execute()",
                 HelperFunctions::retrieve(systNames_tau, m_inputAlgoTaus, 0,
                                           m_store,
                                           m_verbose), "");
  ...
  RETURN_CHECK("OverlapRemover::execute()",
               m_store->record(m_vecOutContainerNames,
...
EL::StatusCode OverlapRemover::fillObjectCutflow(
  const xAOD::IParticleContainer *objCont,
  const std::string             & overlapFlag,
  const std::string             & selectFlag)
{
  SG::AuxElement::ConstAccessor<char> selectAcc(selectFlag);
  SG::AuxElement::ConstAccessor<char> overlapAcc(overlapFlag);
  static SG::AuxElement::ConstAccessor<ElementLink<xAOD::IParticleContainer> >
  objLinkAcc("overlapObject");
  ...
  for (auto obj_itr : *(objCont)) {
    ...
    case xAOD::Type::Electron:
      ...
      if (!overlapAcc(*obj_itr)) m_el_cutflowHist_1->Fill(m_el_cutflow_OR_cut, 1);

      break;

    case xAOD::Type::Muon:

      if (!overlapAcc(*obj_itr)) m_mu_cutflowHist_1->Fill(m_mu_cutflow_OR_cut, 1);
    ...
    case xAOD::Type::Jet:
      ...
      if (!overlapAcc(*obj_itr)) m_jet_cutflowHist_1->Fill(m_jet_cutflow_OR_cut,
                                                           1);
      type = "jet";
      break;
    ...
    case xAOD::Type::Photon:
      ...
      if (!overlapAcc(*obj_itr)) m_ph_cutflowHist_1->Fill(m_ph_cutflow_OR_cut, 1);
    ...
    case xAOD::Type::Tau:
      ...
      if (!overlapAcc(*obj_itr)) m_tau_cutflowHist_1->Fill(m_tau_cutflow_OR_cut,
                                                           1);
      type = "tau";
      break;
    ...
    if (m_debug) {
      ...
      if (isBTag.isAvailable(*obj_itr) &&
          (isBTag(*obj_itr) == true)) isBTagged = 1;

      if (selectAcc.isAvailable(*obj_itr)) {
        Info("fillObjectCutflow()",
             "  %s pt %6.2f eta %5.2f phi %5.2f btagged %i selected %i passesOR %i ",
             type.c_str(),
             (obj_itr)->pt() / 1000.,
             (obj_itr)->eta(),
             (obj_itr)->phi(),
        ...
        Info("fillObjectCutflow()",
             "  %s pt %6.2f eta %5.2f phi %5.2f btagged %i passesOR %i ",
             type.c_str(),
             (obj_itr)->pt() / 1000.,
             (obj_itr)->eta(),
             (obj_itr)->phi(),
      ...
      if (objLinkAcc.isAvailable(*obj_itr) && objLinkAcc(*obj_itr).isValid()) {
        ...
        Info("fillObjectCutflow()", " Overlap: type %s pt %6.2f",
             (ss_or.str()).c_str(), overlapObj->pt() / 1e3);
      }
    }
  }
...
EL::StatusCode OverlapRemover::executeOR(
  const xAOD::ElectronContainer *inElectrons,
  ...
  std::vector<std::string>      *sysVec)
{
  ...
  case NOMINAL: // this is the nominal case
  {
    ...
    if (m_useElectrons) {
      if (m_store->contains<ConstDataVector<xAOD::ElectronContainer> >(
            m_inContainerName_Electrons)) {
        RETURN_CHECK("OverlapRemover::execute()",
        ...
        if (m_numEvent == 1) Warning("executeOR()",
                                     "Could not find nominal container %s in xAOD::TStore. Overlap Removal will not be done for the 'all-nominal' case...",
                                     m_inContainerName_Electrons.c_str());
      }
    }

    if (m_useMuons) {
      if (m_store->contains<ConstDataVector<xAOD::MuonContainer> >(
            m_inContainerName_Muons)) {
        RETURN_CHECK("OverlapRemover::execute()",
                     HelperFunctions::retrieve(inMuons, m_inContainerName_Muons,
                                               m_event,
                                               m_store, m_verbose), "");
      } else {
        nomContainerNotFound = true;
        ...
        if (m_numEvent == 1) Warning("executeOR()",
                                     "Could not find nominal container %s in xAOD::TStore. Overlap Removal will not be done for the 'all-nominal' case...",
                                     m_inContainerName_Muons.c_str());
      }
    }

    if (m_store->contains<ConstDataVector<xAOD::JetContainer> >(
          m_inContainerName_Jets)) {
      RETURN_CHECK("OverlapRemover::execute()",
                   HelperFunctions::retrieve(inJets, m_inContainerName_Jets,
                                             m_event,
                                             m_store, m_verbose), "");
    } else {
      nomContainerNotFound = true;
      ...
      if (m_numEvent == 1) Warning("executeOR()",
                                   "Could not find nominal container %s in xAOD::TStore. Overlap Removal will not be done for the 'all-nominal' case...",
                                   m_inContainerName_Jets.c_str());
    }

    if (m_usePhotons) {
      if (m_store->contains<ConstDataVector<xAOD::PhotonContainer> >(
            m_inContainerName_Photons)) {
        RETURN_CHECK("OverlapRemover::execute()",
                     HelperFunctions::retrieve(inPhotons,
                                               m_inContainerName_Photons, m_event,
                                               m_store, m_verbose), "");
      } else {
        nomContainerNotFound = true;
        ...
        if (m_numEvent == 1) Warning("executeOR()",
                                     "Could not find nominal container %s in xAOD::TStore. Overlap Removal will not be done for the 'all-nominal' case...",
                                     m_inContainerName_Photons.c_str());
      }
    }

    if (m_useTaus) {
      if (m_store->contains<ConstDataVector<xAOD::TauJetContainer> >(
            m_inContainerName_Taus)) {
        RETURN_CHECK("OverlapRemover::execute()",
                     HelperFunctions::retrieve(inTaus, m_inContainerName_Taus,
                                               m_event,
                                               m_store, m_verbose), "");
      } else {
        nomContainerNotFound = true;
    ...
    if (m_debug) {
      if (m_useElectrons) Info("execute()",
                               "inElectrons : %lu",
                               inElectrons->size());

      if (m_useMuons) Info("execute()",  "inMuons : %lu", inMuons->size()); Info(
        "execute()",
        "inJets : %lu",
        inJets->size());
    ...
    // do the actual OR
    //
    if (m_debug) Info("execute()",  "Calling removeOverlaps()"); RETURN_CHECK(
      "OverlapRemover::execute()",
      m_ORToolbox.masterTool->removeOverlaps(inElectrons, inMuons, inJets, inTaus,
                                             inPhotons),
      "");
    ...
    if (m_createSelectedContainers) {
      ...
      if (m_useMuons) selectedMuons = new ConstDataVector<xAOD::MuonContainer>(
          SG::VIEW_ELEMENTS);
      selectedJets = new ConstDataVector<xAOD::JetContainer>(SG::VIEW_ELEMENTS);

      if (m_usePhotons) selectedPhotons =
    ...
    //
    // if an object has been flagged as 'passOR', it will be stored in the
    // 'selected' container
    //
    ...
    if (m_useMuons) RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inMuons, selectedMuons, ORdecor.c_str(), ToolName::SELECTOR), ""); RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inJets, selectedJets, ORdecor.c_str(), ToolName::SELECTOR), "");
    ...
    if (m_debug) {
      ...
      if (m_useMuons) Info("execute()",  "selectedMuons : %lu",     selectedMuons->size()); Info("execute()",  "selectedJets : %lu", selectedJets->size());
    ...
    if (m_createSelectedContainers) {
      ...
      if (m_useMuons) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedMuons,  m_outContainerName_Muons), "Failed to store const data container"); RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedJets,  m_outContainerName_Jets), "Failed to store const data container");
  ...
  {
    if (m_debug) Info("execute()",  "Doing  electron systematics"); bool nomContainerNotFound(false);
    ...
    for (auto systName : *sysVec) {
      ...
      // ... instead, the electron input container will be different for each
      // syst
      //
      ...
      //
      if (m_createSelectedContainers) {
        // a different syst varied container will be stored for each syst
        // variation
        ...
        if (m_useMuons) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedMuons,     m_outContainerName_Muons + systName), "Failed to store const data container"); RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedJets,      m_outContainerName_Jets + systName), "Failed to store const data container");
  ...
  {
    if (m_debug) Info("execute()",  "Doing  muon systematics"); bool nomContainerNotFound(false);
    ...
    for (auto systName : *sysVec) {
      ...
      //
      if (m_useElectrons) RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inElectrons, selectedElectrons, ORdecor.c_str(), ToolName::SELECTOR), ""); RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inMuons, selectedMuons, ORdecor.c_str(), ToolName::SELECTOR), "");
      ...
      if (m_createSelectedContainers) {
        ...
        // variation
        //
        if (m_useElectrons) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedElectrons, m_outContainerName_Electrons + systName), "Failed to store const data container"); RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedMuons,     m_outContainerName_Muons + systName), "Failed to store const data container");
  ...
  {
    if (m_debug) Info("execute()",  "Doing  jet systematics"); bool nomContainerNotFound(false);
  ...
  {
    if (m_debug) Info("execute()",  "Doing  photon systematics"); bool nomContainerNotFound(false);
  ...
  {
    if (m_debug) Info("execute()",  "Doing tau systematics"); bool nomContainerNotFound(false);
    ...
    for (auto systName : *sysVec) {
      ...
      if (m_usePhotons) RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inPhotons, selectedPhotons, ORdecor.c_str(), ToolName::SELECTOR), ""); RETURN_CHECK("OverlapRemover::execute()", HelperFunctions::makeSubsetCont(inTaus, selectedTaus, ORdecor.c_str(), ToolName::SELECTOR), "");
      ...
      if (m_createSelectedContainers) {
        ...
        if (m_usePhotons) RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedPhotons,   m_outContainerName_Photons + systName), "Failed to store const data container"); RETURN_CHECK("OverlapRemover::execute()", m_store->record(selectedTaus, m_outContainerName_Taus + systName), "Failed to store const data container");
      }

...
EL::StatusCode OverlapRemover :: fillObjectCutflow (const xAOD::IParticleContainer* objCont, const std::string& overlapFlag, const std::string& selectFlag)
{
  ...
  for ( auto obj_itr : *(objCont) ) {
    ...
    if ( m_debug ) {
      ...
      ATH_MSG_INFO( type << " pt " << obj_itr->pt()/1e3 << " eta " << obj_itr->eta() << " phi " << obj_itr->phi() << " btagged " << isBTagged << (selectAcc.isAvailable(*obj_itr)?(" selected " << selectAcc(*obj_itr)):"") << " passesOR  " << overlapAcc( *obj_itr ) );

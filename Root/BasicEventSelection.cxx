...
BasicEventSelection::BasicEventSelection(std::string className) :
  ...
  m_reweightSherpa22_tool_handle(
    "PMGTools::PMGSherpa22VJetsWeightTool/ReweightSherpa22"),
  ...
  m_duplicatesTree(nullptr)
{
  ...
  m_GRLxml =
    "$ROOTCOREBIN/data/xAODAnaHelpers/data15_13TeV.periodAllYear_HEAD_DQDefects-00-01-02_PHYS_StandardGRL_Atlas_Ready.xml";
...
EL::StatusCode BasicEventSelection::histInitialize()
{
  ...
  // write the metadata hist to this file so algos downstream can pick up the
  // pointer
  ...
  if (!m_histEventCount) {
    m_histEventCount = new TH1D("MetaData_EventCount",
                                "MetaData_EventCount",
                                6,
                                0.5,
                                6.5);
...
EL::StatusCode BasicEventSelection::fileExecute()
{
  ...
  // get TEvent and TStore - must be done here b/c we need to retrieve
  // CutBookkeepers container from TEvent!
  //
  ...
  // if running on a MC DAOD which had some skimming applied at the derivation
  // stage
  ...
  if (m_useMetaData) {
    ...
    if (!m_event->retrieveMetaInput(incompleteCBC,
                                    "IncompleteCutBookkeepers").isSuccess()) {
      Error("fileExecute()",
            "Failed to retrieve IncompleteCutBookkeepers from MetaData! Exiting.");
      return EL::StatusCode::FAILURE;
    }
    ...
    if (incompleteCBC->size() != 0) {
      ...
      for (auto cbk : *incompleteCBC) {
        Info("fileExecute()", "Incomplete cbk name: %s - stream: %s ",
             (cbk->name()).c_str(), (cbk->inputStream()).c_str());
      ...
      if (!allFromUnknownStream) Warning("fileExecute()",
                                         "Found incomplete CBK from stream: %s. This is not necessarily a sign of file corruption (incomplete CBK appear when 'maxevents' is set in the AOD jo, for instance), but you may still want to check input file for potential corruption...",
                                         stream.c_str());
    }
    ...
    for (auto cbk : *completeCBC)
      if (!(cbk->name().empty())  &&
          (minCycle > cbk->cycle())) minCycle = cbk->cycle();

    // Now, let's actually find the right one that contains all the needed
    // info...
    ...
    if (m_isDerivation) {
      if (m_derivationName != "") {
        Info("fileExecute()",
             "Override auto config to look at DAOD made by Derivation Algorithm: %s",
             m_derivationName.c_str());
      } else {
        Info("fileExecute()",
             "Will autoconfig to look at DAOD made by Derivation Algorithm.");
      }
    }
    ...
    for (const auto& cbk: *completeCBC) {
      Info("fileExecute()", "Complete cbk name: %s - stream: %s",
           (cbk->name()).c_str(), (cbk->inputStream()).c_str());
      ...
      if (m_isDerivation) {
        if (m_derivationName != "") {
          ...
          Info("fileExecute()",
               "Auto config found DAOD made by Derivation Algorithm: %s",
               cbk->name().c_str());
          DxAODEventsCBK = cbk;
        }
      } // is derivation
    ...
    if (m_isDerivation && !DxAODEventsCBK) {
      Error("fileExecute()",
            "No CutBookkeeper corresponds to the selected Derivation Framework algorithm name. Check it with your DF experts! Aborting.");
      return EL::StatusCode::FAILURE;
    }

    m_MD_finalNevents =
      (m_isDerivation) ? DxAODEventsCBK->nAcceptedEvents() : m_MD_initialNevents;
    m_MD_finalSumW =
      (m_isDerivation) ? DxAODEventsCBK->sumOfEventWeights() : m_MD_initialSumW;
    m_MD_finalSumWSquared =
      (m_isDerivation) ? DxAODEventsCBK->sumOfEventWeightsSquared() :
    ...
    Info("fileExecute()",
         ...
         static_cast<unsigned int>(
           m_MD_finalNevents));
    Info("fileExecute()", "Initial  sum of weights = %f",
         m_MD_initialSumW);
    Info("fileExecute()", "Selected sum of weights = %f", m_MD_finalSumW);
    Info("fileExecute()",
         "Initial  sum of weights squared = %f",
         m_MD_initialSumWSquared);
    Info("fileExecute()",
         "Selected sum of weights squared = %f",
         m_MD_finalSumWSquared);
...
EL::StatusCode BasicEventSelection::initialize()
{
  ...
  RETURN_CHECK("BasicEventSelection::initialize()",
  ...
  if (m_applyGRLCut) {
    ...
    if (m_GRLExcludeList.find(runNumString) != std::string::npos) {
      Info("initialize()",
           "RunNumber is in GRLExclusion list, setting applyGRL to false");
      m_applyGRLCut = false;
    }
  }
  ...
  if (eventInfo->runNumber() == 426005) { // Powheg+Pythia J5
    m_cleanPowheg = true;
    Info("initialize()",
         "This is J5 Powheg - cleaning that nasty huge weight event");
  }
  ...
  if (m_isMC &&
      (((eventInfo->mcChannelNumber() >= 363331) &&
        (eventInfo->mcChannelNumber() <= 363483)) ||
       ((eventInfo->mcChannelNumber() >= 363102) &&
        (eventInfo->mcChannelNumber() <= 363122)) ||
       ((eventInfo->mcChannelNumber() >= 363361) &&
        (eventInfo->mcChannelNumber() <= 363435)))) {
    Info("initialize()",
         "This is Sherpa 2.2 dataset and should be reweighted.  An extra weight will be saved to EventInfo called \"weight_Sherpa22\".");
    m_reweightSherpa22 = true;

    // Choose Jet Truth container, WZ has more information and is favored by the
    // tool
    ...
    if (m_event->contains<xAOD::JetContainer>("AntiKt4TruthWZJets")) {
      ...
      Warning("initialize()",
              "No Truth Jet Container found for Sherpa 22 reweighting, weight_Sherpa22 will not be set.");
      m_reweightSherpa22 = false;
    }
    ...
    if (m_reweightSherpa22) {
      if (!m_reweightSherpa22_tool_handle.isUserConfigured()) {
        RETURN_CHECK("BasicEventSelection::initialize()",
                     checkToolStore<PMGTools::PMGSherpa22VJetsWeightTool>(
                       "ReweightSherpa22"),
                     "Failed to check whether tool already exists in asg::ToolStore");
        RETURN_CHECK("initialize()",
                     ASG_MAKE_ANA_TOOL(m_reweightSherpa22_tool_handle,
                                       PMGTools::PMGSherpa22VJetsWeightTool),
                     "Could not make the tool");
        RETURN_CHECK("initialize()",
                     m_reweightSherpa22_tool_handle.setProperty(
                       "TruthJetContainer",
                       pmg_TruthJetContainer),
                     "Failed to set TruthJetContainer");
        RETURN_CHECK("initialize()",
                     m_reweightSherpa22_tool_handle.retrieve(),
                     "Failed to properly retrieve PMGTools::PMGSherpa22VJetsWeightTool");
      }
    }
  ...
  // Note: the following code is needed for anyone developing/running in ROOT
  // 6.04.10+
  // Bin extension is not done anymore via TH1::SetBit(TH1::kCanRebin), but with
  // TH1::SetCanExtend(TH1::kAllAxes)

  // initialise event cutflow, which will be picked ALSO by the algos downstream
  // where an event selection is applied (or at least can be applied)
  //
  ...
  // initialise object cutflows, which will be picked by the object selector
  // algos downstream and filled.
  //
  m_el_cutflowHist_1 = new TH1D("cutflow_electrons_1",
                                "cutflow_electrons_1",
                                1,
                                1,
                                2);
  m_el_cutflowHist_1->SetCanExtend(TH1::kAllAxes);
  m_el_cutflowHist_2 = new TH1D("cutflow_electrons_2",
                                "cutflow_electrons_2",
                                1,
                                1,
                                2);
  ...
  m_ph_cutflowHist_1 =
    new TH1D("cutflow_photons_1", "cutflow_photons_1", 1, 1, 2);
  ...
  m_truth_cutflowHist_1 =
    new TH1D("cutflow_truths_1", "cutflow_truths_1", 1, 1, 2);
  ...
  if (m_doPUreweighting) {
    ...
    // RETURN_CHECK("BasicEventSelection::initialize()",
    // checkToolStore<CP::PileupReweightingTool>("Pileup"), "Failed to check
    // whether tool already exists in asg::ToolStore" );
    ////RETURN_CHECK("BasicEventSelection::initialize()",
    // m_pileup_tool_handle.makeNew<CP::PileupReweightingTool>("CP::PileupReweightingTool/Pileup"),
    // "Failed to create handle to CP::PileupReweightingTool");
    // RETURN_CHECK("initialize()", ASG_MAKE_ANA_TOOL(m_pileup_tool_handle,
    // CP::PileupReweightingTool), "Could not make the tool");
    //
    ...
    RETURN_CHECK("BasicEventSelection::initialize()",
                 m_pileup_tool_handle.setProperty("ConfigFiles", PRWFiles),
                 "");
    RETURN_CHECK("BasicEventSelection::initialize()",
                 m_pileup_tool_handle.setProperty("LumiCalcFiles", lumiCalcFiles),
                 "");
    RETURN_CHECK("BasicEventSelection::initialize()",
                 m_pileup_tool_handle.setProperty("DataScaleFactor", 1.0 / 1.09),
                 "Failed to set pileup reweighting data scale factor");
    RETURN_CHECK("BasicEventSelection::initialize()",
                 m_pileup_tool_handle.setProperty("DataScaleFactorUP", 1.0),
                 "Failed to set pileup reweighting data scale factor up");
    RETURN_CHECK("BasicEventSelection::initialize()",
                 m_pileup_tool_handle.setProperty("DataScaleFactorDOWN",
                                                  1.0 / 1.18),
                 "Failed to set pileup reweighting data scale factor down");

    // RETURN_CHECK("BasicEventSelection::initialize()",
    // m_pileup_tool_handle.initialize(), "Failed to properly initialize
    // CP::PileupReweightingTool");
    RETURN_CHECK("BasicEventSelection::retrieve()",
                 m_pileup_tool_handle.retrieve(),
                 "Failed to properly retrieve CP::PileupReweightingTool");

    //    RETURN_CHECK("BasicEventSelection::retrieve()",
    // asg::ToolStore::put(m_pileup_tool_handle.get()), "Failed to put");
  }

  // pileup reweighing tool is needed to get the data weight for unprescaling
  if (m_savePrescaleDataWeight && !m_doPUreweighting) {
    Error("initialize()",
          "m_savePrescaleDataWeight is true but m_doPUreweighting is false !!!");
    return EL::StatusCode::FAILURE;
  }
  ...
  if (!m_triggerSelection.empty() || !m_extraTriggerSelection.empty() ||
      m_applyTriggerCut || m_storeTrigDecisions || m_storePassL1 ||
      m_storePassHLT || m_storeTrigKeys) {
    // if it's there, it must be already configured
    if (asg::ToolStore::contains<Trig::TrigDecisionTool>("TrigDecisionTool") &&
        asg::ToolStore::contains<TrigConf::xAODConfigTool>("xAODConfigTool")) {
      m_trigDecTool = asg::ToolStore::get<Trig::TrigDecisionTool>(
        "TrigDecisionTool");
      m_trigConfTool = asg::ToolStore::get<TrigConf::xAODConfigTool>(
        "xAODConfigTool");
    }
      ...
      RETURN_CHECK("BasicEventSelection::initialize()",
                   ...
                   "Failed to properly initialize TrigConf::xAODConfigTool");
      ...
      RETURN_CHECK("BasicEventSelection::initialize()",
                   ...
                   "Failed to properly initialize Trig::TrigDecisionTool");
...
EL::StatusCode BasicEventSelection::execute()
{
  ...
  if (!m_triggerSelection.empty()) {
    if ((m_eventCounter == 0) || m_savePrescaleDataWeight) {
      if (m_eventCounter == 0) Info("execute()",
                                    "*** Triggers used (in OR) are:\n");
      auto printingTriggerChainGroup = m_trigDecTool->getChainGroup(
        m_triggerSelection);
      std::vector<std::string> triggersUsed =
        printingTriggerChainGroup->getListOfTriggers();

      for (unsigned int iTrigger = 0; iTrigger < triggersUsed.size();
           ++iTrigger) {
  ...
  // ------------------
  const xAOD::EventInfo *eventInfo(nullptr);
  RETURN_CHECK("BasicEventSelection::execute()",
               HelperFunctions::retrieve(eventInfo, m_eventInfoContainerName,
                                         m_event, m_store,
                                         m_verbose), "");
  ...
  if (!mcEvtWeightDecor.isAvailable(*eventInfo)) {
    if (m_isMC) {
      const std::vector<float> weights = eventInfo->mcEventWeights(); // The
                                                                      // weight
                                                                      // (and
                                                                      // systs)
                                                                      // of all
      ...
      if (m_cleanPowheg) {
        if (eventInfo->eventNumber() == 1652845) {
          Info("execute()",
               "Dropping huge weight event. Weight should be 352220000");
  ...
  // https://cds.cern.ch/record/2014726/files/ATL-COM-SOFT-2015-119.pdf (line
  // 130)
  // ------------------------------------------------------------------------------------------

  static SG::AuxElement::Decorator<float> prsc_DataWeightDecor(
    "prescale_DataWeight");
  ...
  if (!prsc_DataWeightDecor.isAvailable(*eventInfo)) {
    if (!m_isMC && m_savePrescaleDataWeight) {
      ...
      prsc_DataWeight = m_pileup_tool_handle->getDataWeight(*eventInfo,
                                                            TriggerExpression,
                                                            true);
    }
  ...
  // ------------------------------------------------------------------------------------------
  // Declare an 'eventInfo' decorator with the Sherpa 2.2 reweight to multijet
  // truth
  // https://twiki.cern.ch/twiki/bin/viewauth/AtlasProtected/CentralMC15ProductionList#Sherpa_v2_2_0_V_jets_NJet_reweig
  // ------------------------------------------------------------------------------------------
  ...
  if (m_reweightSherpa22) {
    ...
    if (!weight_Sherpa22Decor.isAvailable(*eventInfo)) {
      ...
      weight_Sherpa22 =
        m_reweightSherpa22_tool_handle->getWeight();
      weight_Sherpa22Decor(*eventInfo) = weight_Sherpa22;

      if (m_debug) Info("exectue()",
                        "Setting Sherpa 2.2 reweight to %f",
                        weight_Sherpa22);
  ...
  // This is done by checking against the std::set of <runNumber,eventNumber>
  // filled for all previous events
  // --------------------------------------------------------------------------------------------------------

  if ((!m_isMC && m_checkDuplicatesData) || (m_isMC && m_checkDuplicatesMC)) {
    std::pair<uint32_t, uint32_t> thispair = std::make_pair(
      eventInfo->runNumber(), eventInfo->eventNumber());
  ...
  if (m_isMC && m_doPUreweighting) {
    ...
    m_pileup_tool_handle->apply(*eventInfo); // NB: this call automatically

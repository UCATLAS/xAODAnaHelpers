...
MuonContainer::MuonContainer(const std::string& name,
...
void MuonContainer::setTree(TTree *tree)
{
  ...
  if (m_infoSwitch.m_trigger) {
    ...
    connectBranch<vector<int> >(tree,
                                "isTrigMatchedToChain",
                                &m_isTrigMatchedToChain);
    connectBranch<string>(      tree, "listTrigChains", &m_listTrigChains);
  }

  if (m_infoSwitch.m_isolation) {
    connectBranch<int>(  tree,
                         "isIsolated_LooseTrackOnly",
                         &m_isIsolated_LooseTrackOnly);
    connectBranch<int>(  tree,
                         "isIsolated_Loose",
                         &m_isIsolated_Loose);
    connectBranch<int>(  tree,
    ...
    connectBranch<int>(  tree,
                         "isIsolated_Gradient",
                         &m_isIsolated_Gradient);
    connectBranch<int>(  tree,
                         "isIsolated_GradientLoose",
                         &m_isIsolated_GradientLoose);
    connectBranch<int>(  tree,
                         "isIsolated_FixedCutLoose",
                         &m_isIsolated_FixedCutLoose);
    connectBranch<int>(  tree,
                         "isIsolated_FixedCutTightTrackOnly",
                         &m_isIsolated_FixedCutTightTrackOnly);
    connectBranch<int>(  tree,
                         "isIsolated_UserDefinedFixEfficiency",
                         &m_isIsolated_UserDefinedFixEfficiency);
    connectBranch<int>(  tree,
                         "isIsolated_UserDefinedCut",
                         &m_isIsolated_UserDefinedCut);
    connectBranch<float>(tree, "ptcone20",
                         &m_ptcone20);
    connectBranch<float>(tree, "ptcone30",
                         &m_ptcone30);
    connectBranch<float>(tree, "ptcone40",
                         &m_ptcone40);
    connectBranch<float>(tree,
                         "ptvarcone20",
                         &m_ptvarcone20);
    connectBranch<float>(tree,
                         "ptvarcone30",
  ...
  if (m_infoSwitch.m_trackhitcont) {
    ...
    connectBranch<int>(  tree,
                         ...
                         &m_trknInnermostPixLayHits);
    connectBranch<float>(tree, "trkPixdEdX", &m_trkPixdEdX);
  }

  if (m_infoSwitch.m_energyLoss) {
    connectBranch<float>(        tree, "EnergyLoss",
                                 &m_EnergyLoss);
    connectBranch<float>(        tree,
                                 "EnergyLossSigma",
                                 &m_EnergyLossSigma);
    connectBranch<unsigned char>(tree,
                                 "energyLossType",
                                 &m_energyLossType);
    connectBranch<float>(        tree,
                                 "MeasEnergyLoss",
    ...
    connectBranch<float>(        tree,
                                 "MeasEnergyLossSigma",
                                 &m_MeasEnergyLossSigma);
    connectBranch<float>(        tree,
                                 "ParamEnergyLoss",
                                 &m_ParamEnergyLoss);
    connectBranch<float>(        tree,
                                 "ParamEnergyLossSigmaMinus",
                                 &m_ParamEnergyLossSigmaMinus);
    connectBranch<float>(        tree,
                                 "ParamEnergyLossSigmaPlus",
                                 &m_ParamEnergyLossSigmaPlus);
  }
}
...
void MuonContainer::updateParticle(uint idx, Muon& muon)
{
  ...
  // isolation
  if (m_infoSwitch.m_isolation) {
    muon.isIsolated_LooseTrackOnly =
      m_isIsolated_LooseTrackOnly->at(idx);
    ...
    muon.isIsolated_GradientLoose =     m_isIsolated_GradientLoose->at(
      idx);
    muon.isIsolated_FixedCutLoose =     m_isIsolated_FixedCutLoose->at(
      idx);
    muon.isIsolated_FixedCutTightTrackOnly =
      m_isIsolated_FixedCutTightTrackOnly->at(idx);
    muon.isIsolated_UserDefinedFixEfficiency =
      m_isIsolated_UserDefinedFixEfficiency->at(idx);
    muon.isIsolated_UserDefinedCut =
      m_isIsolated_UserDefinedCut->at(idx);
  ...
  if (m_infoSwitch.m_trackhitcont) {
    ...
    muon.trknInnermostPixLayHits = m_trknInnermostPixLayHits->at(idx); // not
                                                                       // available
                                                                       // in
                                                                       // DC14
    muon.trkPixdEdX = m_trkPixdEdX->at(idx);                           // not
                                                                       // available
                                                                       // in
                                                                       // DC14
  }
...
void MuonContainer::setBranches(TTree *tree)
{
  ...
  if (m_infoSwitch.m_trigger) {
    ...
    // a vector of strings for each muon trigger chain - 1:1 correspondence w/
    // vector above
...
void MuonContainer::FillMuon(const xAOD::Muon   *muon,
                             const xAOD::Vertex *primaryVertex) {
  return FillMuon(static_cast<const xAOD::IParticle *>(muon), primaryVertex);
}

void MuonContainer::FillMuon(const xAOD::IParticle *particle,
                             const xAOD::Vertex    *primaryVertex)
{
  ...
  if (m_infoSwitch.m_trigger) {
    ...
    //
    static SG::AuxElement::Accessor<std::map<std::string,
                                             char> > isTrigMatchedMapMuAcc(
      "isTrigMatchedMapMu");
    ...
    // if at least one match among the chains is found, say this muon is trigger
    // matched
    if (std::find(matches.begin(), matches.end(),
                  1) != matches.end()) m_isTrigMatched->push_back(1);
    else m_isTrigMatched->push_back(0);
  }


  if (m_infoSwitch.m_isolation) {
    static SG::AuxElement::Accessor<char> isIsoLooseTrackOnlyAcc(
      "isIsolated_LooseTrackOnly");
    safeFill<char, int, xAOD::Muon>(muon,
                                    isIsoLooseTrackOnlyAcc,
                                    m_isIsolated_LooseTrackOnly,
                                    -1);
    ...
    static SG::AuxElement::Accessor<char> isIsoGradientLooseAcc(
      "isIsolated_GradientLoose");
    safeFill<char, int, xAOD::Muon>(muon,
                                    isIsoGradientLooseAcc,
                                    m_isIsolated_GradientLoose,
                                    -1);

    static SG::AuxElement::Accessor<char> isIsoFixedCutLooseAcc(
      "isIsolated_FixedCutLoose");
    safeFill<char, int, xAOD::Muon>(muon,
                                    isIsoFixedCutLooseAcc,
                                    m_isIsolated_FixedCutLoose,
                                    -1);

    static SG::AuxElement::Accessor<char> isIsoFixedCutTightTrackOnlyAcc(
      "isIsolated_FixedCutTightTrackOnly");
    safeFill<char, int, xAOD::Muon>(muon,
                                    isIsoFixedCutTightTrackOnlyAcc,
                                    m_isIsolated_FixedCutTightTrackOnly,
                                    -1);

    static SG::AuxElement::Accessor<char> isIsoUserDefinedFixEfficiencyAcc(
      "isIsolated_UserDefinedFixEfficiency");
    safeFill<char, int, xAOD::Muon>(muon,
                                    isIsoUserDefinedFixEfficiencyAcc,
                                    m_isIsolated_UserDefinedFixEfficiency,
                                    -1);

    static SG::AuxElement::Accessor<char> isIsoUserDefinedCutAcc(
      "isIsolated_UserDefinedCut");
    safeFill<char, int, xAOD::Muon>(muon,
                                    isIsoUserDefinedCutAcc,
                                    m_isIsolated_UserDefinedCut,
                                    -1);
  ...
  if (m_infoSwitch.m_trackparams) {
    if (trk) {
      //
      // NB.:
      // All track parameters are calculated at the perigee, i.e., the point of
      // closest approach to the origin of some r.f. (which in RunII is NOT the
      // ATLAS detector r.f!).
      // The reference  frame is chosen to be a system centered in the beamspot
      // position, with z axis parallel to the beam line.
      // Remember that the beamspot size ( of O(10 micrometers) in the
      // transverse plane) is << average vertex transverse position resolution (
      // O(60-80 micrometers) )
      // The coordinates of this r.f. wrt. the ATLAS system origin are returned
      // by means of vx(), vy(), vz()
      //
      m_trkd0->push_back(trk->d0());

      static SG::AuxElement::Accessor<float> d0SigAcc("d0sig");
      float d0_significance =  (d0SigAcc.isAvailable(*muon)) ? fabs(d0SigAcc(
                                                                      *muon)) : -
                              1.0;
      ...
      float z0sintheta =
        (z0sinthetaAcc.isAvailable(*muon)) ? z0sinthetaAcc(*muon) : -999.0;
  ...
  if (m_infoSwitch.m_trackhitcont) {
    uint8_t nPixHits(-1), nPixHoles(-1), nSCTHits(-1), nSCTHoles(-1),
    nTRTHits(-1), nTRTHoles(-1), nBLayerHits(-1), nInnermostPixLayHits(-1);
    float pixdEdX(-1.0);
    ...
    if (trk) {
      ...
      trk->summaryValue(nInnermostPixLayHits,
  ...
  if (m_infoSwitch.m_effSF && m_mc) {
    ...
    static SG::AuxElement::Accessor<std::vector<float> > accTTVASF(
      "MuonEfficiencyCorrector_TTVASyst_TTVA");

    static std::map<std::string,
                    SG::AuxElement::Accessor<std::vector<float> > > accRecoSF;

    for (auto& reco : m_infoSwitch.m_recoWPs) {
      std::string recoEffSF = "MuonEfficiencyCorrector_RecoSyst_" + reco;
      accRecoSF.insert(std::pair<std::string,
                                 SG::AuxElement::Accessor<std::vector<float> > >(
                         reco,
                         SG
                         ::AuxElement::Accessor<std::vector<float> >(recoEffSF)));
    ...
    static std::map<std::string,
                    SG::AuxElement::Accessor<std::vector<float> > > accIsoSF;

    for (auto& isol : m_infoSwitch.m_isolWPs) {
      std::string isolEffSF = "MuonEfficiencyCorrector_IsoSyst_" + isol;
      accIsoSF.insert(std::pair<std::string,
                                SG::AuxElement::Accessor<std::vector<float> > >(
                        isol,
                        SG
                        ::AuxElement::Accessor<std::vector<float> >(isolEffSF)));
    ...
    static std::map<std::string,
                    SG::AuxElement::Accessor<std::vector<float> > > accTrigSF;
    static std::map<std::string,
                    SG::AuxElement::Accessor<std::vector<float> > > accTrigEFF;

    for (auto& trig : m_infoSwitch.m_trigWPs) {
      std::string trigEffSF = "MuonEfficiencyCorrector_TrigSyst_" + trig;
      accTrigSF.insert(std::pair<std::string,
                                 SG::AuxElement::Accessor<std::vector<float> > >(
                         trig,
                         SG
                         ::AuxElement::Accessor<std::vector<float> >(trigEffSF)));
      ...
      accTrigEFF.insert(std::pair<std::string,
                                  SG::AuxElement::Accessor<std::vector<float> > >(
                          trig,
                          SG
                          ::AuxElement::Accessor<std::vector<float> >(trigMCEff)));
    ...
    if (accTTVASF.isAvailable(*muon)) m_TTVAEff_SF->push_back(accTTVASF(*muon));
    else m_TTVAEff_SF->push_back(junkSF);
  }

  if (m_infoSwitch.m_energyLoss) {
    static SG::AuxElement::Accessor<float> accMuon_EnergyLoss("EnergyLoss");
    safeFill<float, float, xAOD::Muon>(muon,
                                       accMuon_EnergyLoss,
                                       m_EnergyLoss,
                                       -1);

    static SG::AuxElement::Accessor<float> accMuon_EnergyLossSigma(
      "EnergyLossSigma");
    safeFill<float, float, xAOD::Muon>(muon,
                                       accMuon_EnergyLossSigma,
                                       m_EnergyLossSigma,
                                       -1);

    static SG::AuxElement::Accessor<unsigned char> accMuon_energyLossType(
      "energyLossType");
    safeFill<unsigned char, unsigned char, xAOD::Muon>(muon,
                                                       accMuon_energyLossType,
                                                       m_energyLossType,
                                                       -1);

    static SG::AuxElement::Accessor<float> accMuon_MeasEnergyLoss("MeasEnergyLoss");
    safeFill<float, float, xAOD::Muon>(muon,
                                       accMuon_MeasEnergyLoss,
                                       m_MeasEnergyLoss,
                                       -1);

    static SG::AuxElement::Accessor<float> accMuon_MeasEnergyLossSigma(
      "MeasEnergyLossSigma");
    safeFill<float, float, xAOD::Muon>(muon,
                                       accMuon_MeasEnergyLossSigma,
                                       m_MeasEnergyLossSigma,
                                       -1);

    static SG::AuxElement::Accessor<float> accMuon_ParamEnergyLoss(
      "ParamEnergyLoss");
    safeFill<float, float, xAOD::Muon>(muon,
                                       accMuon_ParamEnergyLoss,
                                       m_ParamEnergyLoss,
                                       -1);

    static SG::AuxElement::Accessor<float> accMuon_ParamEnergyLossSigmaMinus(
      "ParamEnergyLossSigmaMinus");
    safeFill<float, float, xAOD::Muon>(muon,
                                       accMuon_ParamEnergyLossSigmaMinus,
                                       m_ParamEnergyLossSigmaMinus,
                                       -1);

    static SG::AuxElement::Accessor<float> accMuon_ParamEnergyLossSigmaPlus(
      "ParamEnergyLossSigmaPlus");
    safeFill<float, float, xAOD::Muon>(muon,
                                       accMuon_ParamEnergyLossSigmaPlus,
                                       m_ParamEnergyLossSigmaPlus,
                                       -1);
  }
}

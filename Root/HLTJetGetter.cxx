/******************************************
*
* This class gets HLT jets from the TDT and can be expanded to get other
* features
*
...
EL::StatusCode HLTJetGetter::initialize()
{
  ...
  if (asg::ToolStore::contains<Trig::TrigDecisionTool>("TrigDecisionTool")) {
    ...
    Info("Initialize()",
         "the Trigger Decision Tool is not yet initialized...[%s]. Doing so now.",
         m_name.c_str());
    m_ownTDTAndTCT = true;
    ...
    RETURN_CHECK("BasicEventSelection::initialize()",
                 m_trigDecTool->setProperty("ConfigTool",
                                            configHandle),    "");
    RETURN_CHECK("BasicEventSelection::initialize()",
                 m_trigDecTool->setProperty("TrigDecisionKey",
                                            "xTrigDecision"), "");
    RETURN_CHECK("BasicEventSelection::initialize()",
                 m_trigDecTool->setProperty("OutputLevel",
                                            MSG::ERROR),      "");
    RETURN_CHECK("BasicEventSelection::initialize()",
...
EL::StatusCode HLTJetGetter::execute()
{
  ...
  const Trig::ChainGroup *chainGroup = m_trigDecTool->getChainGroup(
    m_triggerList.c_str());                    // Trigger list:
  auto chainFeatures = chainGroup->features(); // Gets features
                                               // associated to
                                               // chain defined
                                               // above
  auto JetFeatureContainers =
    chainFeatures.containerFeature<xAOD::JetContainer>(m_inContainerName.c_str());

  RETURN_CHECK("HLTJetGetter::execute()",
               m_store->record(hltJets,
                               m_outContainerName),
               "Failed to record selected hltJets");
  RETURN_CHECK("HLTJetGetter::execute()",
               m_store->record(hltJetsAux,
                               m_outContainerName + "Aux."),
               "Failed to record selected hltJetsAux.");

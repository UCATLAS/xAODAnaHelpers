...
ElectronSelector::ElectronSelector(std::string className) :
  ...
  m_isolationSelectionTool_handle(
    "CP::IsolationSelectionTool/ElectronIsoSelToolName",
    nullptr),
  m_el_LH_PIDManager(nullptr),
  m_el_CutBased_PIDManager(nullptr),
  m_trigDecTool(nullptr),
  m_trigElectronMatchTool_handle("Trig::MatchingTool/TrigElectronMatchingName",
                                 nullptr)
{
  ...
  //                            decorating and output container should not be
  // mutually exclusive
  ...
  m_diElTrigChains     = "";
  m_minDeltaR          = 0.07; // Recommended threshold for egamma triggers: see
                               // https://svnweb.cern.ch/trac/atlasoff/browser/Trigger/TrigAnalysis/TriggerMatchingTool/trunk/src/TestMatchingToolAlg.cxx
  m_doTrigMatch = true;
}
...
EL::StatusCode ElectronSelector::initialize()
{
  ...
  if (this->numInstances() > 0) {
    m_isUsedBefore = true;
    Info("initialize()",
         "\t An algorithm of the same type has been already used %i times",
         this->numInstances());
  }
  ...
  if (m_useCutFlow) {
    ...
    m_el_cutflow_author_cut = m_el_cutflowHist_1->GetXaxis()->FindBin(
      "author_cut");
    m_el_cutflow_OQ_cut =
      m_el_cutflowHist_1->GetXaxis()->FindBin("OQ_cut");
    m_el_cutflow_ptmax_cut = m_el_cutflowHist_1->GetXaxis()->FindBin(
      "ptmax_cut");
    m_el_cutflow_ptmin_cut = m_el_cutflowHist_1->GetXaxis()->FindBin(
      "ptmin_cut");
    m_el_cutflow_eta_cut = m_el_cutflowHist_1->GetXaxis()->FindBin(
      "eta_cut"); // including crack veto, if applied
    m_el_cutflow_z0sintheta_cut = m_el_cutflowHist_1->GetXaxis()->FindBin(
      "z0sintheta_cut");
    m_el_cutflow_d0_cut =
      m_el_cutflowHist_1->GetXaxis()->FindBin("d0_cut");
    m_el_cutflow_d0sig_cut = m_el_cutflowHist_1->GetXaxis()->FindBin(
      "d0sig_cut");
    m_el_cutflow_BL_cut =
      m_el_cutflowHist_1->GetXaxis()->FindBin("BL_cut");
    m_el_cutflow_PID_cut = m_el_cutflowHist_1->GetXaxis()->FindBin(
      "PID_cut");
    m_el_cutflow_iso_cut = m_el_cutflowHist_1->GetXaxis()->FindBin(
      "iso_cut");
    ...
    if (m_isUsedBefore) {
      ...
      m_el_cutflow_all =
        m_el_cutflowHist_2->GetXaxis()->FindBin("all");
      m_el_cutflow_author_cut = m_el_cutflowHist_2->GetXaxis()->FindBin(
        "author_cut");
      m_el_cutflow_OQ_cut = m_el_cutflowHist_2->GetXaxis()->FindBin(
        "OQ_cut");
      m_el_cutflow_ptmax_cut = m_el_cutflowHist_2->GetXaxis()->FindBin(
        "ptmax_cut");
      m_el_cutflow_ptmin_cut = m_el_cutflowHist_2->GetXaxis()->FindBin(
        "ptmin_cut");
      m_el_cutflow_eta_cut = m_el_cutflowHist_2->GetXaxis()->FindBin(
        "eta_cut"); // including crack veto, if applied
      m_el_cutflow_z0sintheta_cut = m_el_cutflowHist_2->GetXaxis()->FindBin(
        "z0sintheta_cut");
      m_el_cutflow_d0_cut = m_el_cutflowHist_2->GetXaxis()->FindBin(
        "d0_cut");
      m_el_cutflow_d0sig_cut = m_el_cutflowHist_2->GetXaxis()->FindBin(
        "d0sig_cut");
      m_el_cutflow_BL_cut = m_el_cutflowHist_2->GetXaxis()->FindBin(
        "BL_cut");
      m_el_cutflow_PID_cut = m_el_cutflowHist_2->GetXaxis()->FindBin(
        "PID_cut");
      m_el_cutflow_iso_cut = m_el_cutflowHist_2->GetXaxis()->FindBin(
        "iso_cut");
    }
  }
  ...
  m_outAuxContainerName = m_outContainerName + "Aux."; // the period is very
                                                       // important!

  // Compatible with Efficiency Nomenclature
  if (m_LHOperatingPoint == "LooseAndBLayer") m_LHOperatingPoint = "LooseBL";
  ...
  if ( // m_LHOperatingPoint != "VeryLoose"       &&
    ...
    Error("initialize()",
          "Unknown electron likelihood PID requested %s!",
          m_LHOperatingPoint.c_str());
  ...
  if ((m_CutBasedOperatingPoint != "Loose") &&
    ...
    Error("initialize()",
          "Unknown electron cut-based PID requested %s!",
          m_CutBasedOperatingPoint.c_str());
    return EL::StatusCode::FAILURE;
  }

  // Parse input isolation WP list, split by comma, and put into a vector for
  // later use
  ...
  // Assume no local BLayer track quality cut unless doing LooseBL and reading
  // decision from the derivation
  m_doBLTrackQualityCut = false;

  // initialise PID tool(s) using classes defined in ParticlePIDManager.h
  //
  // if not using cut-based PID, make sure all the decorations will be set ...
  // by choosing the loosest WP!
  if (m_doCutBasedPID) {
    std::string cutbasedWP =
      (m_doCutBasedPIDcut) ? m_CutBasedOperatingPoint : "Loose";
    m_el_CutBased_PIDManager =
      new ElectronCutBasedPIDManager(cutbasedWP, m_debug);

    if  (m_doCutBasedPIDcut) {
      Info("initialize()",
           "Cutting on Electron Cut-Based PID! \n ********************");
      Info("initialize()", "Selected cut-based WP: %s",
           (m_el_CutBased_PIDManager->getSelectedWP()).c_str());
    } else {
      Info("initialize()",
           "Will decorate each electron with all Electron Cut-Based PID WPs decision (pass/not pass)!");
    }

    if (m_readIDFlagsFromDerivation) {
      Info("initialize()", "Reading Electron cut-based ID from DAODs ...");
      RETURN_CHECK("ElectronSelector::initialize()",
                   m_el_CutBased_PIDManager->setupWPs(
                     false),
                   "Failed to setup ElectronCutBasedPIDManager in Derivation mode.");
    } else {
      Info("initialize()", "Reading Electron cut-based ID from CP Tool ...");
      RETURN_CHECK("ElectronSelector::initialize()",
                   m_el_CutBased_PIDManager->setupWPs(true, this->m_name),
                   "Failed to setup ElectronCutBasedPIDManager in tool mode.");
    }
  } // if m_doCutBasedPID

  if (m_doLHPID) {
    // if not using LH PID, make sure all the decorations will be set ... by
    // choosing the loosest WP!
    ...
    if  (m_doLHPIDcut) {
      Info("initialize()",
           "Cutting on Electron Likelihood PID! \n ********************");
      Info("initialize()",
           "\t Input WP: %s corresponding to actual LikeEnum::Menu WP: %s",
           likelihoodWP.c_str(),
           (m_el_LH_PIDManager->getSelectedWP()).c_str());
    } else {
      Info("initialize()",
           "Will decorate each electron with all Electron Likelihood PID WPs decision (pass/not pass)!");
    }

    if (m_readIDFlagsFromDerivation) {
      // LooseBL is not in Derivations, so choose Loose and do BLayer cut
      // locally
      ...
      RETURN_CHECK("ElectronSelector::initialize()",
                   m_el_LH_PIDManager->setupWPs(
                     false),
                   "Failed to setup ElectronLHPIDManager in Derivation mode.");
    } else {
      Info("initialize()", "Reading Electron LH ID from CP Tool ...");
      RETURN_CHECK("ElectronSelector::initialize()",
                   m_el_LH_PIDManager->setupWPs(true,
                                                this->m_name),
                   "Failed to setup ElectronLHPIDManager in tool mode.");
    }
  } // if m_doLHPID
  ...
  std::string isolationSelectionTool_handle_name = "CP::IsolationSelectionTool/" +
                                                   m_isolationSelectionTool_name;

  RETURN_CHECK("ElectronSelector::initialize()",
               checkToolStore<CP::IsolationSelectionTool>(
                 m_isolationSelectionTool_name), "");
  RETURN_CHECK("ElectronSelector::initialize()",
               m_isolationSelectionTool_handle.makeNew<CP::IsolationSelectionTool>(
                 isolationSelectionTool_handle_name),
               "Failed to create handle to CP::IsolationSelectionTool");

  // Do this only for the first WP in the list
  //
  if (m_debug) Info("initialize()",
                    "Adding isolation WP %s to IsolationSelectionTool",
                    (m_IsoKeys.at(0)).c_str()); RETURN_CHECK(
    "ElectronSelector::initialize()",
    m_isolationSelectionTool_handle.setProperty(
      "ElectronWP",
      (
        m_IsoKeys.at(
          0)).c_str()),
    "Failed to configure base WP");
  RETURN_CHECK("ElectronSelector::initialize()",
               m_isolationSelectionTool_handle.initialize(),
               "Failed to properly initialize IsolationSelectionTool.");

  // Add the remaining input WPs to the tool
  // (start from 2nd element)
  //
  for (auto WP_itr = std::next(m_IsoKeys.begin()); WP_itr != m_IsoKeys.end();
       ++WP_itr) {
    if (m_debug) Info("initialize()",
                      "Adding extra isolation WP %s to IsolationSelectionTool",
                      (*WP_itr).c_str());
    ...
    if ((*WP_itr).find("UserDefined") != std::string::npos) {
      ...
      myCuts.push_back(std::make_pair<xAOD::Iso::IsolationType,
                                      std::string>(isoParser.parseEnum(
                                                     m_TrackBasedIsoType),
                                                   m_TrackIsoEff.c_str()));
      myCuts.push_back(std::make_pair<xAOD::Iso::IsolationType,
                                      std::string>(isoParser.parseEnum(
      ...
      CP::IsolationSelectionTool::IsoWPType iso_type(
        CP::IsolationSelectionTool::Efficiency);

      if ((*WP_itr).find("Cut") !=
          std::string::npos) iso_type = CP::IsolationSelectionTool::Cut;
      RETURN_CHECK("ElectronSelector::initialize()",
                   m_isolationSelectionTool_handle->addUserDefinedWP((*WP_itr).
                                                                     c_str(),
                                                                     xAOD::Type::
                                                                     Electron,
                                                                     myCuts, "",
                                                                     iso_type),
                   "Failed to add user-defined isolation WP");
    } else {
      RETURN_CHECK("ElectronSelector::initialize()",
                   m_isolationSelectionTool_handle->addElectronWP(
                     (*WP_itr).c_str()),
                   "Failed to add isolation WP");
    }
  }

  // m_isolationSelectionTool_handle->msg().setLevel(MSG::ERROR); // ERROR,
  // VERBOSE, DEBUG, INFO
  ...
  //
  // NB: need to retrieve the TrigDecisionTool from asg::ToolStore to configure
  // the tool!
  //     do not initialise if there are no input trigger chains
  //
  if (!(m_singleElTrigChains.empty() && m_diElTrigChains.empty()) &&
      asg::ToolStore::contains<Trig::TrigDecisionTool>("TrigDecisionTool")) {
    m_trigDecTool =
      asg::ToolStore::get<Trig::TrigDecisionTool>("TrigDecisionTool");
    ToolHandle<Trig::TrigDecisionTool> trigDecHandle(m_trigDecTool);

    //  everything went fine, let's initialise the tool!
    //
    m_trigElMatchTool_name = "MatchingTool_" + m_name;
    std::string trigElectronMatchTool_handle_name = "Trig::MatchingTool/" +
                                                    m_trigElMatchTool_name;

    RETURN_CHECK("ElectronSelector::initialize()",
                 checkToolStore<Trig::MatchingTool>(
                   m_trigElMatchTool_name),
                 "");
    RETURN_CHECK("ElectronSelector::initialize()",
                 m_trigElectronMatchTool_handle.makeNew<Trig::MatchingTool>(
                   trigElectronMatchTool_handle_name),
                 "Failed to create handle to MatchingTool");
    RETURN_CHECK("ElectronSelector::initialize()",
                 m_trigElectronMatchTool_handle.setProperty("TrigDecisionTool",
                                                            trigDecHandle),
                 "Failed to pass TrigDecisionTool to MatchingTool");
    RETURN_CHECK("ElectronSelector::initialize()",
                 m_trigElectronMatchTool_handle.initialize(),
                 "Failed to properly initialize MatchingTool");
  } else {
    m_doTrigMatch = false;
    ...
    std::cout << "" << std::endl;
    std::cout << "\t -) could not find the TrigDecisionTool in asg::ToolStore" <<
      std::endl;
    std::cout << "\t AND/OR" << std::endl;
    std::cout << "\t -) all input HLT trigger chain lists are empty" << std::endl;
    std::cout << "" << std::endl;
    std::cout <<
      "However, if you really didn't want to do the matching now, it's all good!"
              <<
      std::endl;
    std::cout << "***********************************************************" <<
      std::endl;
...
EL::StatusCode ElectronSelector::execute()
{
  ...
  RETURN_CHECK("ElectronSelector::execute()",
  ...
  if (!mcEvtWeightAcc.isAvailable(*eventInfo)) {
    Error("execute()  ",
          "mcEventWeight is not available as decoration! Aborting");
    return EL::StatusCode::FAILURE;
  }
  ...
  // QUESTION: why this must be done in execute(), and does not work in
  // initialize()?
  //
  if ((m_numEvent == 1) && m_trigDecTool) {
    ...
    Info("execute()",
         "Input single electron trigger chains that will be considered for matching:\n");

    for (auto const& chain : m_singleElTrigChainsList) Info("execute()",
                                                            "\t %s",
                                                            chain.c_str()); Info(
      "execute()",
      "\n");
    ...
    Info("execute()",
         "Input di-electron trigger chains that will be considered for matching:\n");

    for (auto const& chain : m_diElTrigChainsList) Info("execute()",
                                                        "\t %s",
                                                        chain.c_str()); Info(
      "execute()",
      "\n");
  ...
  bool countPass(true); // for cutflow: count for the 1st collection in the syst
                        // container - could be better as should only count for
                        // the nominal
  ...
  if (m_inputAlgoSystNames.empty()) {
    ...
    //
    RETURN_CHECK("ElectronSelector::execute()",
                 HelperFunctions::retrieve(inElectrons, m_inContainerName,
                                           m_event, m_store,
                                           m_verbose), "");
    ...
    if (m_createSelectedContainer) selectedElectrons =
        new ConstDataVector<xAOD::ElectronContainer>(SG::VIEW_ELEMENTS);

    // find the selected electrons, and return if event passes object selection
    //
    eventPass = executeSelection(inElectrons,
                                 mcEvtWeight,
                                 countPass,
                                 selectedElectrons);
    ...
    if (m_createSelectedContainer) {
      if (eventPass) {
        ...
        RETURN_CHECK("ElectronSelector::execute()",
                     m_store->record(selectedElectrons,
                                     m_outContainerName),
                     "Failed to store const data container");
      } else {
        // if the event does not pass the selection, CDV won't be ever recorded
        // to TStore, so we have to delete it!
    ...
    // prepare a vector of the names of CDV containers for usage by downstream
    // algos
    // must be a pointer to be recorded in TStore
    //
    std::vector<std::string> *vecOutContainerNames = new std::vector<std::string>;

    if (m_debug) Info("execute()", " input list of syst size: %i ",
                      static_cast<int>(systNames->size()));
    ...
    for (auto systName : *systNames) {
      ...
      RETURN_CHECK("ElectronSelector::execute()",
                   HelperFunctions::retrieve(inElectrons,
      ...
      // find the selected electrons, and return if event passes object
      // selection
      //
      eventPassThisSyst = executeSelection(inElectrons,
                                           mcEvtWeight,
                                           countPass,
                                           selectedElectrons);

      if (countPass) countPass = false;  // only count objects/events for 1st
                                         // syst collection in iteration (i.e.,
                                         // nominal)

      if (eventPassThisSyst) {
        // save the string of syst set under question if event is passing the
        // selection
        vecOutContainerNames->push_back(systName);
      }

      // if for at least one syst set the event passes selection, this will
      // remain true!
      //
      ...
      if (m_createSelectedContainer) {
        if (eventPassThisSyst) {
          ...
          // if the event does not pass the selection for this syst, CDV won't
          // be ever recorded to TStore, so we have to delete it!
          delete selectedElectrons; selectedElectrons = nullptr;
        }
      }
    ...
    // record in TStore the list of systematics names that should be considered
    // down stream
    //
                                        ...
                                        ConstDataVector<xAOD::ElectronContainer> *selectedElectrons)
{
  ...
  // for cutflow: make sure to count passed objects only once (i.e., this flag
  // will be true only for nominal)
  //
  if (countPass) {
    m_numObject     += nObj;
  ...
  // apply event selection based on minimal/maximal requirements on the number
  // of objects per event passing cuts
  //
  ...
  // for cutflow: make sure to count passed events only once (i.e., this flag
  // will be true only for nominal)
  //
  if (countPass) {
    m_numEventPass++;
  ...
  //
  //  1. the user didn't pass any trigger chains to the algo (see initialize():
  // in that case, the tool is not even initialised!)
  ...
  if (m_doTrigMatch && selectedElectrons) {
    ...
    if (nSelectedElectrons > 0) {
      ...
      for (auto const& chain : m_singleElTrigChainsList) {
        ...
        for (auto const electron : *selectedElectrons) {
          //  For each electron, decorate w/ a map<string,char> with the
          // 'isMatched' info associated
          //  to each trigger chain in the input list.
          //  If decoration map doesn't exist for this electron yet, create it
          // (will be done only for the 1st iteration on the chain names)
          //
    ...
    // If checking dilepton trigger, form lepton pairs and test matching for
    // each one.
    // Save a:
    //
    ...
    // A multimap is used b/c a given key (i.e., a chain) can be associated to
    // more than one pair. This is the case for e.g., trilepton events.
    //
    // By retrieving this map later on, user can decide what to do with the
    // event
    // (Generally one could just loop over the map and save a flag if there's at
    // least one pair that matches a given chain)
    ...
    if ((nSelectedElectrons > 1) && !m_diElTrigChains.empty()) {
      ...
      for (auto const& chain : m_diElTrigChainsList) {
        ...
        //  If decoration map doesn't exist for this event yet, create it (will
        // be done only for the 1st iteration on the chain names)
...
int ElectronSelector::passCuts(const xAOD::Electron *electron, const xAOD::Vertex *primaryVertex) {
  ...
  // all the eta cuts are done using the measurement of the cluster position
  // with the 2nd layer cluster,
  ...
  if (m_doAuthorCut) {
    if (!(electron->author(xAOD::EgammaParameters::AuthorElectron) || electron->author(xAOD::EgammaParameters::AuthorAmbiguous))) {
      if (m_debug) Info("PassCuts()", "Electron failed author kinematic cut."); return 0;
    }
  }
  ...
  if (m_doOQCut) {
    if (!electron->isGoodOQ(xAOD::EgammaParameters::BADCLUSELECTRON)) {
      if (m_debug) Info("PassCuts()", "Electron failed Object Quality cut BADCLUSELECTRON."); return 0;
    }
  }
  ...
  if (m_pT_max != 1e8) {
    if (et > m_pT_max) {
      if (m_debug) Info("PassCuts()", "Electron failed pT max cut."); return 0;
    }
  }
  ...
  if (m_pT_min != 1e8) {
    if (et < m_pT_min) {
      if (m_debug) Info("PassCuts()", "Electron failed pT min cut."); return 0;
    }
  }
  ...
  if (m_eta_max != 1e8) {
    if (fabs(eta) > m_eta_max) {
      if (m_debug) Info("PassCuts()", "Electron failed |eta| max cut."); return 0;
    }
  }
  ...
  if (m_vetoCrack) {
    if ((fabs(eta) > 1.37) && (fabs(eta) < 1.52)) {
      if (m_debug) Info("PassCuts()", "Electron failed |eta| crack veto cut."); return 0;
    }
  }
  ...
  // Take distance between z0 and zPV ( after referring the PV z coordinate to
  // the beamspot position, given by vz() ), multiplied by sin(theta)
  // see https://twiki.cern.ch/twiki/bin/view/AtlasProtected/InDetTrackingDC14
  ...
  if (m_z0sintheta_max != 1e8) {
    if (!(fabs(z0sintheta) < m_z0sintheta_max)) {
      if (m_debug) Info("PassCuts()", "Electron failed z0*sin(theta) cut."); return 0;
    }
  }
  ...
  if (m_d0_max != 1e8) {
    if (!(tp->d0() < m_d0_max)) {
      if (m_debug) Info("PassCuts()", "Electron failed d0 cut."); return 0;
    }
  }
  ...
  if (m_d0sig_max != 1e8) {
    if (!(d0_significance < m_d0sig_max)) {
      if (m_debug) Info("PassCuts()", "Electron failed d0 significance cut."); return 0;
    }
  }
  ...
  if (m_doBLTrackQualityCut) {
    // this is taken from
    // ElectronPhotonSelectorTools/Root/AsgElectronLikelihoodTool.cxx
    ...
    if (expectBlayer && ((nBlayerHits + nBlayerOutliers) < 1)) {
      if (m_debug) Info("PassCuts()", "Electron failed BL track quality cut."); return 0;
    }
  }
  ...
  if (m_doLHPID) {
    ...
    if (m_readIDFlagsFromDerivation) {
      if (m_doLHPIDcut) {
        ...
        if (LHDecision.isAvailable(*electron)) passSelID = LHDecision(*electron);

        if (!passSelID) {
          if (m_debug) Info("PassCuts()", "Electron failed likelihood PID cut w/ operating point %s", m_LHOperatingPoint.c_str()); return 0;
        }
      }
      ...
      for (auto it : (myLHWPs)) {
        ...
        if (LHDecisionAll.isAvailable(*electron)) passThisID = LHDecisionAll(*electron);
      ...
      // retrieve only tools with WP >= selected WP, cut electrons if not
      // satisfying selected WP, and decorate w/ tool decision all the others
      //
  ...
  if (m_doCutBasedPID) {
    ...
    if (m_readIDFlagsFromDerivation) {
      if (m_doCutBasedPIDcut) {
        ...
        if (CutDecision.isAvailable(*electron)) passSelID = CutDecision(*electron);

        if (!passSelID) {
          if (m_debug) Info("PassCuts()", "Electron failed cut-based PID cut w/ operating point %s", m_CutBasedOperatingPoint.c_str()); return 0;
        }
      }
      ...
      for (auto it : (myCutBasedWPs)) {
        ...
        if (CutDecisionAll.isAvailable(*electron)) passThisID = CutDecisionAll(*electron);
      ...
      if (m_doCutBasedPIDcut && !((myCutBasedTools.find(m_CutBasedOperatingPoint)->second)->accept(*electron))) {
        if (m_debug) Info("PassCuts()", "Electron failed cut-based PID cut."); return 0;
      }
  ...
  for (auto WP_itr : m_IsoKeys) {
    ...
    if (m_debug) Info("PassCuts()", "Decorate electron with %s - accept() ? %i", decorWP.c_str(), accept_list.getCutResult(WP_itr.c_str())); electron->auxdecor<char>(decorWP) = static_cast<char>(accept_list.getCutResult(WP_itr.c_str()));
  }

  // Apply the cut if needed
  //
  if (!m_MinIsoWPCut.empty() && !accept_list.getCutResult(m_MinIsoWPCut.c_str())) {
    if (m_debug) Info("PassCuts()", "Electron failed isolation cut %s ",  m_MinIsoWPCut.c_str()); return 0;
  }

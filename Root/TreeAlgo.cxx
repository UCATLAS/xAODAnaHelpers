...
TreeAlgo::TreeAlgo(std::string className) :
  ...
  m_trees({})
{
  this->SetName("TreeAlgo"); // needed if you want to retrieve this algo with
                             // wk()->getAlg(ALG_NAME) downstream
...
EL::StatusCode TreeAlgo::initialize()
{
  ...
  if (!m_jetContainerName.empty() &&
    ...
    Error("initialize()",
          "The number of jet containers must be equal to the number of jet name branches. Exiting");
  ...
  if (!m_truthJetContainerName.empty() &&
    ...
    Error("initialize()",
          "The number of truth jet containers must be equal to the number of truth jet name branches. Exiting");
    return EL::StatusCode::FAILURE;
  }

  // allow to store different variables for each jet collection (reco only,
  // default: store the same)
  ...
  if ((m_jetDetails.size() != 1) &&
    ...
    Error("initialize()",
          "The size of m_jetContainers should be equal to the size of m_jetDetailStr. Exiting");
    return EL::StatusCode::FAILURE;
  }
...
EL::StatusCode TreeAlgo::fileExecute() {
  return EL::StatusCode::SUCCESS;
}

EL::StatusCode TreeAlgo::changeInput(bool /*firstFile*/) {
  return EL::StatusCode::SUCCESS;
}
...
EL::StatusCode TreeAlgo::execute()
{
  ...
  // this is a temporary pointer that gets switched around to check each of the
  // systematics
  std::vector<std::string> *systNames(nullptr);

  // note that the way we set this up, none of the below ##SystNames vectors
  // contain the nominal case
  // TODO: do we really need to check for duplicates? Maybe, maybe not.
  if (!m_muSystsVec.empty()) {
    RETURN_CHECK("TreeAlgo::execute()",
                 HelperFunctions::retrieve(systNames, m_muSystsVec, 0, m_store,
                                           m_verbose),
                 "");

    for (const auto& systName: *systNames) {
      if (std::find(event_systNames.begin(), event_systNames.end(),
                    systName) != event_systNames.end()) continue;
  ...
  if (!m_elSystsVec.empty()) {
    RETURN_CHECK("TreeAlgo::execute()",
                 HelperFunctions::retrieve(systNames, m_elSystsVec, 0, m_store,
                                           m_verbose),
                 "");
  ...
  if (!m_jetSystsVec.empty()) {
    RETURN_CHECK("TreeAlgo::execute()",
                 HelperFunctions::retrieve(systNames, m_jetSystsVec, 0, m_store,
                                           m_verbose),
                 "");
  ...
  if (!m_fatJetSystsVec.empty()) {
    RETURN_CHECK("TreeAlgo::execute()",
                 HelperFunctions::retrieve(systNames,
                                           m_fatJetSystsVec,
                                           0,
                                           m_store,
                                           m_verbose), "");
  ...
  for (const auto& systName: event_systNames) {
    ...
    m_trees[systName] = new HelpTreeBase(m_event,
                                         outTree,
                                         treeFile,
                                         m_units,
                                         m_debug,
    ...
    if (m_outHistDir) {
      if (m_trees.size() > 1) Warning("execute()",
                                      "You're running systematics! You may find issues in writing all of the output TTrees to the output histogram file... Set `m_outHistDir = false` if you run into issues!");
    ...
    if (!m_jetContainerName.empty())           {
      for (unsigned int ll = 0; ll < m_jetContainers.size(); ++ll) {
        if (m_jetDetails.size() == 1) helpTree->AddJets(m_jetDetailStr,
                                                        m_jetBranches.at(
                                                          ll).c_str());
    ...
    if (!m_trigJetContainerName.empty()) helpTree->AddJets(m_trigJetDetailStr,
                                                           "trigJet");
    ...
    if (!m_truthParticlesContainerName.empty()) helpTree->AddTruthParts(
        "xAH_truth",
        m_truthParticlesDetailStr);

    if (!m_trackParticlesContainerName.empty()) helpTree->AddTrackParts(
        m_trackParticlesContainerName,
        m_trackParticlesDetailStr);
  }
  ...
  for (const auto& systName: event_systNames) {
    ...
    /*
       if we find the systematic in the corresponding vector, we will use that
          container's systematic version instead of nominal version
        NB: since none of these contain the "" (nominal) case because of how I
           filter it, we handle the merging.. why?
        - in each loop to make the ##systNames vectors, we check to see if the
           systName exists in event_systNames which is initialized
        -   to {""} - the nominal case. If the systName exists, we do not add it
           to the corresponding ##systNames vector, otherwise, we do.
        -   This precludes the nominal case in all of the ##systNames vectors,
           which means the default will always be to run nominal.
     */
    if (std::find(muSystNames.begin(), muSystNames.end(),
                  systName) != muSystNames.end()) muSuffix = systName;

    if (std::find(elSystNames.begin(), elSystNames.end(),
                  systName) != elSystNames.end()) elSuffix = systName;

    if (std::find(jetSystNames.begin(), jetSystNames.end(),
                  systName) != jetSystNames.end()) jetSuffix = systName;

    if (std::find(photonSystNames.begin(), photonSystNames.end(),
                  systName) != photonSystNames.end()) photonSuffix = systName;

    if (std::find(fatJetSystNames.begin(), fatJetSystNames.end(),
                  systName) != fatJetSystNames.end()) fatJetSuffix = systName;
    ...
    // Fill jet trigger information - this can be used if with layer/cleaning
    // info we need to turn off some variables?
    ...
    if (!m_muContainerName.empty()) {
      ...
      RETURN_CHECK("TreeAlgo::execute()",
                   HelperFunctions::retrieve(inMuon, m_muContainerName + muSuffix,
                                             m_event,
                                             m_store, m_verbose), "");
    ...
    if (!m_elContainerName.empty()) {
      ...
      RETURN_CHECK("TreeAlgo::execute()",
                   HelperFunctions::retrieve(inElec, m_elContainerName + elSuffix,
                                             m_event,
                                             m_store, m_verbose), "");
    ...
    if (!m_jetContainerName.empty()) {
      for (unsigned int ll = 0; ll < m_jetContainers.size(); ++ll) { // Systs
                                                                     // only for
                                                                     // first
                                                                     // jet
                                                                     // container
        ...
        if (ll == 0) RETURN_CHECK("TreeAlgo::execute()",
                                  HelperFunctions::retrieve(inJets,
                                                            m_jetContainers.at(ll)
                                                            + jetSuffix,
                                                            m_event, m_store,
                                                            m_verbose),
                                  "");
        else RETURN_CHECK("TreeAlgo::execute()",
                          HelperFunctions::retrieve(inJets,
                                                    m_jetContainers.at(ll),
                                                    m_event,
                                                    m_store, m_verbose), "");
        helpTree->FillJets(inJets,
    ...
    if (!m_l1JetContainerName.empty()) {
      ...
      RETURN_CHECK("TreeAlgo::execute()",
                   HelperFunctions::retrieve(inL1Jets, m_l1JetContainerName,
                                             m_event, m_store,
                                             m_verbose), "");
    ...
    if (!m_trigJetContainerName.empty()) {
      ...
      RETURN_CHECK("TreeAlgo::execute()",
                   HelperFunctions::retrieve(inTrigJets, m_trigJetContainerName,
                                             m_event,
                                             m_store, m_verbose), "");
      helpTree->FillJets(inTrigJets,
                         HelperFunctions::getPrimaryVertexLocation(
                           vertices), "trigJet");
    }
    ...
    if (!m_truthJetContainerName.empty()) {
      for (unsigned int ll = 0; ll < m_truthJetContainers.size(); ++ll) {
        ...
        RETURN_CHECK("TreeAlgo::execute()",
                     HelperFunctions::retrieve(inTruthJets,
                                               m_truthJetContainers.at(ll),
                                               m_event, m_store, m_verbose), "");
        helpTree->FillJets(inTruthJets,
                           HelperFunctions::getPrimaryVertexLocation(
                             vertices), m_truthJetBranches.at(ll));
      }
    }
    ...
    if (!m_fatJetContainerName.empty()) {
      ...
      while (std::getline(ss, token, ' ')) {
        ...
        RETURN_CHECK("TreeAlgo::execute()",
                     HelperFunctions::retrieve(inFatJets, token + fatJetSuffix,
                                               m_event,
                                               m_store, m_verbose), "");
    ...
    if (!m_truthFatJetContainerName.empty()) {
      ...
      RETURN_CHECK("TreeAlgo::execute()",
                   HelperFunctions::retrieve(inTruthFatJets,
                                             m_truthFatJetContainerName,
                                             m_event, m_store, m_verbose), "");
    ...
    if (!m_tauContainerName.empty()) {
      ...
      RETURN_CHECK("HTopMultilepTreeAlgo::execute()",
                   HelperFunctions::retrieve(inTaus, m_tauContainerName, m_event,
                                             m_store,
                                             m_verbose), "");
    ...
    if (!m_METContainerName.empty()) {
      ...
      RETURN_CHECK("HTopMultilepTreeAlgo::execute()",
                   HelperFunctions::retrieve(inMETCont, m_METContainerName,
                                             m_event, m_store,
                                             m_debug), "");
    ...
    if (!m_photonContainerName.empty()) {
      ...
      RETURN_CHECK("TreeAlgo::execute()",
                   HelperFunctions::retrieve(inPhotons,
                                             m_photonContainerName + photonSuffix,
                                             m_event, m_store, m_verbose), "");
    ...
    if (!m_truthParticlesContainerName.empty()) {
      ...
      RETURN_CHECK("TreeAlgo::execute()",
                   HelperFunctions::retrieve(inTruthParticles,
                                             m_truthParticlesContainerName,
                                             m_event, m_store, m_verbose), "");
    ...
    if (!m_trackParticlesContainerName.empty()) {
      ...
      RETURN_CHECK("TreeAlgo::execute()",
                   HelperFunctions::retrieve(inTrackParticles,
                                             m_trackParticlesContainerName,
                                             m_event, m_store, m_verbose), "");
...
EL::StatusCode TreeAlgo::postExecute() {
  return EL::StatusCode::SUCCESS;
}
...
EL::StatusCode TreeAlgo::histFinalize() {
  return EL::StatusCode::SUCCESS;
}

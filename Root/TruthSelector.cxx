...
EL::StatusCode TruthSelector::initialize()
{
  ...
  if (m_useCutFlow) {
    ...
    m_truth_cutflow_ptmax_cut = m_truth_cutflowHist_1->GetXaxis()->FindBin(
      "ptmax_cut");
    m_truth_cutflow_ptmin_cut = m_truth_cutflowHist_1->GetXaxis()->FindBin(
      "ptmin_cut");
    m_truth_cutflow_eta_cut = m_truth_cutflowHist_1->GetXaxis()->FindBin(
      "eta_cut");
  }
...
EL::StatusCode TruthSelector::execute()
{
  ...
  RETURN_CHECK("TruthSelector::execute()",
  ...
  RETURN_CHECK("TruthSelector::execute()",
               HelperFunctions::retrieve(inTruthParts, m_inContainerName, m_event,
                                         m_store,
                                         m_verbose), "");
...
bool TruthSelector::executeSelection(
  const xAOD::TruthParticleContainer *inTruthParts,
  float                               mcEvtWeight,
  bool                                count,
  ...
  )
{
  ...
  if (m_createSelectedContainer) {
    selectedTruthParts = new ConstDataVector<xAOD::TruthParticleContainer>(
      SG::VIEW_ELEMENTS);
  }
  ...
  if (m_createSelectedContainer) {
    RETURN_CHECK("TruthSelector::execute()",
                 m_store->record(selectedTruthParts,
                                 outContainerName),

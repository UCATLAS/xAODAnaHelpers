...
void EventInfo::setTree(TTree *tree)
{
  ...
  if (m_infoSwitch.m_pileup) {
    ...
    connectBranch<float>(tree, "weight_pileup",
                         &m_weight_pileup);
  ...
  if (m_infoSwitch.m_caloClus) {
    ...
    HelperFunctions::connectBranch<float>("caloCluster",
                                          tree,
                                          "pt",
                                          &m_caloCluster_pt_addr);
...
void EventInfo::setBranches(TTree *tree)
{
  ...
  if (m_mc) {
    ...
    tree->Branch("prescale_DataWeight",
                 &m_prescale_DataWeight,
                 "prescale_DataWeight/F");
  }

  if (m_infoSwitch.m_eventCleaning) {
    tree->Branch("timeStamp",         &m_timeStamp, "timeStamp/i");
    tree->Branch("timeStampNSOffset", &m_timeStampNSOffset,
                 "timeStampNSOffset/i");
  ...
  if (m_infoSwitch.m_pileup) {
    ...
    tree->Branch("actualInteractionsPerCrossing",
                 &m_actualMu,
                 "actualInteractionsPerCrossing/F");
    tree->Branch("averageInteractionsPerCrossing",
                 &m_averageMu,
                 "averageInteractionsPerCrossing/F");
    tree->Branch("weight_pileup",
                 &m_weight_pileup,
                 "weight_pileup/F");

    if (m_infoSwitch.m_pileupsys) {
      tree->Branch("weight_pileup_up",
                   &m_weight_pileup_up,
                   "weight_pileup_up/F");
      tree->Branch("weight_pileup_down",
                   &m_weight_pileup_down,
                   "weight_pileup_down/F");
    }

    if (m_mc) {
      tree->Branch("correct_mu",  &m_correct_mu,  "correct_mu/F");
      tree->Branch("rand_run_nr", &m_rand_run_nr, "rand_run_nr/I");
      tree->Branch("rand_lumiblock_nr",
                   &m_rand_lumiblock_nr,
                   "rand_lumiblock_nr/I");
    }
  }
...
{
  m_runNumber = m_eventNumber = m_mcEventNumber = m_mcChannelNumber = m_bcid =
                                                                        m_lumiBlock;
  m_coreFlags = 0;
...
void EventInfo::FillEvent(const xAOD::EventInfo *eventInfo,
                          xAOD::TEvent          *event) {
  ...
  if (m_infoSwitch.m_eventCleaning) {
    if (eventInfo->errorState(xAOD::EventInfo::LAr) ==
        xAOD::EventInfo::Error) m_LArError = true;
    else m_LArError = false;
    m_LArFlags = eventInfo->eventFlags(xAOD::EventInfo::LAr);

    if (eventInfo->errorState(xAOD::EventInfo::Tile) ==
        xAOD::EventInfo::Error) m_TileError = true;
    else m_TileError = false;
    m_TileFlags = eventInfo->eventFlags(xAOD::EventInfo::Tile);

    if (eventInfo->errorState(xAOD::EventInfo::SCT) ==
        xAOD::EventInfo::Error) m_SCTError = true;
  ...
  if (m_infoSwitch.m_pileup) {
    ...
    if (m_mc) {
      ...
      static SG::AuxElement::ConstAccessor<float> correct_mu(
        "corrected_averageInteractionsPerCrossing");
      static SG::AuxElement::ConstAccessor<unsigned int> rand_run_nr(
        "RandomRunNumber");
      static SG::AuxElement::ConstAccessor<unsigned int> rand_lumiblock_nr(
  ...
  if (m_infoSwitch.m_caloClus && event) {
    ...
    for (auto clus : *caloClusters) {
      if (clus->pt(xAOD::CaloCluster::State::UNCALIBRATED) < 2000) continue;  //
                                                                              //
                                                                              //
                                                                              //
                                                                              // 2

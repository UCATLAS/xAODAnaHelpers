...
void EventInfo::FillEvent(const xAOD::EventInfo* eventInfo, xAOD::TEvent* event) {
    ...
    if(m_infoSwitch.m_pileup) {
        ...
        if(m_mc) {
            ...
            if( weight_pileup.isAvailable(*eventInfo) ) m_weight_pileup = weight_pileup(*eventInfo); else m_weight_pileup = 1.0; if( correct_mu.isAvailable(*eventInfo) ) m_correct_mu = correct_mu(*eventInfo); else m_correct_mu = -1.0; if( rand_run_nr.isAvailable(*eventInfo) ) m_rand_run_nr = rand_run_nr(*eventInfo); else m_rand_run_nr = 900000; if( rand_lumiblock_nr.isAvailable(*eventInfo) ) m_rand_lumiblock_nr = rand_lumiblock_nr(*eventInfo); else m_rand_lumiblock_nr = 0;
            ...
            if( weight_pileup_up.isAvailable(*eventInfo) ) m_weight_pileup_up = weight_pileup_up(*eventInfo); else m_weight_pileup_up = 1.0; if( weight_pileup_down.isAvailable(*eventInfo) ) m_weight_pileup_down = weight_pileup_down(*eventInfo); else m_weight_pileup_down = 1.0;
        }

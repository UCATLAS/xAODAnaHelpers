...
HelpTreeBase::HelpTreeBase(xAOD::TEvent *event,
                           TTree        *tree,
                           TFile        *file,
                           const float   units,
                           bool          debug,
...
HelpTreeBase::~HelpTreeBase() {
  ...
  // muon
  for (auto muon: m_muons) delete muon.second;

  // el
  for (auto elec: m_elecs) delete elec.second;

  // ph
  for (auto photon: m_photons) delete photon.second;

  // fatjet
  for (auto fatjet: m_fatjets) delete fatjet.second;

  // tau
  for (auto tau: m_taus) delete tau.second;

  // met
  delete m_met;

  // jet
  for (auto jet: m_jets) delete jet.second;

  // truth fat jet
  for (auto truth_fatjet: m_truth_fatjets) delete truth_fatjet.second;

  // truth
  for (auto truth: m_truth) delete truth.second;

  // track
  for (auto track: m_tracks) delete track.second;
}

HelpTreeBase::HelpTreeBase(TTree        *tree,
                           TFile        *file,
                           xAOD::TEvent *event,
                           xAOD::TStore *store,
                           const float   units,
...
void HelpTreeBase::AddEvent(const std::string detailStr) {
  if (m_debug) Info("AddEvent()", "Adding event variables: %s",
                    detailStr.c_str());
...
void HelpTreeBase::AddTrigger(const std::string detailStr) {
  if (m_debug) Info("AddTrigger()",
                    "Adding trigger variables: %s",
                    detailStr.c_str());
...
void HelpTreeBase::FillTrigger(const xAOD::EventInfo *eventInfo) {
  ...
  if (m_trigInfoSwitch->m_basic) {
    if (m_debug) Info("HelpTreeBase::FillTrigger()",
                      "Switch: m_trigInfoSwitch->m_basic");
    static SG::AuxElement::ConstAccessor<int> passAny("passAny");
    static SG::AuxElement::ConstAccessor<int> passL1("passL1");

    if (passL1.isAvailable(*eventInfo)) m_passL1 = passL1(*eventInfo); else m_passL1 = -1; static SG::AuxElement::ConstAccessor<int> passHLT("passHLT");

    if (passHLT.isAvailable(*eventInfo)) m_passHLT = passHLT(*eventInfo); else m_passHLT = -1;
  }
  ...
  if (m_trigInfoSwitch->m_menuKeys) {
    ...
    if (masterKey.isAvailable(*eventInfo)) m_masterKey = masterKey(*eventInfo); else m_masterKey = -999; static SG::AuxElement::ConstAccessor<int> L1PSKey("L1PSKey");

    if (L1PSKey.isAvailable(*eventInfo)) m_L1PSKey = L1PSKey(*eventInfo); else m_L1PSKey = -999; static SG::AuxElement::ConstAccessor<int> HLTPSKey("HLTPSKey");

    if (HLTPSKey.isAvailable(*eventInfo)) m_HLTPSKey = HLTPSKey(*eventInfo); else m_HLTPSKey = -999;
  }

  // If detailed information about each and every trigger is desired
  // save a vector of strings holding passing decisions
  if (m_trigInfoSwitch->m_passTriggers) {
    if (m_debug) Info("HelpTreeBase::FillTrigger()", "Switch: m_trigInfoSwitch->m_passTriggers"); static SG::AuxElement::ConstAccessor<std::vector<std::string> > passTrigs("passTriggers");
...
void HelpTreeBase::FillTruth(const std::string truthName, const xAOD::TruthParticleContainer *truthParts) {
  ...
  // We need some basic cuts here to avoid many PseudoRapiditity warnings being
  // thrown ...
...
void HelpTreeBase::FillTracks(const std::string trackName, const xAOD::TrackParticleContainer *trackParts) {
  ...
  for (auto track_itr : *trackParts) {
    // if((track_itr->pt() / m_units < trackparticle_ptmin) ||
    // (fabs(track_itr->eta()) > trackparticle_etamax) ){
    //  continue;
    // }

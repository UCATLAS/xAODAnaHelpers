...
bool HelperFunctions::passPrimaryVertexSelection(const xAOD::VertexContainer* vertexContainer, int Ntracks) {
    ...
    if(!primaryVertex) return false; if( (int) (primaryVertex)->nTrackParticles() < Ntracks ) {
        return false;
    }
    return true;
}
...
bool HelperFunctions::applyPrimaryVertexSelection(const xAOD::JetContainer* jets, const xAOD::VertexContainer* vertices) {
    ...
    if(pvLocation < 0) return false; const xAOD::Vertex* vertex = vertices->at(pvLocation);
    ...
    for(auto jet_itr : *jets) {
        if( !ghostTrack.isAvailable(*jet_itr) ) continue; std::vector<ElementLink<xAOD::IParticleContainer> > trackLinks = ghostTrack(*jet_itr);
    ...
    return true;
} // HelperFunctions::applyPrimaryVertexSelection
...
float HelperFunctions::GetBTagMV2c20_Cut(int efficiency) {
    if(efficiency == 85) return -0.7887; else if(efficiency == 77) return -0.4434;
    else if(efficiency == 70) return -0.0436;
    else if(efficiency == 60) return 0.4496;
    else std::cout << "WARNING!! UNKNOWN BTAG EFFICIENCY POINT " << efficiency << std::endl; return -1; // no cut
}
...
std::vector<TLorentzVector> HelperFunctions::jetReclustering(
    ...
    return rc_t_jets;
} // HelperFunctions::jetReclustering

...
bool HelperFunctions::passPrimaryVertexSelection(
...
int HelperFunctions::countPrimaryVertices(
  const xAOD::VertexContainer *vertexContainer,
  int                          Ntracks)
{
  int NPV = 0;
  ...
  {
    if ((int)vtx_itr->nTrackParticles() < Ntracks) continue; NPV++;
  }

  return NPV;
}

int HelperFunctions::getPrimaryVertexLocation(
  const xAOD::VertexContainer *vertexContainer)
{
  int location(0);
  ...
  Warning("HelperFunctions::getPrimaryVertexLocation()",
          "No primary vertex location was found! Returning -1");
  return -1;
}

bool HelperFunctions::applyPrimaryVertexSelection(
  ...
  const xAOD::VertexContainer *vertices)
{
  ...
  if (pvLocation < 0) return false; const xAOD::Vertex *vertex = vertices->at(
    pvLocation);

  // check if the PV compatible Ghost Matched tracks are already here
  static SG::AuxElement::ConstAccessor<std::vector<ElementLink<xAOD::
                                                               IParticleContainer> > >
  ghostTrackPVAcc("GhostTrackPV");
  ...
  for (auto jet_itr : *jets) {
    ...
    std::vector<ElementLink<xAOD::IParticleContainer> > trackLinks = ghostTrack(
      *jet_itr);
    ...
    for (auto link_itr : trackLinks) {
      ...
      if (track->vertex() != vertex) {                  // if not in PV vertex
                                                        // fit
        if (track->vertex() != 0) continue;             // make sure in no
                                                        // vertex fits

        if (fabs((track->z0() + track->vz() - vertex->z()) *
                 sin(track->theta())) > 3.0) continue;  // make sure close to PV
                                                        // in z
      }
    ...
    jet_itr->auxdecor<std::vector<ElementLink<xAOD::IParticleContainer> > >(
      "GhostTrackPV") = selectedTrackHolder;
  } // loop over jets
...
float HelperFunctions::GetBTagMV2c20_Cut(int efficiency) {
  ...
  else if (efficiency == 60) return 0.4496; else std::cout << "WARNING!! UNKNOWN BTAG EFFICIENCY POINT " << efficiency << std::endl; return -1; //
                                                                                                                                                //
                                                                                                                                                //
                                                                                                                                                // no
                                                                                                                                                //
...
std::string HelperFunctions::GetBTagMV2c20_CutStr(int efficiency) {
  ...
  valueStr.replace(valueStr.find('.'), 1, "_"); // replace period with
                                                // underscore
...
std::vector<TLorentzVector>HelperFunctions::jetReclustering(
  ...
  // 3. Run the Cluster Sequence on pseudojets with the right jet definition
  // above
  //    cs = clustersequence
...
TLorentzVector HelperFunctions::jetTrimming(
  ...
  // 3. Need to use ClusterSequence to recluster jet again once we found
  // constituents
...
const xAOD::Vertex * HelperFunctions::getPrimaryVertex(const xAOD::VertexContainer *vertexContainer, bool quiet)
{
  ...
  for (auto vtx_itr : *vertexContainer)
  {
    if (vtx_itr->vertexType() != xAOD::VxType::VertexType::PriVtx) continue; return vtx_itr;
  }

  if (!quiet) Warning("HelperFunctions::getPrimaryVertex()", "No primary vertex was found! Returning nullptr");
...
//
// CP::make_systematics_vector(recSysts); has some similar functionality but
// does not
// prune down to 1 systematic if only request that one.  It does however include
// the
...
bool HelperFunctions::has_exact(const std::string input, const std::string flag)
{
  ...
  while (std::getline(ss, token, ' ')) inputSet.insert(token);

...
TrackContainer::TrackContainer(const std::string& name,
                               const std::string& detailStr,
                               float              units) : ParticleContainer(name,
                                                                             detailStr,
                                                                             units,
                                                                             true)
...
void TrackContainer::setTree(TTree *tree)
{
  ...
  if (m_infoSwitch.m_numbers) {
    ...
    connectBranch<unsigned char>(tree,
                                 ...
                                 &m_numberOfPrecisionHoleLayers);
    connectBranch<unsigned char>(tree,
                                 "numberOfPrecisionLayers",
                                 &m_numberOfPrecisionLayers);
    connectBranch<unsigned char>(tree,
  ...
  if (m_infoSwitch.m_vertex) {
    /*
       ...
       connectBranch<UInt_t>(tree, "vertexLink_persIndex",
          &m_vertexLink_persIndex);
...
void TrackContainer::updateParticle(uint idx, TrackPart& track)
{
  ...
  if (m_infoSwitch.m_fitpars) {
    ...
    track.definingParametersCovMatrix = m_definingParametersCovMatrix->at(
      idx);
    track.expectInnermostPixelLayerHit = m_expectInnermostPixelLayerHit->at(
      idx);
    track.expectNextToInnermostPixelLayerHit =
      m_expectNextToInnermostPixelLayerHit->at(idx);
  }

  if (m_infoSwitch.m_numbers) {
    track.numberDoF                       = m_numberDoF->at(idx);
    track.numberOfInnermostPixelLayerHits =
      m_numberOfInnermostPixelLayerHits->at(idx);
    track.numberOfNextToInnermostPixelLayerHits =
      m_numberOfNextToInnermostPixelLayerHits->at(idx);
    track.numberOfPhiHoleLayers =
      m_numberOfPhiHoleLayers->at(idx);
    track.numberOfPhiLayers        = m_numberOfPhiLayers->at(idx);
    track.numberOfPixelDeadSensors = m_numberOfPixelDeadSensors->at(
      idx);
    track.numberOfPixelHits       = m_numberOfPixelHits->at(idx);
    track.numberOfPixelHoles      = m_numberOfPixelHoles->at(idx);
    track.numberOfPixelSharedHits = m_numberOfPixelSharedHits->at(
      idx);
    track.numberOfPrecisionHoleLayers =
      m_numberOfPrecisionHoleLayers->at(idx);
    track.numberOfPrecisionLayers = m_numberOfPrecisionLayers->at(
      idx);
    track.numberOfSCTDeadSensors =
      m_numberOfSCTDeadSensors->at(idx);
    track.numberOfSCTHits       = m_numberOfSCTHits->at(idx);
    track.numberOfSCTHoles      = m_numberOfSCTHoles->at(idx);
    track.numberOfSCTSharedHits =
      m_numberOfSCTSharedHits->at(idx);
...
void TrackContainer::FillTrack(const xAOD::IParticle *particle) {
  ...
  const xAOD::TrackParticle *track =
    dynamic_cast<const xAOD::TrackParticle *>(particle);
  ...
  if (m_infoSwitch.m_fitpars) {
    ...
    // m_definingParametersCovMatrix
    // ->push_back(track->definingParametersCovMatrix() ); // fix this too

    static SG::AuxElement::ConstAccessor<char> expectInnermostPixelLayerHit(
      "expectInnermostPixelLayerHit");

    // safeFill<char, int, xAOD::TrackParticle>(track,
    // expectInnermostPixelLayerHit, m_expectInnermostPixelLayerHit, -999);
    ...
    // );
    static SG::AuxElement::ConstAccessor<char> expectNextToInnermostPixelLayerHit(
      "expectNextToInnermostPixelLayerHit");
    m_expectNextToInnermostPixelLayerHit->push_back(expectNextToInnermostPixelLayerHit(
                                                      *track));
  }
  ...
  if (m_infoSwitch.m_numbers) {
    ...
    static SG::AuxElement::ConstAccessor<unsigned char>
    numberOfInnermostPixelLayerHits("numberOfInnermostPixelLayerHits");
    m_numberOfInnermostPixelLayerHits->push_back(numberOfInnermostPixelLayerHits(*
                                                                                 track));

    static SG::AuxElement::ConstAccessor<unsigned char>
    numberOfNextToInnermostPixelLayerHits("numberOfNextToInnermostPixelLayerHits");
    m_numberOfNextToInnermostPixelLayerHits->push_back(numberOfNextToInnermostPixelLayerHits(
                                                         *track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfPhiHoleLayers(
      "numberOfPhiHoleLayers");
    m_numberOfPhiHoleLayers->push_back(numberOfPhiHoleLayers(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfPhiLayers(
      "numberOfPhiLayers");
    m_numberOfPhiLayers->push_back(numberOfPhiLayers(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfPixelDeadSensors(
      "numberOfPixelDeadSensors");
    m_numberOfPixelDeadSensors->push_back(numberOfPixelDeadSensors(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfPixelHits(
      "numberOfPixelHits");
    m_numberOfPixelHits->push_back(numberOfPixelHits(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfPixelHoles(
      "numberOfPixelHoles");
    m_numberOfPixelHoles->push_back(numberOfPixelHoles(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfPixelSharedHits(
      "numberOfPixelSharedHits");
    m_numberOfPixelSharedHits->push_back(numberOfPixelSharedHits(*track));

    static SG::AuxElement::ConstAccessor<unsigned char>
    numberOfPrecisionHoleLayers("numberOfPrecisionHoleLayers");
    m_numberOfPrecisionHoleLayers->push_back(numberOfPrecisionHoleLayers(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfPrecisionLayers(
      "numberOfPrecisionLayers");
    m_numberOfPrecisionLayers->push_back(numberOfPrecisionLayers(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfSCTDeadSensors(
      "numberOfSCTDeadSensors");
    m_numberOfSCTDeadSensors->push_back(numberOfSCTDeadSensors(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfSCTHits(
      "numberOfSCTHits");
    m_numberOfSCTHits->push_back(numberOfSCTHits(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfSCTHoles(
      "numberOfSCTHoles");
    m_numberOfSCTHoles->push_back(numberOfSCTHoles(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfSCTSharedHits(
      "numberOfSCTSharedHits");
    m_numberOfSCTSharedHits->push_back(numberOfSCTSharedHits(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfTRTHits(
      "numberOfTRTHits");
    m_numberOfTRTHits->push_back(numberOfTRTHits(*track));

    static SG::AuxElement::ConstAccessor<unsigned char> numberOfTRTOutliers(
      "numberOfTRTOutliers");

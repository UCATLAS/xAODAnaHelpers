...
EL::StatusCode TauEfficiencyCorrector :: initialize ()
{
  ...
  // 1.
  // initialize the CP::TauEfficiencyScaleFactors Tool for reco efficiency SF
  //
  //m_recoEffSF_tool_name = "TauEfficiencyScaleFactors_effSF_Reco_" + m_WorkingPointReco;
  ...
  //} else {
    ...
    //
    //m_outputSystNames = m_outputSystNames + "_Reco" + m_WorkingPointReco;

    //ANA_CHECK( m_tauEffCorrTool_handle->setProperty("WorkingPoint", m_WorkingPointReco ));

    if (!m_RecommendationTag.empty()) ANA_CHECK(m_tauEffCorrTool_handle.setProperty("RecommendationTag",m_RecommendationTag));

    ANA_CHECK(m_tauEffCorrTool_handle.setProperty("IDLevel", (int)TauAnalysisTools::JETIDBDTTIGHT));
 
    std::vector<int> setting;
    setting.push_back(TauAnalysisTools::SFRecoHadTau);
    setting.push_back(TauAnalysisTools::SFJetIDHadTau);

    ANA_CHECK(m_tauEffCorrTool_handle.setProperty("EfficiencyCorrectionTypes",setting));
    //ANA_CHECK(m_tauEffCorrTool_handle.setProperty("ApplyCombinedTES",m_applyCombinedTES));
...
EL::StatusCode TauEfficiencyCorrector :: execute ()
{
  ...
  for ( auto systName : systNames ) {
    ...
    if ( m_store->contains<xAOD::TauJetContainer>( m_inContainerName+systName ) ) {
      ...
      ANA_MSG_DEBUG( "Number of muons: " << static_cast<int>(inputTaus->size()) );
...
EL::StatusCode TauEfficiencyCorrector :: executeSF ( const xAOD::EventInfo* /*eventInfo*/, const xAOD::TauJetContainer* inputTaus, bool nominal, bool writeSystNames )
{
  ...
  if ( !isToolAlreadyUsed(m_recoEffSF_tool_name) ) {
    ...
    for ( const auto& syst_it : m_systListReco ) {
      ...
      for ( auto mu_itr : *(inputTaus) ) {
         ...
         ANA_MSG_DEBUG( "===>>>");
         ANA_MSG_DEBUG( "Tau " << idx << ", pt = " << mu_itr->pt()*1e-3 << " GeV" );
  ...
  if ( !isToolAlreadyUsed(m_isoEffSF_tool_name) ) {
    ...
    for ( const auto& syst_it : m_systListIso ) {
      ...
      ANA_MSG_DEBUG( "Tau iso efficiency SF sys name (to be recorded in xAOD::TStore) is: " << syst_it.name() );
    ...
    for ( const auto& trig_it : m_SingleMuTriggers ) {
      ...
      for ( const auto& syst_it : m_systListTrig ) {
        ...
        if ( m_muTrigSF_tool->applySystematicVariation(syst_it) != CP::SystematicCode::Ok ) {
          ANA_MSG_ERROR( "Failed to configure TauTriggerScaleFactors for trigger " << trig_it << " systematic " << syst_it.name());
          return EL::StatusCode::FAILURE;
        }
        ...
        for ( auto mu_itr : *(inputTaus) ) {
           ...
           //
           ConstDataVector<xAOD::TauJetContainer> mySingleTauCont(SG::VIEW_ELEMENTS);
           mySingleTauCont.push_back( mu_itr );
           ...
           if ( trig_it != full_scan_chain ) {
             if ( m_muTrigSF_tool->getTriggerScaleFactor( *mySingleTauCont.asDataVector(), triggerEffSF, trig_it ) != CP::CorrectionCode::Ok ) {

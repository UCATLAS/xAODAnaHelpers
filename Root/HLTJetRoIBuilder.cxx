...
EL::StatusCode HLTJetRoIBuilder::initialize()
{
  if (m_debug) Info("initialize()",
                    "Initializing HLTJetRoIBuilder Interface... ");
  ...
  //
  m_trigDecTool =
    dynamic_cast<Trig::TrigDecisionTool *>(asg::ToolStore::get("TrigDecisionTool"));
...
EL::StatusCode HLTJetRoIBuilder::buildHLTBJets()
{
  ...
  for (std::string trig : triggersUsed) {
    ...
    if ((m_trigItemVeto != "") &&
        (trig.find(m_trigItemVeto) != std::string::npos)) {
      continue;
    }
  ...
  if (m_debug) {
    ...
    auto triggerChainGroupAfterVeto = m_trigDecTool->getChainGroup(
      m_trigItemAfterVeto);
    std::vector<std::string> triggersUsedAfterVeto =
      triggerChainGroupAfterVeto->getListOfTriggers();
  ...
  static xAOD::Jet::Decorator<vector<const xAOD::TrackParticle *> >
  m_track_decoration("HLTBJetTracks");
  static xAOD::Jet::Decorator<const xAOD::Vertex *> m_vtx_decoration(
    "HLTBJetTracks_vtx");
  static xAOD::Jet::Decorator<const xAOD::Vertex *> m_vtx_decoration_bkg(
    "HLTBJetTracks_vtx_bkg");
  static xAOD::Jet::Decorator<char> m_vtx_hadDummyPV("hadDummyPV");
  static xAOD::Jet::Decorator<const xAOD::Vertex *> m_offline_vtx_decoration(
    "offline_vtx");
  ...
  if (HelperFunctions::isAvailable(offline_vertices, "PrimaryVertices", m_event,
                                   m_store, m_verbose)) {
    RETURN_CHECK("HLTJetRoIBuilder::execute()",
                 HelperFunctions::retrieve(offline_vertices, "PrimaryVertices",
                                           m_event,
                                           m_store, m_verbose), "");
  ...
  Trig::FeatureContainer fc = m_trigDecTool->features(m_trigItemAfterVeto,
                                                      TrigDefs::Physics);
  Trig::FeatureContainer::combination_const_iterator comb(
    fc.getCombinations().begin());
  Trig::FeatureContainer::combination_const_iterator combEnd(
    fc.getCombinations().end());

  if (m_debug) cout << m_name << " New Event --------------- " << endl;

  for (; comb != combEnd; ++comb) {
    std::vector<Trig::Feature<xAOD::JetContainer> > jetCollections =
      comb->containerFeature<xAOD::JetContainer>(m_jetName);
    std::vector<Trig::Feature<xAOD::BTaggingContainer> > bjetCollections =
      comb->containerFeature<xAOD::BTaggingContainer>("HLTBjetFex");
    std::vector<Trig::Feature<xAOD::TrackParticleContainer> > trkCollections;

    if (m_readHLTTracks) trkCollections =
        comb->containerFeature<xAOD::TrackParticleContainer>(m_trkName);

    // std::vector< Trig::Feature<xAOD::TrackParticleContainer> >
    //  ftfCollections  =
    // comb->containerFeature<xAOD::TrackParticleContainer>("InDetTrigTrackingxAODCnv_Bjet_FTF");
    ...
    std::vector<Trig::Feature<xAOD::VertexContainer> > backupVtxCollections =
      comb->containerFeature<xAOD::VertexContainer>("EFHistoPrmVtx");
    ...
    //    cout << " Test Size " << endl;
    //    cout << " \tSplitJet: " <<
    // comb->containerFeature<xAOD::JetContainer>("SplitJet").size() << endl;
    //    cout << " \tGSCJet: "   <<
    // comb->containerFeature<xAOD::JetContainer>("GSCJet")  .size() << endl;
    //    cout << " \tEFJet:  "   <<
    // comb->containerFeature<xAOD::JetContainer>("EFJet")   .size() << endl;
    //
    // FTK Vertex debugging
    //    cout << " Test Size " << endl;
    //    cout << " \tempty: " <<
    // comb->containerFeature<xAOD::VertexContainer>().size() << endl;
    //    cout << " \tEFHistoPrmVtx: " <<
    // comb->containerFeature<xAOD::VertexContainer>("EFHistoPrmVtx").size()
    // <<endl;
    //    cout << " \txPrimVx: " <<
    // comb->containerFeature<xAOD::VertexContainer>("xPrimVx").size() << endl;
    //    cout << " \tPrimVertexFTK " <<
    // comb->containerFeature<xAOD::VertexContainer>("PrimVertexFTK").size() <<
    // endl;
    //    cout << " \tPrimVertexFTKRaw " <<
    // comb->containerFeature<xAOD::VertexContainer>("PrimVertexFTKRaw").size()
    // << endl;
    //    cout << " \tPrimVertexFTKRefit " <<
    // comb->containerFeature<xAOD::VertexContainer>("PrimVertexFTKRefit").size()<<
    // endl;
    //    cout << " \tHLT_PrimVertexFTK " <<
    // comb->containerFeature<xAOD::VertexContainer>("HLT_PrimVertexFTK").size()
    // << endl;
    //
    //    std::vector<Trig::Feature<xAOD::VertexContainer> > EvtxCollections =
    // comb->containerFeature<xAOD::VertexContainer>();
    ...
    if (jetCollections.size() != bjetCollections.size()) {
      cout << "ERROR Problem in container size: " << m_name << " jets: " <<
        jetCollections.size() << " bjets: " << bjetCollections.size() << endl;
      isValid = false;

      if (m_debug) {
        auto triggerChainGroupAfterVeto =
          m_trigDecTool->getChainGroup(m_trigItemAfterVeto);
        std::vector<std::string> triggersUsedAfterVeto =
          triggerChainGroupAfterVeto->getListOfTriggers();
        ...
        for (std::string trig : triggersUsedAfterVeto) {
          ...
          if (trigChain->isPassed()) cout << " \t " << trig << endl;
        }
      }
    }
    ...
    if (m_readHLTTracks && (jetCollections.size() != trkCollections.size())) {
      cout << "ERROR Problem in container size: " << m_name << " jets: " <<
        jetCollections.size() << " trks: " << trkCollections.size() << endl;
      cout << " Jet Collection " << m_jetName << " Trk Collection:  " <<
        m_trkName << endl;
      isValid = false;
    }
    ...
    //  cout << "ERROR Problem in container size: " << m_name  << " jets: "<<
    // jetCollections.size() << " ftfs: "<< ftfCollections.size() << endl;
    //  isValid = false;
    // }
    ...
    float var_bs_online_vx = m_onlineBSTool.getOnlineBSInfo(eventInfo,
                                                            xAH::OnlineBeamSpotTool::BSData::BSx);
    float var_bs_online_vy = m_onlineBSTool.getOnlineBSInfo(eventInfo,
                                                            xAH::OnlineBeamSpotTool::BSData::BSy);
    float var_bs_online_vz = m_onlineBSTool.getOnlineBSInfo(eventInfo,
                                                            xAH::OnlineBeamSpotTool::BSData::BSz);
    ...
    // cout << " is Valid " << jetCollections.size() << " " <<
    // vtxCollections.size() << endl;
    for (unsigned ifeat = 0; ifeat < jetCollections.size(); ifeat++) {
      const xAOD::Jet *hlt_jet = getTrigObject<xAOD::Jet, xAOD::JetContainer>(jetCollections.at(
                                                                                ifeat));
      ...
      if (m_debug) cout << "New Jet: pt: " << hlt_jet->pt() << " eta: " <<
          hlt_jet->eta() << " phi: " << hlt_jet->phi() << endl;

      const xAOD::BTagging *hlt_btag =
        getTrigObject<xAOD::BTagging,
                      xAOD::BTaggingContainer>(bjetCollections.at(ifeat));
      ...
      if (!HelperFunctions::getPrimaryVertex(vtxCollections.at(ifeat).cptr(),
        ...
        if (backupVtxCollections.size()) {
          ...
          const xAOD::Vertex *backup_pvx = HelperFunctions::getPrimaryVertex(backupVtxCollections.at(
                                                                               ifeat).cptr());
        ...
        // cout << "hadDummy and vtxType " << m_vtx_hadDummyPV (*newHLTBJet) <<
        // " " << m_vtxName << endl;
      } else {
        m_vtx_decoration(*newHLTBJet) = HelperFunctions::getPrimaryVertex(vtxCollections.at(
                                                                            ifeat).cptr());
        m_vtx_hadDummyPV(*newHLTBJet) = '0';

        if (backupVtxCollections.size()) {
          m_vtx_decoration_bkg(*newHLTBJet) = HelperFunctions::getPrimaryVertex(backupVtxCollections.at(
                                                                                  ifeat).cptr());
        } else {
          m_vtx_decoration_bkg(*newHLTBJet) = 0;
        }
      ...
      //   cout << "hadDummy and vtxType and m_outContainerName  " <<
      // m_vtx_hadDummyPV (*newHLTBJet) << " "
...
EL::StatusCode HLTJetRoIBuilder::buildHLTJets()
{
  ...
  // DataModel_detail::const_iterator<JetContainer >::reference {aka const
  // xAOD::Jet_v1*}

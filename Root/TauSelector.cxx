  ...
  m_TOELLHDecorator(nullptr)
{
  ...
  m_ConfigPath =
    "$ROOTCOREBIN/data/xAODAnaHelpers/TauConf/00-01-19/Selection/recommended_selection_mc15.conf";
  m_EleOLRFilePath = "";
...
EL::StatusCode TauSelector::initialize()
{
  ...
  if (m_useCutFlow) {
    ...
    if (m_isUsedBefore) {
      ...
      m_tau_cutflow_selected =
        m_tau_cutflowHist_2->GetXaxis()->FindBin("selected");
    }
  }
  ...
  RETURN_CHECK("TauSelector::initialize()",
               m_TauSelTool->setProperty("ConfigPath",
                                         m_ConfigPath.c_str()),
               "Failed to set ConfigPath property");

  if (!m_EleOLRFilePath.empty()) {
    RETURN_CHECK("TauSelector::initialize()",
                 m_TauSelTool->setProperty("EleOLRFilePath",
                                           m_EleOLRFilePath.c_str()),
                 "Failed to set EleOLRFilePath property");
  }
  RETURN_CHECK("TauSelector::initialize()",
               m_TauSelTool->initialize(),
               "Failed to properly initialize TauSelectionTool");

  if (m_setTauOverlappingEleLLHDecor) {
    std::string eleOLR_tool_name = std::string(
      "TauOverlappingElectronLLHDecorator_") + m_name;
    m_TOELLHDecorator = new TauAnalysisTools::TauOverlappingElectronLLHDecorator(
      eleOLR_tool_name);
    m_TOELLHDecorator->msg().setLevel(MSG::INFO); // VERBOSE, INFO, DEBUG
    RETURN_CHECK("TauSelector::initialize()",
                 m_TOELLHDecorator->initialize(),
                 "Failed to properly initialize TauOverlappingElectronLLHDecorator");
  }
...
EL::StatusCode TauSelector::execute()
{
  ...
  RETURN_CHECK("TauSelector::execute()",
  ...
  if (m_inputAlgoSystNames.empty()) {
    ...
    //
    RETURN_CHECK("TauSelector::execute()",
                 HelperFunctions::retrieve(inTaus, m_inContainerName, m_event,
                                           m_store,
                                           m_verbose), "");
    ...
    if (m_createSelectedContainer) selectedTaus =
        new ConstDataVector<xAOD::TauJetContainer>(SG::VIEW_ELEMENTS);
    ...
    for (auto systName : *systNames) {
      ...
      RETURN_CHECK("TauSelector::execute()",
                   HelperFunctions::retrieve(inTaus, m_inContainerName + systName,
                                             m_event,
                                             m_store, m_verbose), "");
      ...
      // find the selected Taus, and return if event passes object selection
      //
      eventPassThisSyst = executeSelection(inTaus,
                                           mcEvtWeight,
                                           countPass,
                                           selectedTaus);
...
bool TauSelector::executeSelection(const xAOD::TauJetContainer            *inTaus,
                                   float                                   mcEvtWeight,
                                   bool                                    countPass,
                                   ConstDataVector<xAOD::TauJetContainer> *selectedTaus)
{
  ...
  if (m_debug) Info("executeSelection()", "Initial Taus: %u ",
                    static_cast<uint32_t>(inTaus->size()));
  ...
  if (m_debug) Info("executeSelection()",
                    "Initial Taus (count obj): %i - Selected Taus: %i",
                    nObj,
                    nPass);
  ...
  if ((m_pass_min > 0) && (nPass < m_pass_min)) {
    if (m_debug) Info("executeSelection()",
      "Reject event: nSelectedTaus (%i) < nPassMin (%i)",
      nPass,
      m_pass_min); return false;
  }

  if ((m_pass_max > 0) && (nPass > m_pass_max)) {
    if (m_debug) Info("executeSelection()", "Reject event: nSelectedTaus (%i) > nPassMax (%i)", nPass, m_pass_max); return false;
  }
...
int TauSelector::passCuts(const xAOD::TauJet *tau) {
  ...
  if (tau->pt() <= m_minPtDAOD) {
    if (m_debug) Info("PassCuts()", "Tau failed minimal pT requirement for usage with derivations"); return 0;
  }

  if (m_setTauOverlappingEleLLHDecor) m_TOELLHDecorator->decorate(*tau);

  if (!m_TauSelTool->accept(*tau)) {
    if (m_debug) Info("PassCuts()", "Tau failed requirements of TauSelectionTool"); return 0;
  }

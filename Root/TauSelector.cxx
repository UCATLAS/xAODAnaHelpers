  ...
  m_TauSelTool(nullptr)
{
  ...
  m_EleOLRFilePath          = "$ROOTCOREBIN/data/xAODAnaHelpers/TauConf/00-01-19/Selection/eveto_cutvals.root";
...
EL::StatusCode TauSelector :: initialize ()
{
  ...
  //
  RETURN_CHECK("TauSelector::initialize()", m_muonSelectionTool->setProperty("MaxEta",    static_cast<double>(m_eta_max)), "Failed to set MaxEta property");
  RETURN_CHECK("TauSelector::initialize()", m_muonSelectionTool->setProperty("MuQuality", m_muonQuality), "Failed to set MuQuality property" );
  RETURN_CHECK("TauSelector::initialize()", m_muonSelectionTool->initialize(), "Failed to properly initialize the Muon Selection Tool");
  ...
  RETURN_CHECK( "TauSelector::initialize()", m_IsolationSelectionTool->setProperty("MuonWP", (m_IsoKeys.at(0)).c_str()), "Failed to configure base WP" );
  RETURN_CHECK( "TauSelector::initialize()", m_IsolationSelectionTool->initialize(), "Failed to properly initialize IsolationSelectionTool." );
  //
  ...
  if( !( m_singleMuTrigChains.empty() && m_diMuTrigChains.empty() ) && asg::ToolStore::contains<Trig::TrigDecisionTool>( "TrigDecisionTool" ) ) {
    ...
    RETURN_CHECK( "TauSelector::initialize()", m_trigMuonMatchTool->setProperty( "TriggerTool", trigDecHandle ), "Failed to pass TrigDecisionTool to TrigMuonMatching" );
    RETURN_CHECK( "TauSelector::initialize()", m_trigMuonMatchTool->initialize(), "Failed to properly initialize TrigMuonMatching." );
...
EL::StatusCode TauSelector :: execute ()
{
  ...
  if ( m_inputAlgoSystNames.empty() ) {
    ...
    //
    RETURN_CHECK("TauSelector::execute()", HelperFunctions::retrieve(inMuons, m_inContainerName, m_event, m_store, m_verbose) ,"");
	...
	//
        RETURN_CHECK( "TauSelector::execute()", m_store->record( selectedMuons, m_outContainerName ), "Failed to store const data container");
      } else {
    ...
    for ( auto systName : *systNames ) {
      ...
      RETURN_CHECK("TauSelector::execute()", HelperFunctions::retrieve(inMuons, m_inContainerName + systName, m_event, m_store, m_verbose) ,"");
	...
	vecOutContainerNames->push_back( systName );
      ...
      if ( m_createSelectedContainer ) {
        if ( eventPassThisSyst ) {
          // add ConstDataVector to TStore
	  //
          RETURN_CHECK( "TauSelector::execute()", m_store->record( selectedMuons, m_outContainerName+systName ), "Failed to store const data container");
        } else {
...
bool TauSelector :: executeSelection ( const xAOD::MuonContainer* inMuons, float mcEvtWeight, bool countPass,
					    ConstDataVector<xAOD::MuonContainer>* selectedMuons )
{

  const xAOD::VertexContainer* vertices(nullptr);
  RETURN_CHECK("TauSelector::executeSelection()", HelperFunctions::retrieve(vertices, "PrimaryVertices", m_event, m_store, m_verbose) ,"");
...
int TauSelector :: passCuts( const xAOD::Muon* muon, const xAOD::Vertex *primaryVertex  ) {
  ...
  // *********************************************************************************************************************************************************************
  // 
  // TauSelectorTool cut: quality & |eta| acceptance cut
  //

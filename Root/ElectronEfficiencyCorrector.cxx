...
EL::StatusCode ElectronEfficiencyCorrector::setupJob(EL::Job& job)
{
  ...
  job.useXAOD();
  xAOD::Init("ElectronEfficiencyCorrector").ignore(); // call before opening
                                                      // first file
...
EL::StatusCode ElectronEfficiencyCorrector::initialize()
{
  ...
  RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
  ...
  if (m_isMC) {
    const std::string stringMeta = wk()->metaData()->castString(
      "SimulationFlavour");

    if (m_setAFII ||
        (!stringMeta.empty() && (stringMeta.find("AFII") != std::string::npos))) {
      Info("initialize()", "Setting simulation flavour to AFII");
      sim_flav = 3;
    }
    else if (!m_setAFII && stringMeta.empty()) {
      Warning("initialize()",
              "No meta-data string found. Simulation flavour will be set to FullSim. Care if you are running on Fast-Sim MC.");
    }
  }
  ...
  if (!m_corrFileNamePID.empty()) {
    ...
    if (m_PID_WP.empty()) {
      Error("initialize()",
            "ID working point for electronID SF not found in config file! This should not happen. Exiting.");
      return EL::StatusCode::FAILURE;
    }

    std::cout << "\n\n Electron ID wp: " << m_PID_WP << "\n\n" << std::endl;

    m_pidEffSF_tool_name = "ElectronEfficiencyCorrectionTool_effSF_PID_" +
                           m_PID_WP;

    RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                 checkToolStore<AsgElectronEfficiencyCorrectionTool>(
                   m_pidEffSF_tool_name),
                 "");

    if (asg::ToolStore::contains<AsgElectronEfficiencyCorrectionTool>(
          m_pidEffSF_tool_name)) {
      m_asgElEffCorrTool_elSF_PID =
        asg::ToolStore::get<AsgElectronEfficiencyCorrectionTool>(
          m_pidEffSF_tool_name);
    } else {
      m_asgElEffCorrTool_elSF_PID = new AsgElectronEfficiencyCorrectionTool(
        m_pidEffSF_tool_name);
      m_asgElEffCorrTool_elSF_PID->msg().setLevel(MSG::ERROR);      // DEBUG,
                                                                    // VERBOSE,
                                                                    // INFO
      std::vector<std::string> inputFilesPID { m_corrFileNamePID }; // initialise
                                                                    // vector w/
                                                                    // all the
                                                                    // files
                                                                    // containing
                                                                    // corrections
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_PID->setProperty(
                     "CorrectionFileNameList",
                     inputFilesPID),
                   "Failed to set property CorrectionFileNameList");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_PID->setProperty("ForceDataType",
                                                            sim_flav),
                   "Failed to set property ForceDataType");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_PID->setProperty("CorrelationModel",
                                                            m_correlationModel),
                   "Failed to set property CorrelationModel");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_PID->initialize(),
                   "Failed to properly initialize the AsgElectronEfficiencyCorrectionTool PID");
    }

    if (m_debug) {
      // Get a list of affecting systematics
      //
      CP::SystematicSet affectSystsPID =
        m_asgElEffCorrTool_elSF_PID->affectingSystematics();

      //
      // Convert into a simple list
      //
      for (const auto& syst_it : affectSystsPID) Info("initialize()",
                                                      "AsgElectronEfficiencyCorrectionTool can be affected by PID efficiency systematic: %s",
                                                      (syst_it.name()).c_str());
    }
    ...
    //
    const CP::SystematicSet recSystsPID =
      m_asgElEffCorrTool_elSF_PID->recommendedSystematics();
    m_systListPID = HelperFunctions::getListofSystematics(recSystsPID,
                                                          m_systNamePID,
                                                          m_systValPID,
                                                          m_debug);

    Info("initialize()",
         "Will be using AsgElectronEfficiencyCorrectionTool PID efficiency systematic:");
  ...
  // 2.
  // initialize the AsgElectronEfficiencyCorrectionTool for isolation efficiency
  // SF
  //
  if (!m_corrFileNameIso.empty()) {
    ...
    if (m_Iso_WP.empty()) {
      Error("initialize()",
            "ISO working point for isolation SF not found in config file! This should not happen. Exiting.");
      return EL::StatusCode::FAILURE;
    }

    if (m_IsoPID_WP.empty()) {
      Error("initialize()",
            "ID working point for isolation SF not found in config file! This should not happen. Exiting.");
      return EL::StatusCode::FAILURE;
    }
    ...
    m_IsoEffSF_tool_name = "ElectronEfficiencyCorrectionTool_effSF_Iso_" +
                           m_IsoPID_WP + "_isol" + m_Iso_WP;
    ...
    if (asg::ToolStore::contains<AsgElectronEfficiencyCorrectionTool>(
      ...
      m_asgElEffCorrTool_elSF_Iso = new AsgElectronEfficiencyCorrectionTool(
        m_IsoEffSF_tool_name);
      m_asgElEffCorrTool_elSF_Iso->msg().setLevel(MSG::ERROR);      // DEBUG,
                                                                    // VERBOSE,
                                                                    // INFO
      std::vector<std::string> inputFilesIso { m_corrFileNameIso }; // initialise
                                                                    // vector w/
                                                                    // all the
                                                                    // files
                                                                    // containing
                                                                    // corrections
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Iso->setProperty(
                     "CorrectionFileNameList",
                     inputFilesIso),
                   "Failed to set property CorrectionFileNameList");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Iso->setProperty("ForceDataType",
                                                            sim_flav),
                   "Failed to set property ForceDataType");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Iso->setProperty("CorrelationModel",
                                                            m_correlationModel),
                   "Failed to set property CorrelationModel");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Iso->initialize(),
                   "Failed to properly initialize the AsgElectronEfficiencyCorrectionTool Iso");
    }

    if (m_debug) {
      // Get a list of affecting systematics
      //
      CP::SystematicSet affectSystsIso =
        m_asgElEffCorrTool_elSF_Iso->affectingSystematics();

      //
      // Convert into a simple list
      //
      for (const auto& syst_it : affectSystsIso) Info("initialize()",
                                                      "AsgElectronEfficiencyCorrectionTool can be affected by Iso efficiency systematic: %s",
                                                      (syst_it.name()).c_str());
    }
    ...
    const CP::SystematicSet recSystsIso =
      m_asgElEffCorrTool_elSF_Iso->recommendedSystematics();
    ...
    Info("initialize()",
         "Will be using AsgElectronEfficiencyCorrectionTool Iso efficiency systematic:");
    ...
    //  Add the chosen WP to the string labelling the vector<SF> decoration
    //
    m_outputSystNamesIso = m_outputSystNamesIso + "_" + m_IsoPID_WP + "_isol" +
                           m_Iso_WP;
  }
  ...
  if (!m_corrFileNameReco.empty()) {
    ...
    if (asg::ToolStore::contains<AsgElectronEfficiencyCorrectionTool>(
      ...
      m_asgElEffCorrTool_elSF_Reco = new AsgElectronEfficiencyCorrectionTool(
        m_RecoEffSF_tool_name);
      m_asgElEffCorrTool_elSF_Reco->msg().setLevel(MSG::ERROR);       // DEBUG,
                                                                      // VERBOSE,
                                                                      // INFO
      std::vector<std::string> inputFilesReco { m_corrFileNameReco }; // initialise
                                                                      // vector
                                                                      // w/ all
                                                                      // the
                                                                      // files
                                                                      ...
                                                                      // corrections
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Reco->setProperty(
                     "CorrectionFileNameList",
                     inputFilesReco),
                   "Failed to set property CorrectionFileNameList");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Reco->setProperty("ForceDataType",
                                                             sim_flav),
                   "Failed to set property ForceDataType");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Reco->setProperty("CorrelationModel",
                                                             m_correlationModel),
                   "Failed to set property CorrelationModel");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Reco->initialize(),
                   "Failed to properly initialize the AsgElectronEfficiencyCorrectionTool Reco");
    }

    if (m_debug) {
      // Get a list of affecting systematics
      //
      CP::SystematicSet affectSystsReco =
        m_asgElEffCorrTool_elSF_Reco->affectingSystematics();

      //
      // Convert into a simple list
      //
      for (const auto& syst_it : affectSystsReco) Info("initialize()",
                                                       "AsgElectronEfficiencyCorrectionTool can be affected by reco efficiency systematic: %s",
                                                       (syst_it.name()).c_str());
    }
    ...
    const CP::SystematicSet recSystsReco =
      m_asgElEffCorrTool_elSF_Reco->recommendedSystematics();
    ...
    Info("initialize()",
         "Will be using AsgElectronEfficiencyCorrectionTool reco efficiency systematic:");
  ...
  // 4.
  // Initialise the AsgElectronEfficiencyCorrectionTool for Trigger Efficiency
  // SF
  //
  if (!m_corrFileNameTrig.empty()) {
    ...
    m_WorkingPointTrigTrig = HelperFunctions::parse_wp("TRIG",
                                                       m_corrFileNameTrigMCEff);

    if (m_WorkingPointIDTrig.empty()) {
      Error("initialize()",
            "ID working point for trigger SF not found in config file! This should not happen. Exiting.");
      return EL::StatusCode::FAILURE;
    }
    ...
    m_TrigEffSF_tool_name = "ElectronEfficiencyCorrectionTool_effSF_Trig_" +
                            m_WorkingPointTrigTrig + "_" + m_WorkingPointIDTrig;
    ...
    if (asg::ToolStore::contains<AsgElectronEfficiencyCorrectionTool>(
      ...
      m_asgElEffCorrTool_elSF_Trig = new AsgElectronEfficiencyCorrectionTool(
        m_TrigEffSF_tool_name);
      m_asgElEffCorrTool_elSF_Trig->msg().setLevel(MSG::ERROR);       // DEBUG,
                                                                      // VERBOSE,
                                                                      // INFO
      std::vector<std::string> inputFilesTrig { m_corrFileNameTrig }; // initialise
                                                                      // vector
                                                                      // w/ all
                                                                      // the
                                                                      // files
                                                                      ...
                                                                      // corrections
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Trig->setProperty(
                     "CorrectionFileNameList",
                     inputFilesTrig),
                   "Failed to set property CorrectionFileNameList");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Trig->setProperty("ForceDataType",
                                                             sim_flav),
                   "Failed to set property ForceDataType");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Trig->setProperty("CorrelationModel",
                                                             m_correlationModel),
                   "Failed to set property CorrelationModel");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_Trig->initialize(),
                   "Failed to properly initialize the AsgElectronEfficiencyCorrectionTool Trig");
    }
    ...
    if (m_debug) {
      ...
      CP::SystematicSet affectSystsTrig =
        m_asgElEffCorrTool_elSF_Trig->affectingSystematics();

      //
      // Convert into a simple list
      //
      for (const auto& syst_it : affectSystsTrig) Info("initialize()",
                                                       "AsgElectronEfficiencyCorrectionTool can be affected by Trig efficiency SF systematic: %s",
                                                       (syst_it.name()).c_str());
    }
    ...
    const CP::SystematicSet recSystsTrig =
      m_asgElEffCorrTool_elSF_Trig->recommendedSystematics();
    ...
    Info("initialize()",
         "Will be using AsgElectronEfficiencyCorrectionTool Trig efficiency SF systematic:");
    ...
    //  Add the chosen WP to the string labelling the vector<SF> decoration
    //
    m_outputSystNamesTrig = m_outputSystNamesTrig + "_" + m_WorkingPointTrigTrig +
                            "_" + m_WorkingPointIDTrig;
  ...
  if (!m_corrFileNameTrigMCEff.empty()) {
    m_TrigMCEff_tool_name = "ElectronEfficiencyCorrectionTool_effSF_TrigMCEff_" +
                            m_WorkingPointTrigTrig + "_" + m_WorkingPointIDTrig;
    ...
    if (asg::ToolStore::contains<AsgElectronEfficiencyCorrectionTool>(
      ...
      m_asgElEffCorrTool_elSF_TrigMCEff = new AsgElectronEfficiencyCorrectionTool(
        m_TrigMCEff_tool_name);
      m_asgElEffCorrTool_elSF_TrigMCEff->msg().setLevel(MSG::ERROR);            //
                                                                                //
                                                                                //
                                                                                //
                                                                                // DEBUG,
                                                                                ...
                                                                                //
                                                                                // INFO
      std::vector<std::string> inputFilesTrigMCEff { m_corrFileNameTrigMCEff }; //
                                                                                //
                                                                                //
                                                                                //
                                                                                // initialise
                                                                                ...
                                                                                // corrections
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_TrigMCEff->setProperty(
                     "CorrectionFileNameList",
                     inputFilesTrigMCEff),
                   "Failed to set property CorrectionFileNameList");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_TrigMCEff->setProperty("ForceDataType",
                                                                  sim_flav),
                   "Failed to set property ForceDataType");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_TrigMCEff->setProperty(
                   ...
                   "Failed to set property CorrelationModel");
      RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
                   m_asgElEffCorrTool_elSF_TrigMCEff->initialize(),
                   "Failed to properly initialize the AsgElectronEfficiencyCorrectionTool TrigMCEff");
    }

    if (m_debug) {
      // Get a list of affecting systematics
      //
      CP::SystematicSet affectSystsTrigMCEff =
        m_asgElEffCorrTool_elSF_TrigMCEff->affectingSystematics();

      //
      // Convert into a simple list
      //
      for (const auto& syst_it : affectSystsTrigMCEff) Info("initialize()",
                                                            "AsgElectronEfficiencyCorrectionTool can be affected by TrigMCEff efficiency systematic: %s",
                                                            (syst_it.name()).c_str());
    ...
    // Use HelperFunctions::getListofSystematics() for this!
    //
    const CP::SystematicSet recSystsTrigMCEff =
      m_asgElEffCorrTool_elSF_TrigMCEff->recommendedSystematics();
    m_systListTrigMCEff = HelperFunctions::getListofSystematics(recSystsTrigMCEff,
                                                                m_systNameTrigMCEff,
                                                                m_systValTrigMCEff,
                                                                m_debug);

    Info("initialize()",
         "Will be using AsgElectronEfficiencyCorrectionTool TrigMCEff efficiency systematic:");
    ...
    //  Add the chosen WP to the string labelling the vector<SF> decoration
    //
    m_outputSystNamesTrigMCEff = m_outputSystNamesTrigMCEff + "_" +
                                 m_WorkingPointTrigTrig + "_" +
                                 m_WorkingPointIDTrig;
  ...
  Info("initialize()",
       "ElectronEfficiencyCorrector Interface succesfully initialized!");
...
EL::StatusCode ElectronEfficiencyCorrector::execute()
{
  ...
  if (!m_isMC) {
    if (m_numEvent == 1) Info("execute()",
                              "Sample is Data! Do not apply any Electron Efficiency correction... ");
  ...
  const xAOD::EventInfo *eventInfo(nullptr);
  RETURN_CHECK("ElectronEfficiencyCorrector::execute()",
               HelperFunctions::retrieve(eventInfo, m_eventInfoContainerName,
                                         m_event, m_store,
                                         m_verbose),
               "");
  ...
  if (m_inputAlgoSystNames.empty()) {
    // I might not want to decorate sys altered electrons but for some events
    // the nominal container might not exist
    // if electrons are only allowed in systematic instances, hence, we have to
    // check for the existence of the nominal container
    //
    if (m_store->contains<xAOD::ElectronContainer>(m_inContainerName)) {
      RETURN_CHECK("ElectronEfficiencyCorrector::execute()",
                   HelperFunctions::retrieve(inputElectrons, m_inContainerName,
                                             m_event,
                                             m_store, m_verbose),
                   "");

      if (m_debug) Info("execute", "Number of electrons: %i",
                        static_cast<int>(inputElectrons->size()));

      // decorate electrons w/ SF - there will be a decoration w/ different name
      // for each syst!
      //
    ...
    // from an upstream algo.
    // This is the case of calibrators: one different SC for each calibration
    // syst applied
    ...
    RETURN_CHECK("ElectronEfficiencyCorrector::execute()",
                 HelperFunctions::retrieve(systNames, m_inputAlgoSystNames, 0,
                                           m_store,
                                           m_verbose),
                 "");
    ...
    for (auto systName : *systNames) {
      ...
      if (m_store->contains<xAOD::ElectronContainer>(m_inContainerName +
                                                     systName)) {
        RETURN_CHECK("ElectronEfficiencyCorrector::execute()",
                     HelperFunctions::retrieve(inputElectrons,
                                               m_inContainerName + systName,
                                               m_event, m_store, m_verbose),
                     "");

        if (!m_store->contains<xAOD::ElectronContainer>(m_outContainerName +
                                                        systName)) {
          RETURN_CHECK("ElectronEfficiencyCorrector::execute()",
                       (HelperFunctions::makeDeepCopy<xAOD::ElectronContainer,
                                                      xAOD::ElectronAuxContainer,
                                                      xAOD::Electron>(m_store,
                                                                      m_outContainerName
                                                                      + systName,
                                                                      inputElectrons)),
                       "");
        ...
        RETURN_CHECK("ElectronEfficiencyCorrector::execute()",
                     HelperFunctions::retrieve(outputElectrons,
                                               m_outContainerName + systName,
                                               m_event, m_store, m_verbose),
                     "");

        if (m_debug) {
          Info("execute", "Number of electrons: %i",
               static_cast<int>(outputElectrons->size()));
          Info("execute", "Input syst: %s", systName.c_str());
          unsigned int idx(0);

          for (auto el : *(outputElectrons)) {
            Info("execute", "Input electron %i, pt = %.2f GeV ", idx,
                 (el->pt() * 1e-3));
            ++idx;
          }
        }

        // decorate electrons w/ SF - there will be a decoration w/ different
        // name for each syst!
        //
...
EL::StatusCode ElectronEfficiencyCorrector::executeSF(
  const xAOD::ElectronContainer *inputElectrons,
  unsigned int                   countSyst)
{
  // In the following, every electron gets decorated with several
  // vector<double>'s (for various SFs),
  //
  ...
  // Do it only if a tool with *this* name hasn't already been used, and has
  ...
  if (!m_corrFileNamePID.empty() && !isToolAlreadyUsed(m_pidEffSF_tool_name)) {
    ...
    for (const auto& syst_it : m_systListPID) {
      ...
      if (m_debug) Info("executeSF()",
                        "Electron PID efficiency sys name (to be recorded in xAOD::TStore) is: %s",
                        sfName.c_str());
      ...
      // apply syst
      //
      if (m_asgElEffCorrTool_elSF_PID->applySystematicVariation(syst_it) !=
          CP::SystematicCode::Ok) {
        Error("executeSF()",
              "Failed to configure AsgElectronEfficiencyCorrectionTool_PID for systematic %s",
              syst_it.name().c_str());
        return EL::StatusCode::FAILURE;
      }

      if (m_debug) Info("executeSF()",
                        "Successfully applied systematics: %s ",
                        m_asgElEffCorrTool_elSF_PID->appliedSystematics().name().c_str());
      ...
      for (auto el_itr : *(inputElectrons)) {
        ...
        // for the 1st systematic for *this* electron)
        //
        SG::AuxElement::Decorator<std::vector<float> > sfVecPID(
        ...
        // NB: derivations might remove CC and tracks for low pt electrons: add
        // a safety check!
        //
        if (!(el_itr->caloCluster() && el_itr->trackParticle())) {
          if (m_debug) Info("execute",
                            "Apply SF: skipping electron %i, it has no caloCluster or trackParticle info",
                            idx); isBadElectron = true;
        }
        ...
        if (el_itr->pt() < 15e3) {
          if (m_debug) Info("execute",
                            "Apply SF: skipping electron %i, is outside pT acceptance ( currently SF available for pT > 15 GeV )",
                            idx); isBadElectron = true;
        }

        if (fabs(el_itr->caloCluster()->eta()) > 2.47) {
          if (m_debug) Info("execute",
                            "Apply SF: skipping electron %i, is outside |eta| acceptance",
                            idx); isBadElectron = true;
        }
        ...
        if (!isBadElectron &&
            (m_asgElEffCorrTool_elSF_PID->getEfficiencyScaleFactor(*el_itr,
                                                                   pidEffSF) !=
        ...
        if (m_debug) {
          Info("executeSF()", "===>>>");
          Info("executeSF()", " ");
          Info("executeSF()", "Electron %i, pt = %.2f GeV ", idx,
               (el_itr->pt() * 1e-3));
          Info("executeSF()", " ");
          Info("executeSF()",
               "PID SF decoration: %s",
               m_outputSystNamesPID.c_str());
          Info("executeSF()", " ");
          Info("executeSF()",
               "Systematic: %s",
               syst_it.name().c_str());
    ...
    // event
    //
    if (countSyst == 0) RETURN_CHECK("ElectronEfficiencyCorrector::executeSF()",
                                     m_store->record(sysVariationNamesPID,
                                                     m_outputSystNamesPID),
                                     "Failed to record vector of systematic names PID");
  ...
  if (!m_corrFileNameIso.empty() && !isToolAlreadyUsed(m_IsoEffSF_tool_name)) {
    ...
    for (const auto& syst_it : m_systListIso) {
      ...
      if (m_debug) Info("executeSF()",
                        "Electron Iso efficiency sys name (to be recorded in xAOD::TStore) is: %s",
                        sfName.c_str());
      ...
      // apply syst
      //
      if (m_asgElEffCorrTool_elSF_Iso->applySystematicVariation(syst_it) !=
          CP::SystematicCode::Ok) {
        Error("executeSF()",
              "Failed to configure AsgElectronEfficiencyCorrectionTool_Iso for systematic %s",
              syst_it.name().c_str());
        return EL::StatusCode::FAILURE;
      }
      ...
      if (m_debug) Info("executeSF()",
                        "Successfully applied systematics: %s ",
                        m_asgElEffCorrTool_elSF_Iso->appliedSystematics().name().c_str());
      ...
      for (auto el_itr : *(inputElectrons)) {
        ...
        if (fabs(el_itr->caloCluster()->eta()) > 2.47) {
          if (m_debug) Info("execute", "Apply SF: skipping electron %i, is outside |eta| acceptance", idx); isBadElectron = true;
        }
  ...
  if (!m_corrFileNameReco.empty() && !isToolAlreadyUsed(m_RecoEffSF_tool_name)) {
    ...
    for (const auto& syst_it : m_systListReco) {
      ...
      for (auto el_itr : *(inputElectrons)) {
        ...
        //
        if (!(el_itr->caloCluster() && el_itr->trackParticle())) {
          if (m_debug) Info("execute", "Apply SF: skipping electron %i, it has no caloCluster or trackParticle info", idx); isBadElectron = true;
        }
        ...
        //
        if (el_itr->pt() < 15e3) {
          if (m_debug) Info("execute", "Apply SF: skipping electron %i, is outside pT acceptance ( currently SF available for pT > 15 GeV )", idx); isBadElectron = true;
        }
  ...
  if (!m_corrFileNameTrig.empty() && !isToolAlreadyUsed(m_TrigEffSF_tool_name)) {
    ...
    for (const auto& syst_it : m_systListTrig) {
      ...
      for (auto el_itr : *(inputElectrons)) {
        ...
        //
        // obtain Trigger efficiency SF as a float (to be stored away
        // separately)
        //
        //  If SF decoration vector doesn't exist, create it (will be done only
        // for the 1st systematic for *this* electron)
  ...
  if (!m_corrFileNameTrigMCEff.empty() && !isToolAlreadyUsed(m_TrigMCEff_tool_name)) {
    ...
    for (const auto& syst_it : m_systListTrigMCEff) {
      ...
      for (auto el_itr : *(inputElectrons)) {
        ...
        //
        // obtain Trigger MC efficiency as a float (to be stored away
        // separately)
        //
        //  If efficiency decoration vector doesn't exist, create it (will be
        // done only for the 1st systematic for *this* electron)
        //

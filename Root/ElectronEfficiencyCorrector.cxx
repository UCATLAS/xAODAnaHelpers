...
EL::StatusCode  ElectronEfficiencyCorrector :: configure ()
{
  ...
  if ( !getConfig().empty() ) {
    ...
    m_systValEff              = config->GetValue("SystValReco" , m_systValReco);
...
EL::StatusCode ElectronEfficiencyCorrector :: initialize ()
{
  ...
  std::vector<std::string> inputFiles{ m_corrFileNameReco } ; // initialise vector w/ all the files containing corrections
  RETURN_CHECK( "ElectronEfficiencyCorrector::initialize()", m_asgElEffCorrTool_elSF_Reco->setProperty("CorrectionFileNameList",inputFiles),"Failed to set property CorrectionFileNameList");
  ...
  std::vector<std::string> inputFiles{ m_corrFileNameTrig } ; // initialise vector w/ all the files containing corrections
  RETURN_CHECK( "ElectronEfficiencyCorrector::initialize()", m_asgElEffCorrTool_elSF_Trig->setProperty("CorrectionFileNameList",inputFiles),"Failed to set property CorrectionFileNameList");
  ...
  Info("initialize()"," Initializing Electron Efficiency Corrector Systematics :");
  m_systListTrg = HelperFunctions::getListofSystematics( recSystsTrig, m_systNameTrig, m_systValTrig );
  // Convert into a simple list
...
EL::StatusCode ElectronEfficiencyCorrector :: executeSF (  const xAOD::ElectronContainer* inputElectrons, unsigned int countSyst  )
{
  ...
  for ( const auto& syst_it : m_systList ) {
    ...
    std::string sfRecoName = "ElRecoEff_SF";
    std::string sfTrigName = "ElTrigEff_SF";
    ...
    if(m_debug) Info("execute()", "SF decoration names are: %s %s %s", sfNamePID.c_str(), sfNameReco.c_str(), sfNameTrig.c_str());
    ...
    if ( m_debug ) { Info("execute()", "Successfully applied systematics: %s %s %s", m_asgElEffCorrTool_elSF_PID->appliedSystematics().name().c_str() ,
                                                                                     m_asgElEffCorrTool_elSF_Reco->appliedSystematics().name().c_str(),
                                                                                     m_asgElEffCorrTool_elSF_Trig->appliedSystematics().name().c_str() ); }
    ...
    for ( auto el_itr : *(inputElectrons) ) {
       ...
       //
       double pidSF(0.0); double recoSF(0.0); double trigSF(0.0)
       ...
       sfVecReco( *el_itr ).push_back( pidReco );
       sfVecTrig( *el_itr ).push_back( pidTrig );

       if ( m_debug ) { 
          Info( "execute", "===>>> Resulting PID  SF: %f for systematic: %s ", pidSF , syst_it.name().c_str()); 
          Info( "execute", "===>>> Resulting Reco SF: %f for systematic: %s ", recoSF, syst_it.name().c_str()); 
          Info( "execute", "===>>> Resulting Trig SF: %f for systematic: %s ", trigSF, syst_it.name().c_str()); 
       }

...
EL::StatusCode TrackSelector::initialize()
{
  ...
  if (m_debug) Info("initialize()",
                    "TrackSelector Interface succesfully initialized!");
...
{
  if (m_debug) Info("execute()", "Applying Track Selection... %s",
                    m_name.c_str());
...
EL::StatusCode TrackSelector::executeTrackCollection()
{
  ...
  RETURN_CHECK("TrackSelector::execute()",
               HelperFunctions::retrieve(inTracks, m_inContainerName, m_event,
                                         m_store,
                                         m_verbose), "");
  ...
  if (m_createSelectedContainer) {
    selectedTracks = new ConstDataVector<xAOD::TrackParticleContainer>(
      SG::VIEW_ELEMENTS);
  }
...
EL::StatusCode TrackSelector::executeTracksInJets()
{
  if (m_debug) Info("execute()",
                    "Applying TracksInJet Selection... %s",
                    m_inJetContainerName.c_str());
  m_numEvent++;

  // get input jet collection
  const xAOD::JetContainer *inJets(nullptr);
  RETURN_CHECK("JetSelector::execute()",
               HelperFunctions::retrieve(inJets, m_inJetContainerName, m_event,
                                         m_store,
                                         m_verbose), "");
  ...
  //
  xAOD::Jet::Decorator<vector<const xAOD::TrackParticle *> > m_track_decoration(
    m_outContainerName.c_str());
  xAOD::Jet::Decorator<const xAOD::Vertex *> m_vtx_decoration(
    (m_outContainerName + "_vtx").c_str());
  ...
  for (auto jet_itr : *inJets) {
    ...
    //
    const vector<const xAOD::TrackParticle *> inputTracks =
      jet_itr->auxdata<vector<const xAOD::TrackParticle *>  >(m_inContainerName);
    const xAOD::Vertex *pvx =
      jet_itr->auxdata<const xAOD::Vertex *>(m_inContainerName + "_vtx");
    ...
    m_vtx_decoration(*jet_itr)   = jet_itr->auxdata<const xAOD::Vertex *>(
      m_inContainerName + "_vtx");
  } // jets

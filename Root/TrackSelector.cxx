...
EL::StatusCode TrackSelector::executeTrackCollection() {
    ...
    return EL::StatusCode::SUCCESS;
} // TrackSelector::executeTrackCollection
...
EL::StatusCode TrackSelector::executeTracksInJets() {
    ...
    m_numEventPass++;
    return EL::StatusCode::SUCCESS;
} // TrackSelector::executeTracksInJets
...
int TrackSelector::PassCuts(const xAOD::TrackParticle* trk, const xAOD::Vertex* pvx) {
    // pT
    if(m_pT_max != 1e8)
        if(trk->pt() > m_pT_max) return 0; if(m_pT_min != 1e8)
        if(trk->pt() < m_pT_min) return 0;

    // eta
    if(m_eta_max != 1e8)
        if(trk->eta() > m_eta_max) return 0; if(m_eta_min != 1e8)

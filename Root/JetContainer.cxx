...
void JetContainer::setTree(TTree* tree, const std::string& tagger) {
    ...
    for(uint i = 0; i < m_infoSwitch.m_sfFTagFix.size(); i++) {
        switch(m_infoSwitch.m_sfFTagFix[i]) {
            case 30: m_btag_Fix30->setTree(tree, m_name);
                break;

            case 50: m_btag_Fix50->setTree(tree, m_name);
                break;

            case 60: m_btag_Fix60->setTree(tree, m_name);
                break;

            case 70: m_btag_Fix70->setTree(tree, m_name);
                break;

            case 77: m_btag_Fix77->setTree(tree, m_name);
                break;

            case 80: m_btag_Fix80->setTree(tree, m_name);
                break;

            case 85: m_btag_Fix85->setTree(tree, m_name);
                break;

            case 90: m_btag_Fix90->setTree(tree, m_name);
                break;
        }
    }
    ...
    for(uint i = 0; i < m_infoSwitch.m_sfFTagFlt.size(); i++) {
        switch(m_infoSwitch.m_sfFTagFlt[i]) {
            case 30: m_btag_Flt30->setTree(tree, m_name);
                break;

            case 50: m_btag_Flt50->setTree(tree, m_name);
                break;

            case 60: m_btag_Flt60->setTree(tree, m_name);
                break;

            case 70: m_btag_Flt70->setTree(tree, m_name);
                break;

            case 77: m_btag_Flt77->setTree(tree, m_name);
                break;

            case 85: m_btag_Flt85->setTree(tree, m_name);
                break;
        }
    }
...
void JetContainer::FillJet(const xAOD::IParticle* particle, const xAOD::Vertex* pv, int pvLocation) {
    ...
    if(m_infoSwitch.m_scales) {
        ...
        if(status) m_emScalePt->push_back(fourVec.Pt() / m_units); else m_emScalePt->push_back(-999);  // Constit Scale
        status = jet->getAttribute<xAOD::JetFourMom_t>("JetConstitScaleMomentum", fourVec);
        if(status) m_constScalePt->push_back(fourVec.Pt() / m_units); else m_constScalePt->push_back(-999);  // Pileup Scale
        status = jet->getAttribute<xAOD::JetFourMom_t>("JetPileupScaleMomentum", fourVec);
        if(status) m_pileupScalePt->push_back(fourVec.Pt() / m_units); else m_pileupScalePt->push_back(-999);  // OriginConstit Scale
        status = jet->getAttribute<xAOD::JetFourMom_t>("JetOriginConstitScaleMomentum", fourVec);
        if(status) m_originConstitScalePt->push_back(fourVec.Pt() / m_units); else m_originConstitScalePt->push_back(-999);  // EtaJES Scale
        status = jet->getAttribute<xAOD::JetFourMom_t>("JetEtaJESScaleMomentum", fourVec);
        if(status) m_etaJESScalePt->push_back(fourVec.Pt() / m_units); else m_etaJESScalePt->push_back(-999);  // GSC Scale
        status = jet->getAttribute<xAOD::JetFourMom_t>("JetGSCScaleMomentum", fourVec);
        if(status) m_gscScalePt->push_back(fourVec.Pt() / m_units); else m_gscScalePt->push_back(-999);  // only available in data
    ...
    if(m_infoSwitch.m_trackAll || m_infoSwitch.m_trackPV) {
        ...
        if( m_infoSwitch.m_trackPV && (pvLocation >= 0) ) {
            ...
            if(m_mc) {
                ...
                if( jvtSF_Loose.isAvailable(*jet) ) m_JvtEff_SF_Loose->push_back( jvtSF_Loose(*jet) ); else m_JvtEff_SF_Loose->push_back(junkSF); if( jvtSF_Medium.isAvailable(*jet) ) m_JvtEff_SF_Medium->push_back( jvtSF_Medium(*jet) ); else m_JvtEff_SF_Medium->push_back(junkSF); if( jvtSF_Tight.isAvailable(*jet) ) m_JvtEff_SF_Tight->push_back( jvtSF_Tight(*jet) ); else m_JvtEff_SF_Tight->push_back(junkSF); }
    ...
    if(m_infoSwitch.m_flavTag || m_infoSwitch.m_flavTagHLT) {
        ...
        if(m_infoSwitch.m_svDetails) {
            ...
            double sv1_pu = -30;
            myBTag->variable<double>("SV1", "pu", sv1_pu);
            double sv1_pb = -30;
            myBTag->variable<double>("SV1", "pb", sv1_pb);
            double sv1_pc = -30;
            myBTag->variable<double>("SV1", "pc", sv1_pc);
            ...
            float sv1_Lxy;
            myBTag->variable<float>("SV1", "Lxy",         sv1_Lxy);
            float sv1_L3d;
            myBTag->variable<float>("SV1", "L3d",         sv1_L3d);
            float sv1_distmatlay;
            myBTag->variable<float>("SV1", "dstToMatLay", sv1_distmatlay);
            float sv1_dR;
            myBTag->variable<float>("SV1", "deltaR",      sv1_dR);
        ...
        if(m_infoSwitch.m_ipDetails) {
            ...
            double ip2_pu = -99;
            myBTag->variable<double>("IP2D", "pu", ip2_pu);
            double ip2_pb = -99;
            myBTag->variable<double>("IP2D", "pb", ip2_pb);
            double ip2_pc = -99;
            myBTag->variable<double>("IP2D", "pc", ip2_pc);
            ...
            double ip3_pu = -30;
            myBTag->variable<double>("IP3D", "pu", ip3_pu);
            double ip3_pb = -30;
            myBTag->variable<double>("IP3D", "pb", ip3_pb);
            double ip3_pc = -30;
            myBTag->variable<double>("IP3D", "pc", ip3_pc);
    ...
    if( !m_infoSwitch.m_sfFTagFix.empty() ) {
        for(unsigned int i = 0; i < m_infoSwitch.m_sfFTagFix.size(); i++) {
            switch( m_infoSwitch.m_sfFTagFix.at(i) ) {
                case 30: m_btag_Fix30->Fill(jet);
                    break;

                case 50: m_btag_Fix50->Fill(jet);
                    break;

                case 60: m_btag_Fix60->Fill(jet);
                    break;

                case 70: m_btag_Fix70->Fill(jet);
                    break;

                case 77: m_btag_Fix77->Fill(jet);
                    break;

                case 80: m_btag_Fix80->Fill(jet);
                    break;

                case 85: m_btag_Fix85->Fill(jet);
                    break;

                case 90: m_btag_Fix90->Fill(jet);
                    break;
            }
        }
    } // sfFTagFix
    ...
    if( !m_infoSwitch.m_sfFTagFlt.empty() ) {
        for(unsigned int i = 0; i < m_infoSwitch.m_sfFTagFlt.size(); i++) {
            switch( m_infoSwitch.m_sfFTagFlt.at(i) ) {
                case 30: m_btag_Flt30->Fill(jet);
                    break;

                case 50: m_btag_Flt50->Fill(jet);
                    break;

                case 60: m_btag_Flt60->Fill(jet);
                    break;

                case 70: m_btag_Flt70->Fill(jet);
                    break;

                case 77: m_btag_Flt77->Fill(jet);
                    break;

                case 85: m_btag_Flt85->Fill(jet);
                    break;
            }
        }
    } // sfFTagFlt
...
} // JetContainer::FillJet

void JetContainer::FillGlobalBTagSF(const xAOD::EventInfo* eventInfo) {
    if( !m_infoSwitch.m_sfFTagFix.empty() ) {
        for(unsigned int i = 0; i < m_infoSwitch.m_sfFTagFix.size(); i++) {
            switch( m_infoSwitch.m_sfFTagFix.at(i) ) {
                case 30:  m_btag_Fix30->FillGlobalSF(eventInfo);
                    break;

                case 50:  m_btag_Fix50->FillGlobalSF(eventInfo);
                    break;

                case 60:  m_btag_Fix60->FillGlobalSF(eventInfo);
                    break;

                case 70:  m_btag_Fix70->FillGlobalSF(eventInfo);
                    break;

                case 77:  m_btag_Fix77->FillGlobalSF(eventInfo);
                    break;

                case 80:  m_btag_Fix80->FillGlobalSF(eventInfo);
                    break;

                case 85:  m_btag_Fix85->FillGlobalSF(eventInfo);
                    break;

                case 90:  m_btag_Fix90->FillGlobalSF(eventInfo);
                    break;
            }
        }
    } // sfFTagFix
    ...
    if( !m_infoSwitch.m_sfFTagFlt.empty() ) {
        for(unsigned int i = 0; i < m_infoSwitch.m_sfFTagFlt.size(); i++) {
            switch( m_infoSwitch.m_sfFTagFlt.at(i) ) {
                case 30:  m_btag_Flt30->FillGlobalSF(eventInfo);
                    break;

                case 50:  m_btag_Flt50->FillGlobalSF(eventInfo);
                    break;

                case 60:  m_btag_Flt60->FillGlobalSF(eventInfo);
                    break;

                case 70:  m_btag_Flt70->FillGlobalSF(eventInfo);
                    break;

                case 77:  m_btag_Flt77->FillGlobalSF(eventInfo);
                    break;

                case 85:  m_btag_Flt85->FillGlobalSF(eventInfo);
                    break;
            }
        }
    } // sfFTagFlt
}     // JetContainer::FillGlobalBTagSF

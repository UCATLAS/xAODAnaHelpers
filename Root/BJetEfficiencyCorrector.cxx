  ...
  m_BJetEffSFTool(nullptr)
{
  ...
  m_corrFileName = PathResolverFindCalibFile(
  ...
  // For the fixed cut, valid options are: [ "FixedCutBEff_30",
  // "FixedCutBEff_50", "FixedCutBEff_60", "FixedCutBEff_70", "FixedCutBEff_77",
  // "FixedCutBEff_80", "FixedCutBEff_85", "FixedCutBEff_90" ]
  // For the variable cut, valid options are: [ "FlatBEff_30", "FlatBEff_40",
  // "FlatBEff_50", "FlatBEff_60", "FlatBEff_70", "FlatBEff_77", "FlatBEff_85" ]

  // Btag quality
  m_operatingPt     = "FixedCutBEff_70";
  m_operatingPtCDI  = "";
  m_getScaleFactors = false; // will only get scale factors for calibrated
                             // working points
...
EL::StatusCode BJetEfficiencyCorrector::setupJob(EL::Job& job)
{
  ...
  job.useXAOD();
  xAOD::Init("BJetEfficiencyCorrector").ignore(); // call before opening first
                                                  // file
...
EL::StatusCode BJetEfficiencyCorrector::initialize()
{
  ...
  RETURN_CHECK("BJetEfficiencyCorrector::initialize()",
  ...
  if (m_operatingPt == "FixedCutBEff_90") allOK = true;  // these are the only
                                                         // calibrated working
                                                         // points

  if (m_operatingPt == "FixedCutBEff_60") { allOK             = true;
                                            m_getScaleFactors =  true; }
  ...
  /*
     ...
     std::cout << "Get OperatingPoint for CDI BTag Efficiency using eff = " <<
        m_operatingPtCDI << std::endl;
     m_operatingPtCDI = HelperFunctions::GetBTagMV2c20_CutStr( atoi(
        m_operatingPtCDI.c_str() ) );
     }
   */
  ...
  if (m_getScaleFactors) {
    ...
    m_systList = CP::make_systematics_vector(recSysts); // comes back with 1
                                                        // entry for nominal
  }
                                                                    ...
                                                                    bool                      doNominal)
{
  ...
  //
  // Define also an *event* weight, which is the product of all the BTag eff.
  // SFs for each object in the event
  //
  ...
  for (const auto& syst_it : m_systList) {
    ...
    //
    // if not running systematics, only compulte weight for specified systematic
    // (m_systName)
    ...
    for (auto jet_itr : *(inJets)) {
      ...
      // if only decorator with decision because OP is not calibrated, set SF to
      // 1
      if (m_getScaleFactors && (fabs(jet_itr->eta()) < 2.5)) {
        ...
        // if it is out of validity range (jet pt > 1200 GeV), the tools just
        // applies the SF at 200 GeV
      ...
      /*
         ...
         if( (fabs(jet_itr->eta()) < 2.5) &&
            m_BJetEffSFTool->getEfficiency( *jet_itr, eff ) !=

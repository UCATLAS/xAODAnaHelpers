...
EL::StatusCode PhotonCalibrator::execute() {
    ...
    for(const auto& syst_it : m_systList) {
        if(m_debug) Info( "execute()", "Systematic Loop for m_systList=%s", syst_it.name().c_str() );  // discard photon systematics
        //
        ...
        if(m_debug) Info("execute()", "Systematics applied");  // create shallow copy for calibration - one per syst
        //
...
EL::StatusCode PhotonCalibrator::decorate(xAOD::Photon* photon) {
    ...
    if(m_isMC) {
        ...
        if(m_debug) {
            Info("decorate()", "Tight=%f (%f) Medium=%f (%f) Loose=%f (%f)",
...
EL::StatusCode PhotonCalibrator::toolInitializationAtTheFirstEvent(const xAOD::EventInfo* eventInfo) {
    ...
    return EL::StatusCode::SUCCESS;
} // PhotonCalibrator::toolInitializationAtTheFirstEvent

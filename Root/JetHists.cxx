...
StatusCode JetHists::execute(const xAOD::IParticle* particle, float eventWeight, const xAOD::EventInfo* eventInfo) {
    ...
    if(m_infoSwitch->m_flavTag || m_infoSwitch->m_flavTagHLT) {
        ...
        if(m_infoSwitch->m_btag_jettrk) {
            ...
            if(trksOK) {
                ...
                int sv1_ntkv;
                btag_info->variable<int>(  "SV1", "NGTinSvx", sv1_ntkv);
                float sv1_efrc;
                btag_info->variable<float>("SV1", "efracsvx", sv1_efrc);
                float sv_scaled_efc = sv1_ntkv > 0 ? sv1_efrc * (static_cast<float>(ntrks) / sv1_ntkv) : -1;

                int jf_ntrkv;
                btag_info->variable<int>(  "JetFitter", "nTracksAtVtx",   jf_ntrkv);
                int jf_nvtx1t;
                btag_info->variable<int>(  "JetFitter", "nSingleTracks",  jf_nvtx1t);
                float jf_efrc;
                btag_info->variable<float>("JetFitter", "energyFraction", jf_efrc);
        ...
        if(m_infoSwitch->m_jetFitterDetails) {
            ...
            float jf_mass_unco;
            btag_info->variable<float>("JetFitter", "massUncorr",  jf_mass_unco);
            float jf_dR_flight;
            btag_info->variable<float>("JetFitter", "dRFlightDir", jf_dR_flight);
        ...
        if(m_infoSwitch->m_svDetails) {
            ...
            double sv1_pu = -30;
            btag_info->variable<double>("SV1", "pu", sv1_pu);
            double sv1_pb = -30;
            btag_info->variable<double>("SV1", "pb", sv1_pb);
            double sv1_pc = -30;
            btag_info->variable<double>("SV1", "pc", sv1_pc);
            ...
            float sv1_Lxy;
            btag_info->variable<float>("SV1", "Lxy",         sv1_Lxy);
            float sv1_L3d;
            btag_info->variable<float>("SV1", "L3d",         sv1_L3d);
            float sv1_distmatlay;
            btag_info->variable<float>("SV1", "dstToMatLay", sv1_distmatlay);
            float sv1_dR;
            btag_info->variable<float>("SV1", "deltaR",      sv1_dR);
        ...
        if(m_infoSwitch->m_ipDetails) {
            ...
            double ip2_pu = -30;
            btag_info->variable<double>("IP2D", "pu", ip2_pu);
            double ip2_pb = -30;
            btag_info->variable<double>("IP2D", "pb", ip2_pb);
            double ip2_pc = -30;
            btag_info->variable<double>("IP2D", "pc", ip2_pc);
            ...
            double ip3_pu = -30;
            btag_info->variable<double>("IP3D", "pu", ip3_pu);
            double ip3_pb = -30;
            btag_info->variable<double>("IP3D", "pb", ip3_pb);
            double ip3_pc = -30;
            btag_info->variable<double>("IP3D", "pc", ip3_pc);
    ...
    if(m_infoSwitch->m_byEta) {
        if(fabs( jet->eta() ) < 1) {m_jetPt_eta_0_1   -> Fill(jet->pt() / 1e3, eventWeight);} else if(fabs( jet->eta() ) < 2) {
            m_jetPt_eta_1_2   -> Fill(jet->pt() / 1e3, eventWeight);
            m_jetPt_eta_1_2p5 -> Fill(jet->pt() / 1e3, eventWeight);
        } else if(fabs( jet->eta() ) < 2.5) {
            m_jetPt_eta_2_2p5 -> Fill(jet->pt() / 1e3, eventWeight);
            m_jetPt_eta_1_2p5 -> Fill(jet->pt() / 1e3, eventWeight);
        }
    }
    ...
    if(m_infoSwitch->m_onlineBS) {
        ...
        if(fabs(bs_online_vz) < 1) m_eta_bs_online_vz_0_1   -> Fill(jet->eta(), eventWeight); else if(fabs(bs_online_vz) < 1.5) m_eta_bs_online_vz_1_1p5 -> Fill(jet->eta(), eventWeight);
...
StatusCode JetHists::execute(const xAH::Particle* particle, float eventWeight, const xAH::EventInfo* eventInfo) {
    ...
    if(m_infoSwitch->m_byEta) {
        if(fabs( jet->p4.Eta() ) < 1) {m_jetPt_eta_0_1   -> Fill(jet->p4.Pt(), eventWeight);} else if(fabs( jet->p4.Eta() ) < 2) {
            m_jetPt_eta_1_2   -> Fill(jet->p4.Pt(), eventWeight);
            m_jetPt_eta_1_2p5 -> Fill(jet->p4.Pt(), eventWeight);
        } else if(fabs( jet->p4.Eta() ) < 2.5) {
            m_jetPt_eta_2_2p5 -> Fill(jet->p4.Pt(), eventWeight);
            m_jetPt_eta_1_2p5 -> Fill(jet->p4.Pt(), eventWeight);
        }
    }
    ...
    if(m_infoSwitch->m_onlineBS) {
        ...
        if(fabs(bs_online_vz) < 1) m_eta_bs_online_vz_0_1   -> Fill(jet->p4.Eta(), eventWeight); else if(fabs(bs_online_vz) < 1.5) m_eta_bs_online_vz_1_1p5 -> Fill(jet->p4.Eta(), eventWeight);

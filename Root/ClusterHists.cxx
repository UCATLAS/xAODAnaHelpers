...
StatusCode ClusterHists::initialize() {
  ...
  // 2D plots
  m_ccl_eta_vs_phi = book(m_name, "eta_vs_phi", "cluster #phi", 120,
                          -TMath::Pi(), TMath::Pi(), "cluster #eta", 80, -4, 4);
  m_ccl_e_vs_eta = book(m_name,
                        "e_vs_eta",
                        "cluster #eta",
                        80,
                        -4,
                        ...
                        -5,
                        15);
  m_ccl_e_vs_phi = book(m_name, "e_vs_phi", "cluster #phi", 120,
                        -TMath::Pi(),
                        TMath::Pi(), "cluster e [GeV]", 100, -5, 15);

  // if worker is passed to the class add histograms to the output
  return StatusCode::SUCCESS;
}

StatusCode ClusterHists::execute(const xAOD::CaloClusterContainer *ccls,
                                 float                             eventWeight) {
  ...
  for (; ccl_itr != ccl_end; ++ccl_itr) {
    RETURN_CHECK("ClusterHists::execute()", this->execute((*ccl_itr),
                                                          eventWeight), "");
  }
...
StatusCode ClusterHists::execute(const xAOD::CaloCluster *ccl,
                                 float                    eventWeight) {
  // basic

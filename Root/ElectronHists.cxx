...
StatusCode ElectronHists::execute(const xAOD::Electron  *electron,
                                  float                  eventWeight,
                                  const xAOD::EventInfo *eventInfo) {
  return execute(static_cast<const xAOD::IParticle *>(electron),
                 eventWeight,
                 eventInfo);
}

StatusCode ElectronHists::execute(const xAOD::IParticle *particle,
                                  float                  eventWeight,
                                  const xAOD::EventInfo *eventInfo) {
  ...
  if (electron == 0)
  {
    ::Error("ElectronHists::execute()",
            XAOD_MESSAGE("Cannot convert IParticle to Electron"));
  ...
  if (m_infoSwitch->m_isolation) {
    ...
    if (isIsoLooseTrackOnlyAcc.isAvailable(*electron)) m_isIsolated_LooseTrackOnly
      ->Fill(isIsoLooseTrackOnlyAcc(*electron), eventWeight);
    else m_isIsolated_LooseTrackOnly->Fill(-1, eventWeight);

    if (isIsoLooseAcc.isAvailable(*electron)) m_isIsolated_Loose->Fill(isIsoLooseAcc(
                                                                         *electron),
                                                                       eventWeight);
    ...
    if (isIsoTightAcc.isAvailable(*electron)) m_isIsolated_Tight->Fill(isIsoTightAcc(
                                                                         *electron),
                                                                       eventWeight);
    ...
    if (isIsoGradientAcc.isAvailable(*electron)) m_isIsolated_Gradient->Fill(isIsoGradientAcc(
                                                                               *
                                                                               electron),
                                                                             eventWeight);
    ...
    if (isIsoGradientLooseAcc.isAvailable(*electron)) m_isIsolated_GradientLoose->
      Fill(isIsoGradientLooseAcc(*electron), eventWeight);
    else m_isIsolated_GradientLoose->Fill(-1,  eventWeight);

    if (isIsoFixedCutLooseAcc.isAvailable(*electron)) m_isIsolated_FixedCutLoose->
      Fill(isIsoFixedCutLooseAcc(*electron), eventWeight);
    else m_isIsolated_FixedCutLoose->Fill(-1,  eventWeight);

    if (isIsoFixedCutTightAcc.isAvailable(*electron)) m_isIsolated_FixedCutTight->
      Fill(isIsoFixedCutTightAcc(*electron), eventWeight);
    else m_isIsolated_FixedCutTight->Fill(-1,  eventWeight);

    if (isIsoFixedCutTightTrackOnlyAcc.isAvailable(*electron))
      m_isIsolated_FixedCutTightTrackOnly->Fill(isIsoFixedCutTightTrackOnlyAcc(*
                                                                                                                                        electron),
      eventWeight); else m_isIsolated_FixedCutTightTrackOnly->Fill(-1,
      eventWeight);
...
StatusCode ElectronHists::execute(const xAH::Particle *particle, float eventWeight, const xAH::EventInfo * /*eventInfo*/) {
  ...
  // elec->isIsolated_GradientLoose ,  eventWeight );
  // m_isIsolated_GradientT2               ->Fill( elec->isIsoGradientT2Acc ,
  //  eventWeight );
  // m_isIsolated_MU0p06                   ->Fill( elec->isIsoMU0p06Acc ,
  //  eventWeight );
  m_isIsolated_FixedCutLoose->Fill(elec->isIsolated_FixedCutLoose, eventWeight);

  // m_isIsolated_FixedCutTight            ->Fill(
  // elec->isIsolated_FixedCutTight ,  eventWeight );

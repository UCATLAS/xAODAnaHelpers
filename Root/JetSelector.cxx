  ...
  m_JVT_tool_handle("CP::JetJvtEfficiency/JVTToolName", nullptr)
{
  ...
  m_doBTagCut    = false;
  m_corrFileName =
    "$ROOTCOREBIN/data/xAODBTaggingEfficiency/cutprofiles_22072015.root";
  m_taggerName        = "MV2c20";
...
EL::StatusCode JetSelector::initialize()
{
  ...
  RETURN_CHECK("JetSelector::initialize()",
  ...
  if (m_useCutFlow) {
    ...
    m_jet_cutflow_ptmax_cut = m_jet_cutflowHist_1->GetXaxis()->FindBin(
      "ptmax_cut");
    m_jet_cutflow_ptmin_cut = m_jet_cutflowHist_1->GetXaxis()->FindBin(
      "ptmin_cut");
    m_jet_cutflow_eta_cut = m_jet_cutflowHist_1->GetXaxis()->FindBin(
      "eta_cut");
    m_jet_cutflow_jvt_cut = m_jet_cutflowHist_1->GetXaxis()->FindBin(
      "JVT_cut");
    m_jet_cutflow_btag_cut = m_jet_cutflowHist_1->GetXaxis()->FindBin(
      "BTag_cut");
    m_jet_cutflow_cleaning_cut = m_jet_cutflowHist_1->GetXaxis()->FindBin(
      "cleaning_cut");
  }
  ...
  if (!allOK) {
    Error("initialize()",
          "Requested operating point is not known to xAH. Arrow v Indian? %s",
          m_operatingPt.c_str());
    return EL::StatusCode::FAILURE;
  }
  ...
  if (m_doBTagCut) {
    ...
    // is there a reason to have this configurable here??...I think no (GF to
    // self)
    //
    RETURN_CHECK("JetSelector::initialize()",
                 m_BJetSelectTool->setProperty("MaxEta",
                                               m_b_eta_max),
                 "Failed to set property:MaxEta");
    RETURN_CHECK("JetSelector::initialize()",
                 m_BJetSelectTool->setProperty("MinPt",
                                               m_b_pt_min),
                 "Failed to set property:MinPt");
    RETURN_CHECK("JetSelector::initialize()",
                 m_BJetSelectTool->setProperty("FlvTagCutDefinitionsFileName",
                                               m_corrFileName),
                 "Failed to set property:FlvTagCutDefinitionsFileName");

    // configurable parameters
    //
    RETURN_CHECK("JetSelector::initialize()",
                 m_BJetSelectTool->setProperty("TaggerName",
                                               m_taggerName),
                 "Failed to set property: TaggerName");
    RETURN_CHECK("JetSelector::initialize()",
                 m_BJetSelectTool->setProperty("OperatingPoint",
                                               m_operatingPt),
                 "Failed to set property: OperatingPoint");
    RETURN_CHECK("JetSelector::initialize()",
                 m_BJetSelectTool->setProperty("JetAuthor",
                                               m_jetAuthor),
                 "Failed to set property: JetAuthor");
    RETURN_CHECK("JetSelector::initialize()",
                 m_BJetSelectTool->initialize(),
                 "Failed to properly initialize the BJetSelectionTool");
  }

  // init fJVT
  m_fJVT_tool_handle.setTypeAndName("JetForwardJvtTool/fJvt");
  RETURN_CHECK("JetSelector::initialize()",
               m_fJVT_tool_handle.retrieve(),
               "Failed to retrieve CP::JetForwardJVTtool");
  ...
  RETURN_CHECK("ElectronEfficiencyCorrector::initialize()",
               checkToolStore<CP::IJetJvtEfficiency>(m_JVT_tool_name), "");

  m_JVT_tool_handle.setTypeAndName("CP::JetJvtEfficiency/" + m_JVT_tool_name);

  if (!m_JVT_tool_handle.isUserConfigured()) {
    RETURN_CHECK("JetSelector::initialize()",
                 ASG_MAKE_ANA_TOOL(m_JVT_tool_handle,
                                   CP::JetJvtEfficiency),
                 "Could not make JetJetEfficiency");
    RETURN_CHECK("JetSelector::initialize()",
                 m_JVT_tool_handle.setProperty("WorkingPoint",
                                               m_WorkingPointJVT),
                 "Failed to set Working Point property of JetJvtEfficiency for JVT");
    RETURN_CHECK("JetSelector::initialize()",
                 m_JVT_tool_handle.setProperty("SFFile",
                                               m_SFFileJVT),
                 "Failed to set SFFile property of JetJvtEfficiency for JVT");
    RETURN_CHECK("JetSelector::initialize()",
                 m_JVT_tool_handle.retrieve(),
                 "Failed to retrieve CP::JetJvtEfficiency");
  }
  ...
  if (m_debug) {
    ...
    for (const auto& syst_it : affectSystsJVT) Info("initialize()",
                                                    "JetJvtEfficiency tool can be affected by JVT efficiency systematic: %s",
                                                    (syst_it.name()).c_str());
  }
  ...
  //
  const CP::SystematicSet recSystsJVT =
    m_JVT_tool_handle->recommendedSystematics();
  m_systListJVT = HelperFunctions::getListofSystematics(recSystsJVT,
                                                        m_systNameJVT,
                                                        m_systValJVT,
                                                        m_debug);

  Info("initialize()",
       "Will be using JetJvtEfficiency tool JVT efficiency systematic:");
  ...
  if (m_debug) Info("initialize()",
                    "JetSelector Interface succesfully initialized!");
...
EL::StatusCode JetSelector::execute()
{
  ...
  // retrieve event
  const xAOD::EventInfo *eventInfo(nullptr);
  RETURN_CHECK("JetSelector::execute()",
               HelperFunctions::retrieve(eventInfo, m_eventInfoContainerName,
                                         m_event, m_store,
                                         m_verbose), "");
  ...
  bool pass(false);
  bool count(true); // count for the 1st collection in the container - could be
                    // better as
  ...
  if (m_isMC && m_doJVT) RETURN_CHECK("JetSelector::execute()",
                                      HelperFunctions::retrieve(truthJets,
                                                                m_truthJetContainer,
                                                                m_event, m_store,
                                                                m_verbose),
                                      "");
  ...
  if (m_inputAlgo.empty()) {
    ...
    RETURN_CHECK("JetSelector::execute()",
                 HelperFunctions::retrieve(inJets, m_inContainerName, m_event,
                                           m_store,
                                           m_verbose), "");
    ...
    if (m_isMC && m_doJVT) {
      ...
      for (const auto& jet : *inJets) {
        ...
        for (const auto& tjet : *truthJets) {
          if ((tjet->p4().DeltaR(jet->p4()) < 0.3) &&
              (tjet->pt() > 10e3)) ishs = true;
    ...
    RETURN_CHECK("JetSelector::execute()",
                 HelperFunctions::retrieve(systNames, m_inputAlgo, 0, m_store,
                                           m_verbose), "");
    ...
    for (auto systName : *systNames) {
      RETURN_CHECK("JetSelector::execute()",
                   HelperFunctions::retrieve(inJets, m_inContainerName + systName,
                                             m_event,
                                             m_store, m_verbose), "");
      ...
      passOne = executeSelection(inJets,
                                 mcEvtWeight,
                                 count,
...
{
  if (m_debug) Info("executeSelection()",
                    "in executeSelection... %s",
                    m_name.c_str());
  ...
  if (m_doJVF) {
    ...
    RETURN_CHECK("JetSelector::execute()",
                 HelperFunctions::retrieve(vertices, "PrimaryVertices", m_event,
                                           m_store,
                                           m_verbose), "");
  ...
  if (m_dofJVT) {
    // have to make a deep copy because the fJVT tool wants to modify the jet
    // containers.
    RETURN_CHECK("execute()",
                 (HelperFunctions::makeDeepCopy<xAOD::JetContainer,
    ...
    RETURN_CHECK("execute()",
                 HelperFunctions::retrieve(jets_copy,
                                           m_inContainerName + "Copy",
                                           m_event,
                                           m_store),
                 "Couldn't retrieve jets copy from TStore");
  ...
  for (auto jet_itr : *jetsForSelection) { // duplicated of basic loop
    ...
    if (m_cleanJets && passSel && isCleanAcc.isAvailable(*jet_itr)) {
      if (!isCleanAcc(*jet_itr)) {
        ...
        if (m_decorateSelectedObjects) passSelDecor(*jet_itr) = passSel;

        // If any of the passing jets fail the recommendation is to remove the
        // jet (and MET is wrong)
        // If any of the N leading jets are not clean the event should be
        // removed
        if (m_cleanEvent || (nObj <= m_cleanEvtLeadJets)) {
          passEventClean = false;

          if (m_debug) Info("executeSelection()",
                            "Remove event due to bad jet with pt %f",
                            jet_itr->pt());
        } // if cleaning the event
      }   // if jet is not clean
    }     // if jet clean aux missing

    if (m_useCutFlow && passSel) m_jet_cutflowHist_1->Fill(
        m_jet_cutflow_cleaning_cut,
        1);


    if (passSel) {
      if (m_debug) Info("executeSelection()", "passSel"); nPass++;
  ...
  if (m_isMC && m_doJVT) {
    ...
    if (!isToolAlreadyUsed(m_JVT_tool_name)) {
      for (const auto& syst_it : m_systListJVT) {
        ...
        if (m_debug) Info("executeSelection()",
          "JVT SF sys name (to be recorded in xAOD::TStore) is: %s",
          sfName.c_str()); sysVariationNamesJVT->push_back(sfName);

        // apply syst
        //
        if (m_JVT_tool_handle->applySystematicVariation(syst_it) !=
        CP::SystematicCode::Ok) {
          Error("executeSelection()",
          "Failed to configure CP::JetJvtEfficiency for systematic %s",
          syst_it.name().c_str());
          return EL::StatusCode::FAILURE;
        }
        ...
        for (auto jet : *(selectedJets)) {
          ...
          // only for the 1st systematic for *this* jet)
          //
    ...
    // TStore!
    // This will be the case when this executeSelection() function gets called
    // for every syst varied input container,
  ...
  if (m_debug) Info("executeSelection()", "leave executeSelection... "); return true;
}
...
int JetSelector::PassCuts(const xAOD::Jet *jet) {
  ...
  if (m_doJVT) {
    ...
    if (m_JVTCut > 0) {
      if (jet->pt() < m_pt_max_JVT) {
        if (m_debug) Info("PassCuts()", "Pass JVT-pT Cut"); xAOD::JetFourMom_t jetScaleP4 = jet->getAttribute<xAOD::JetFourMom_t>(m_jetScaleType.c_str());
        ...
        if (fabs(jetScaleP4.eta()) < m_eta_max_JVT) {
          ...
          if (jet->getAttribute<float>("Jvt") < m_JVTCut) {
            if (m_debug) Info("passCuts()", " upper JVTCut is %2f - cutting this jet!!", m_JVTCut); return 0;
          } else if (m_debug) Info("passCuts()", " upper JVTCut is %2f - jet passes JVT ", m_JVTCut);
        }
      }
    } else if (!m_JVT_tool_handle->passesJvtCut(*jet)) return 0;
  } // m_doJVT
  ...
  //
  if (m_truthLabel != -1) {
    if (m_debug) Info("PassCuts()", "Doing Truth Label"); int this_TruthLabel = 0;
  ...
  if (m_debug) Info("PassCuts()", "Passed Cuts"); return 1;
}

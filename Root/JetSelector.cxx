...
EL::StatusCode JetSelector::initialize() {
    ...
    bool allOK(true);
    if(!m_operatingPt.empty() || m_doBTagCut) allOK = false; if(m_operatingPt == "FixedCutBEff_30") allOK = true; if(m_operatingPt == "FixedCutBEff_50") allOK = true; if(m_operatingPt == "FixedCutBEff_60") allOK = true; if(m_operatingPt == "FixedCutBEff_70") allOK = true; if(m_operatingPt == "FixedCutBEff_77") allOK = true; if(m_operatingPt == "FixedCutBEff_80") allOK = true; if(m_operatingPt == "FixedCutBEff_85") allOK = true; if(m_operatingPt == "FixedCutBEff_90") allOK = true; if(m_operatingPt == "FlatCutBEff_30") allOK = true; if(m_operatingPt == "FlatCutBEff_40") allOK = true; if(m_operatingPt == "FlatCutBEff_50") allOK = true; if(m_operatingPt == "FlatCutBEff_60") allOK = true; if(m_operatingPt == "FlatCutBEff_70") allOK = true; if(m_operatingPt == "FlatCutBEff_77") allOK = true; if(m_operatingPt == "FlatCutBEff_85") allOK = true;
    if(!allOK) {
...
bool JetSelector::executeSelection(const xAOD::JetContainer* inJets,
  ...
  std::string                                                outContainerName
    ...
    for(auto jet_itr : *jetsForSelection) { // duplicated of basic loop
        ...
        if(m_useCutFlow && passSel) m_jet_cutflowHist_1->Fill(m_jet_cutflow_cleaning_cut, 1);
    ...
    if(m_isMC && m_doJVT) {
        ...
        if( !isToolAlreadyUsed(m_JVT_tool_name) ) {
            for(const auto& syst_it : m_systListJVT) {
                ...
                if(m_debug) Info( "executeSelection()", "JVT SF sys name (to be recorded in xAOD::TStore) is: %s", sfName.c_str() ); sysVariationNamesJVT->push_back(sfName);
    ...
    if(!passEventClean) return false; if( (m_pass_min > 0) && (nPass < m_pass_min) ) {
        return false;
    }
...
int JetSelector::PassCuts(const xAOD::Jet* jet) {
    ...
    // pT
    if(m_pT_max != 1e8)
        if(jet->pt() > m_pT_max) return 0; if(m_useCutFlow) m_jet_cutflowHist_1->Fill(m_jet_cutflow_ptmax_cut, 1);

    if(m_pT_min != 1e8)
        if(jet->pt() < m_pT_min) return 0; if(m_useCutFlow) m_jet_cutflowHist_1->Fill(m_jet_cutflow_ptmin_cut, 1);

    // eta
    if(m_eta_max != 1e8)
        if(fabs( jet->eta() ) > m_eta_max) return 0; if(m_eta_min != 1e8)
        if(fabs( jet->eta() ) < m_eta_min) return 0; if(m_useCutFlow) m_jet_cutflowHist_1->Fill(m_jet_cutflow_eta_cut, 1);

    // detEta
    if(m_detEta_max != 1e8)
        if(fabs( ( jet->getAttribute<xAOD::JetFourMom_t>( m_jetScaleType.c_str() ) ).eta() ) > m_detEta_max) return 0; if(m_detEta_min != 1e8)
        if(fabs( ( jet->getAttribute<xAOD::JetFourMom_t>( m_jetScaleType.c_str() ) ).eta() ) < m_detEta_min) return 0;

    // mass
    if(m_mass_max != 1e8)
        if(jet->m() > m_mass_max) return 0; if(m_mass_min != 1e8)
        if(jet->m() < m_mass_min) return 0;

    // rapidity
    if(m_rapidity_max != 1e8)
        if(jet->rapidity() > m_rapidity_max) return 0; if(m_rapidity_min != 1e8)
        if(jet->rapidity() < m_rapidity_min) return 0;

    // JVF pileup cut
    if(m_doJVF) {
        if(m_debug) Info("PassCuts()", "Doing JVF"); if(m_debug) Info( "PassCuts()", "Jet Pt %f", jet->pt() ); if(jet->pt() < m_pt_max_JVF) {
    ...
    //
    if(m_doJVT) {
        if(m_debug) Info("PassCuts()", "Checking JVT cut..."); if(m_debug)
        ...
        if(m_JVTCut > 0) {
            if(jet->pt() < m_pt_max_JVT) {
                ...
                if(fabs( jetScaleP4.eta() ) < m_eta_max_JVT) {
                    ...
                    if(m_debug) Info( "passCuts()", " JVT = %2f ", jet->getAttribute<float>("Jvt") ); if(jet->getAttribute<float>("Jvt") < m_JVTCut) {
    ...
    //
    if(m_doBTagCut) {
        if(m_debug) Info("PassCuts()", "Doing BTagging"); if( m_BJetSelectTool->accept(jet) ) {
    ...
    if(m_truthLabel != -1) {
        ...
        if(this_TruthLabel == -1) return 0; if( (m_truthLabel == 5) && (this_TruthLabel != 5) ) return 0; if( (m_truthLabel == 4) && (this_TruthLabel != 4) ) return 0; if( (m_truthLabel == 0) && (this_TruthLabel != 0) ) return 0;
    }

    if(m_debug) Info("PassCuts()", "Passed Cuts"); return 1;
} // JetSelector::PassCuts

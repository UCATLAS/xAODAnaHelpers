...
MuonSelector::MuonSelector(std::string className) :
  ...
  m_isolationSelectionTool_handle("CP::IsolationSelectionTool/MuonIsoSelToolName",
                                  nullptr),
  ...
  m_trigMuonMatchTool_handle("Trig::MatchingTool/TrigMuonMatchingName", nullptr)
{
  ...
  //                            decorrating and output container should not be
  // mutually exclusive
  ...
  m_diMuTrigChains     = "";
  m_minDeltaR          = 0.1; // Recommended threshold for muon triggers: see
...
EL::StatusCode MuonSelector::initialize()
{
  ...
  if (m_useCutFlow) {
    ...
    m_mu_cutflow_all = m_mu_cutflowHist_1->GetXaxis()->FindBin(
      "all");
    m_mu_cutflow_eta_and_quaility_cut = m_mu_cutflowHist_1->GetXaxis()->FindBin(
      "eta_and_quality_cut");
    m_mu_cutflow_ptmax_cut = m_mu_cutflowHist_1->GetXaxis()->FindBin(
      "ptmax_cut");
    m_mu_cutflow_ptmin_cut = m_mu_cutflowHist_1->GetXaxis()->FindBin(
      "ptmin_cut");
    m_mu_cutflow_type_cut = m_mu_cutflowHist_1->GetXaxis()->FindBin(
      "type_cut");
    m_mu_cutflow_z0sintheta_cut = m_mu_cutflowHist_1->GetXaxis()->FindBin(
      "z0sintheta_cut");
    m_mu_cutflow_d0_cut = m_mu_cutflowHist_1->GetXaxis()->FindBin(
      "d0_cut");
    m_mu_cutflow_d0sig_cut = m_mu_cutflowHist_1->GetXaxis()->FindBin(
      "d0sig_cut");
    m_mu_cutflow_iso_cut = m_mu_cutflowHist_1->GetXaxis()->FindBin(
      "iso_cut");

    if (m_removeCosmicMuon) m_mu_cutflow_cosmic_cut =
        m_mu_cutflowHist_1->GetXaxis()->FindBin("cosmic_cut");


    if (m_isUsedBefore) {
      m_mu_cutflowHist_2 = (TH1D *)file->Get("cutflow_muons_2");

      m_mu_cutflow_all = m_mu_cutflowHist_2->GetXaxis()->FindBin(
        "all");
      m_mu_cutflow_eta_and_quaility_cut = m_mu_cutflowHist_2->GetXaxis()->FindBin(
        "eta_and_quality_cut");
      m_mu_cutflow_ptmax_cut = m_mu_cutflowHist_2->GetXaxis()->FindBin(
        "ptmax_cut");
      m_mu_cutflow_ptmin_cut = m_mu_cutflowHist_2->GetXaxis()->FindBin(
        "ptmin_cut");
      m_mu_cutflow_type_cut = m_mu_cutflowHist_2->GetXaxis()->FindBin(
        "type_cut");
      m_mu_cutflow_z0sintheta_cut = m_mu_cutflowHist_2->GetXaxis()->FindBin(
        "z0sintheta_cut");
      m_mu_cutflow_d0_cut = m_mu_cutflowHist_2->GetXaxis()->FindBin(
        "d0_cut");
      m_mu_cutflow_d0sig_cut = m_mu_cutflowHist_2->GetXaxis()->FindBin(
        "d0sig_cut");
      m_mu_cutflow_iso_cut = m_mu_cutflowHist_2->GetXaxis()->FindBin(
        "iso_cut");

      if (m_removeCosmicMuon) m_mu_cutflow_cosmic_cut =
          m_mu_cutflowHist_2->GetXaxis()->FindBin("cosmic_cut");
    }
  } // if m_useCutFlow
  ...
  //  Error("initialize()", "Unknown muon type requested:
  // %s!",m_muonType.c_str());
  //  return EL::StatusCode::FAILURE;
  // }
  ...
  // ********************************
  //
  // Set eta and quality requirements in order to accept the muon - ID tracks
  // required by default
  //

  m_muonSelectionTool_name = "MuonSelectionTool_" + m_name;
  std::string muonSelectionTool_handle_name = "CP::MuonSelectionTool/" +
                                              m_muonSelectionTool_name;

  RETURN_CHECK("MuonSelector::initialize()",
               checkToolStore<CP::MuonSelectionTool>(m_muonSelectionTool_name),
               "");

  // RETURN_CHECK("MuonSelector::initialize()",
  // m_muonSelectionTool_handle.makeNew<CP::MuonSelectionTool>(muonSelectionTool_handle_name),
  // "Failed to create handle to CP::MuonSelectionTool");
  RETURN_CHECK("MuonSelector::initialize()",
               m_muonSelectionTool_handle.make(
                 muonSelectionTool_handle_name),
               "Failed to create handle to CP::MuonSelectionTool");
  RETURN_CHECK("MuonSelector::initialize()",
               m_muonSelectionTool_handle.setProperty("MaxEta",
                                                      static_cast<double>(
                                                        m_eta_max)),
               "Failed to set Failed to set MaxEta property");
  RETURN_CHECK("MuonSelector::initialize()",
               m_muonSelectionTool_handle.setProperty("MuQuality", m_muonQuality),
               "Failed to set MuQuality property");
  RETURN_CHECK("MuonSelector::initialize()",
               m_muonSelectionTool_handle.initialize(),
               "Failed to properly initialize CP::MuonSelectionTool");
  ...
  // RETURN_CHECK("MuonSelector::initialize()",
  // m_isolationSelectionTool_handle.makeNew<CP::IsolationSelectionTool>(isolationSelectionTool_handle_name),
  // "Failed to create handle to CP::IsolationSelectionTool");
  RETURN_CHECK("MuonSelector::initialize()",
               m_isolationSelectionTool_handle.make(
  ...
  for (auto WP_itr = std::next(m_IsoKeys.begin()); WP_itr != m_IsoKeys.end();
    ...
    if ((*WP_itr).find("UserDefined") != std::string::npos) {
      ...
      RETURN_CHECK("MuonSelector::initialize()",
                   m_isolationSelectionTool_handle->addMuonWP((*WP_itr).c_str()),
  ...
  // the tool!
  //     do not initialise if there are no input trigger chains, or the
  // TrigDecisionTool hasn't been configured yet
  //
  if (!(m_singleMuTrigChains.empty() && m_diMuTrigChains.empty()) &&
    ...
    std::string trigMuonMatchTool_handle_name = "Trig::MatchingTool/" +
                                                m_trigMuonMatchTool_name;

    RETURN_CHECK("MuonSelector::initialize()",
                 checkToolStore<Trig::MatchingTool>(m_trigMuonMatchTool_name),
                 "");

    // RETURN_CHECK("MuonSelector::initialize()",
    // m_trigMuonMatchTool_handle.makeNew<Trig::MatchingTool>(trigMuonMatchTool_handle_name),
    // "Failed to create handle to MatchingTool");
    RETURN_CHECK("MuonSelector::initialize()",
                 m_trigMuonMatchTool_handle.make(
                   trigMuonMatchTool_handle_name),
                 "Failed to create handle to MatchingTool");
    RETURN_CHECK("MuonSelector::initialize()",
                 m_trigMuonMatchTool_handle.setProperty("TrigDecisionTool",
                                                        trigDecHandle),
                 "Failed to pass TrigDecisionTool to MatchingTool");
    RETURN_CHECK("MuonSelector::initialize()",
                 m_trigMuonMatchTool_handle.initialize(),
...
EL::StatusCode MuonSelector::execute()
{
  ...
  RETURN_CHECK("MuonSelector::execute()",
  ...
  if ((m_numEvent == 1) && m_trigDecTool) {
    ...
    for (auto const& chain : m_singleMuTrigChainsList) Info("execute()",
                                                            "\t %s",
                                                            chain.c_str()); Info(
      "execute()",
      "\n");
    ...
    Info("execute()",
         "Input di-muon trigger chains that will be considered for matching:\n");

    for (auto const& chain : m_diMuTrigChainsList) Info("execute()",
                                                        "\t %s",
                                                        chain.c_str()); Info(
      "execute()",
      "\n");
  ...
  if (m_inputAlgoSystNames.empty()) {
    ...
    //
    RETURN_CHECK("MuonSelector::execute()",
                 HelperFunctions::retrieve(inMuons, m_inContainerName, m_event,
                                           m_store,
                                           m_verbose), "");
    ...
    if (m_createSelectedContainer) selectedMuons =
        new ConstDataVector<xAOD::MuonContainer>(SG::VIEW_ELEMENTS);
    ...
    for (auto systName : *systNames) {
      ...
      RETURN_CHECK("MuonSelector::execute()",
                   HelperFunctions::retrieve(inMuons,
      m_inContainerName + systName, m_event,
                                             m_store, m_verbose), "");
      ...
      // find the selected muons, and return if event passes object selection
      //
      eventPassThisSyst = executeSelection(inMuons,
      mcEvtWeight,
      countPass,
      selectedMuons);
  ...
  if (m_debug) Info("execute()", "Left Muon Selection..."); return EL::StatusCode::SUCCESS;
}
...
{
  if (m_debug) Info("execute()", "In  executeSelection..."); const xAOD::VertexContainer *vertices(nullptr);
  ...
  if ((m_pass_min > 0) && (nPass < m_pass_min)) {
    if (m_debug) Info("executeSelection()", "Reject event: nSelectedMuons (%i) < nPassMin (%i)", nPass, m_pass_min); return false;
  }

  if ((m_pass_max > 0) && (nPass > m_pass_max)) {
    if (m_debug) Info("executeSelection()", "Reject event: nSelectedMuons (%i) > nPassMax (%i)", nPass, m_pass_max); return false;
  }
    ...
    if (nSelectedMuons > 0) {
      ...
      for (auto const& chain : m_singleMuTrigChainsList) {
        ...
        for (auto const muon : *selectedMuons) {
          //  For each muon, decorate w/ a map<string,char> with the 'isMatched'
          // info associated
          //  to each trigger chain in the input list.
          //  If decoration map doesn't exist for this muon yet, create it (will
  ...
  if (m_debug) Info("execute()", "Left  executeSelection..."); return true;
}
...
int MuonSelector::passCuts(const xAOD::Muon *muon, const xAOD::Vertex *primaryVertex) {
  if (m_debug) Info("execute()", "In  passCuts...");  // fill cutflow bin 'all'
                                                      // before any cut
  ...
  if (m_debug) Info("execute()", "Got the decors"); int this_quality = static_cast<int>(m_muonSelectionTool_handle->getQuality(*muon));
  ...
  if (m_debug) Info("execute()", "Doing muon quality");  // this will accept the
                                                         // muon based on the
                                                         // settings at
                                                         // initialization :
                                                         // eta,
  ...
  if (!m_muonSelectionTool_handle->accept(*muon)) {
    if (m_debug) Info("PassCuts()", "Muon failed requirements of MuonSelectionTool."); return 0;
  }
  ...
  if (m_pT_max != 1e8) {
    if (muon->pt() > m_pT_max) {
      if (m_debug) Info("PassCuts()", "Muon failed pT max cut."); return 0;
    }
  }
  ...
  if (m_pT_min != 1e8) {
    if (muon->pt() < m_pT_min) {
      if (m_debug) Info("PassCuts()", "Muon failed pT min cut."); return 0;
    }
  }
  ...
  //    if ( m_debug ) { Info("PassCuts()", "Muon type: %d - required: %s .
  // Failed", muon->muonType(), m_muonType.c_str()); }
  //    return 0;
  //  }
  // }
  // if(m_useCutFlow) m_mu_cutflowHist_1->Fill( m_mu_cutflow_type_cut, 1 );
  // if ( m_isUsedBefore && m_useCutFlow ) { m_mu_cutflowHist_2->Fill(
  // m_mu_cutflow_type_cut, 1 ); }
  ...
  // The following returns a pointer (which should not usually be NULL, but
  // might be if the muon has been stripped of information) to the
  ...
  if (!(fabs(z0sintheta) < m_z0sintheta_max)) {
    if (m_debug) Info("PassCuts()", "Muon failed z0*sin(theta) cut."); return 0;
  }
  ...
  //
  if (!(tp->d0() < m_d0_max)) {
    if (m_debug) Info("PassCuts()", "Muon failed d0 cut."); return 0;
  }
  ...
  if (!(d0_significance < m_d0sig_max)) {
    if (m_debug) Info("PassCuts()", "Muon failed d0 significance cut."); return 0;
  }
  ...
  for (auto WP_itr : m_IsoKeys) {
    ...
    if (m_debug) Info("PassCuts()", "Decorate muon with %s - accept() ? %i", decorWP.c_str(), accept_list.getCutResult(WP_itr.c_str())); muon->auxdecor<char>(decorWP) = static_cast<char>(accept_list.getCutResult(WP_itr.c_str()));
  }

  // Apply the cut if needed
  //
  if (!m_MinIsoWPCut.empty() && !accept_list.getCutResult(m_MinIsoWPCut.c_str())) {
    if (m_debug) Info("PassCuts()", "Muon failed isolation cut %s ",  m_MinIsoWPCut.c_str()); return 0;
  }
  ...
  if (m_debug) Info("execute()", "Leave passCuts... pass"); return 1;
}

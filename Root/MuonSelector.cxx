...
EL::StatusCode MuonSelector::initialize() {
    ...
    if(m_useCutFlow) {
        ...
        if(m_removeCosmicMuon) m_mu_cutflow_cosmic_cut = m_mu_cutflowHist_1->GetXaxis()->FindBin("cosmic_cut");
        ...
        if(m_isUsedBefore) {
            ...
            if(m_removeCosmicMuon) m_mu_cutflow_cosmic_cut = m_mu_cutflowHist_2->GetXaxis()->FindBin("cosmic_cut");
        }
    }// if m_useCutFlow
    ...
    //
    if(m_debug) Info( "initialize()", "Adding isolation WP %s to IsolationSelectionTool", ( m_IsoKeys.at(0) ).c_str() ); RETURN_CHECK("MuonSelector::initialize()", m_isolationSelectionTool_handle.setProperty( "MuonWP", ( m_IsoKeys.at(0) ).c_str() ), "Failed to configure base WP");
...
int MuonSelector::passCuts(const xAOD::Muon* muon, const xAOD::Vertex* primaryVertex) {
    if(m_debug) Info("execute()", "In  passCuts...");  // fill cutflow bin 'all' before any cut
    if(m_useCutFlow) m_mu_cutflowHist_1->Fill(m_mu_cutflow_all, 1);
    if(m_isUsedBefore && m_useCutFlow) m_mu_cutflowHist_2->Fill(m_mu_cutflow_all, 1); if(m_debug) Info("execute()", "In  passCuts2...");  // *********************************************************************************************************************************************************************
    //
    ...
    if(m_debug) Info("execute()", "Doing muon quality");  // this will accept the muon based on the settings at initialization : eta, ID track info, muon quality
    ...
    // pT max cut
    //
    if(m_debug) Info("execute()", "Doing pt cuts"); if(m_pT_max != 1e8) {
        if(muon->pt() > m_pT_max) {

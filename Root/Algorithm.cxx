...
int xAH::Algorithm::isMC() {
  ...
  if (!HelperFunctions::retrieve(ei, m_eventInfoContainerName, m_event,
                                 m_store).isSuccess()) {
    if (m_debug) Warning("isMC()",
                         "Could not retrieve eventInfo container: %s",
                         m_eventInfoContainerName.c_str());
    return -1;
  }
...
int xAH::Algorithm::numInstances() {
  if (m_instanceRegistry.find(m_className) == m_instanceRegistry.end()) {
    printf(
      "numInstances: we seem to have recorded zero instances of %s. This should not happen.",
      m_className.c_str());
    return 0;
  }
...
void xAH::Algorithm::unregisterInstance() {
  if (m_instanceRegistry.find(m_className) == m_instanceRegistry.end()) {
    printf(
      "unregisterInstance: we seem to have recorded zero instances of %s. This should not happen.",
      m_className.c_str());
  }
  m_instanceRegistry[m_className]--;
}

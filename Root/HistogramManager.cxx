  ...
  m_detailStr(detailStr)
{
  ...
  if (isalnum(m_name.back()) && !ispunct(m_name.back())) {
    ...
    // Info("HistogramManager()", "Adding slash to put hists in TDirectories:
    // %s",m_name.c_str());
  }
}
...
TH1F * HistogramManager::book(std::string name,
                              std::string title,
                              std::string xlabel,
                              int         xbins,
                              double      xlow,
...
TH2F * HistogramManager::book(std::string name,
                              std::string title,
                              std::string xlabel,
                              int         xbins,
                              double      xlow,
                              ...
                              double      yhigh)
{
  TH2F *tmp = new TH2F((name + title).c_str(),
                       title.c_str(), xbins, xlow, xhigh, ybins, ylow, yhigh);
...
TH3F * HistogramManager::book(std::string name,
                              std::string title,
                              std::string xlabel,
                              int         xbins,
                              double      xlow,
                              ...
                              double      zhigh)
{
  TH3F *tmp = new TH3F((name + title).c_str(),
                       title.c_str(), xbins, xlow, xhigh, ybins, ylow, yhigh, zbins, zlow,
                       zhigh);
...
TProfile * HistogramManager::book(std::string name,
                                  std::string title,
                                  std::string xlabel,
                                  int         xbins,
                                  double      xlow,
                                  ...
                                  std::string option)
{
  TProfile *tmp = new TProfile((name + title).c_str(),
                               title.c_str(), xbins, xlow, xhigh, ylow, yhigh,
                               option.c_str());
...
{
  TH2F *tmp = new TH2F((name + title).c_str(),
                       title.c_str(), xbins, xbinArr, ybins, ylow, yhigh);
...
TH2F * HistogramManager::book(std::string     name,
                              ...
                              std::string     ylabel,
                              int             ybins,
                              const Double_t *ybinArr)
{
  TH2F *tmp = new TH2F((name + title).c_str(),
                       title.c_str(), xbins, xlow, xhigh, ybins, ybinArr);
...
{
  TH2F *tmp = new TH2F((name + title).c_str(),
                       title.c_str(), xbins, xbinArr, ybins, ybinArr);
...
TH3F * HistogramManager::book(std::string     name,
                              ...
                              std::string     zlabel,
                              int             zbins,
                              const Double_t *zbinArr)
{
  TH3F *tmp = new TH3F((name + title).c_str(),
                       title.c_str(), xbins, xbinArr, ybins, ybinArr, zbins,
                       zbinArr);
...
void HistogramManager::SetLabel(TH1        *hist,
                                std::string xlabel,
                                std::string ylabel,
                                std::string zlabel)
{

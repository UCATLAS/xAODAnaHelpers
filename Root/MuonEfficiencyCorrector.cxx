...
EL::StatusCode MuonEfficiencyCorrector::initialize() {
    ...
    return EL::StatusCode::SUCCESS;
} // MuonEfficiencyCorrector::initialize
...
EL::StatusCode MuonEfficiencyCorrector::execute() {
    ...
    if(!m_isMC) {
        if(m_numEvent == 1) Info("execute()", "Sample is Data! Do not apply any Muon Efficiency correction... "); return EL::StatusCode::SUCCESS;
    }
    ...
    return EL::StatusCode::SUCCESS;
} // MuonEfficiencyCorrector::execute
...
EL::StatusCode MuonEfficiencyCorrector::executeSF(const xAOD::EventInfo* eventInfo, const xAOD::MuonContainer* inputMuons, unsigned int countSyst, bool isNomSel) {
    ...
    if( !isToolAlreadyUsed(m_recoEffSF_tool_name) ) {
        ...
        for(const auto& syst_it : m_systListReco) {
            ...
            if(m_debug) Info( "executeSF()", "Muon reco efficiency SF sys name (to be recorded in xAOD::TStore) is: %s", sfName.c_str() ); if(countSyst == 0) sysVariationNamesReco->push_back(sfName);
    ...
    if( !isToolAlreadyUsed(m_isoEffSF_tool_name) ) {
        ...
        for(const auto& syst_it : m_systListIso) {
            ...
            if(m_debug) Info( "executeSF()", "Muon iso efficiency SF sys name (to be recorded in xAOD::TStore) is: %s", sfName.c_str() ); if(countSyst == 0) sysVariationNamesIso->push_back(sfName);
    ...
    if(m_isMC && !m_useRandomRunNumber) {
        int rn = 0;
        if(m_YearsList[0] == "2015") {rn = m_runNumber2015;} else if(m_YearsList[0] == "2016") {
            rn = m_runNumber2016;
        } else {
    ...
    if( !isToolAlreadyUsed(m_trigEffSF_tool_names[randYear]) ) {
        for(const auto& trig_it : m_SingleMuTriggers) {
            ...
            for(const auto& syst_it : m_systListTrig) {
                ...
                if(m_debug) Info( "executeSF()", "Trigger efficiency SF sys name (to be recorded in xAOD::TStore) is: %s", sfName.c_str() ); if(countSyst == 0) sysVariationNamesTrig->push_back(sfName);
    ...
    if( !isToolAlreadyUsed(m_TTVAEffSF_tool_name) ) {
        ...
        for(const auto& syst_it : m_systListTTVA) {
            ...
            if(m_debug) Info( "executeSF()", "Muon iso efficiency SF sys name (to be recorded in xAOD::TStore) is: %s", sfName.c_str() ); if(countSyst == 0) sysVariationNamesTTVA->push_back(sfName);
    ...
    return EL::StatusCode::SUCCESS;
} // MuonEfficiencyCorrector::executeSF

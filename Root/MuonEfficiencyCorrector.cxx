...
EL::StatusCode MuonEfficiencyCorrector::setupJob(EL::Job& job)
{
  ...
  job.useXAOD();
  xAOD::Init("MuonEfficiencyCorrector").ignore(); // call before opening first
                                                  // file
...
EL::StatusCode MuonEfficiencyCorrector::initialize()
{
  ...
  RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
               HelperFunctions::retrieve(eventInfo, m_eventInfoContainerName,
                                         m_event, m_store,
                                         m_verbose), "");
  ...
  // Create a ToolHandle of the PRW tool which is passed to the
  // MuonEfficiencyScaleFactors class later
  //
  ...
  if (m_isMC) {
    if (m_pileup_tool_handle.isUserConfigured()) {
      RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                   ...
                   "Failed to retrieve Pileup Tool");
    } else {
      Error("initialize()",
            "A configured CP::PileupReweightingTool must already exist in the asg::ToolStore! Are you creating one in xAH::BasicEventSelector?");
      return EL::StatusCode::FAILURE;
    }
  }
  ...
  //
  m_recoEffSF_tool_name = "MuonEfficiencyScaleFactors_effSF_Reco_" +
                          m_WorkingPointReco;

  std::cout << "" << std::endl;
  Info("MuonEfficiencyCorrector::initialize()",
       " Initialising CP::MuonEfficiencyScaleFactors for RECO efficiency SF...");
  std::cout << "" << std::endl;

  RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
               checkToolStore<CP::MuonEfficiencyScaleFactors>(
                 m_recoEffSF_tool_name), "");

  if (asg::ToolStore::contains<CP::MuonEfficiencyScaleFactors>(
        m_recoEffSF_tool_name)) {
    m_muRecoSF_tool = asg::ToolStore::get<CP::MuonEfficiencyScaleFactors>(
      m_recoEffSF_tool_name);
  } else {
    m_muRecoSF_tool = new CP::MuonEfficiencyScaleFactors(m_recoEffSF_tool_name);
    RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                 m_muRecoSF_tool->setProperty("WorkingPoint",
                                              m_WorkingPointReco),
                 "Failed to set Working Point property of MuonEfficiencyScaleFactors for reco efficiency SF");
    RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                 m_muRecoSF_tool->setProperty("CalibrationRelease",
                                              m_calibRelease),
                 "Failed to set calibration release property of MuonEfficiencyScaleFactors for reco efficiency SF");
    RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                 m_muRecoSF_tool->initialize(),
                 "Failed to properly initialize CP::MuonEfficiencyScaleFactors for reco efficiency SF");
    ...
    if (m_debug) {
      ...
      for (const auto& syst_it : affectSystsReco) Info("initialize()",
                                                       "MuonEfficiencyScaleFactors tool can be affected by reco efficiency systematic: %s",
                                                       (syst_it.name()).c_str());
    }
    ...
    //
    const CP::SystematicSet recSystsReco =
      m_muRecoSF_tool->recommendedSystematics();
    m_systListReco = HelperFunctions::getListofSystematics(recSystsReco,
                                                           m_systNameReco,
                                                           m_systValReco,
                                                           m_debug);

    Info("initialize()",
         "Will be using MuonEfficiencyScaleFactors tool reco efficiency systematic:");
  ...
  // 2.
  // initialize the CP::MuonEfficiencyScaleFactors Tool for isolation efficiency
  // SF
  //

  m_isoEffSF_tool_name = "MuonEfficiencyScaleFactors_effSF_Iso_" +
                         m_WorkingPointIso;
  std::string iso_WP = m_WorkingPointIso + "Iso";

  std::cout << "" << std::endl;
  Info("MuonEfficiencyCorrector::initialize()",
       " Initialising CP::MuonEfficiencyScaleFactors for ISO efficiency SF...");
  std::cout << "" << std::endl;
  ...
  if (asg::ToolStore::contains<CP::MuonEfficiencyScaleFactors>(
        m_isoEffSF_tool_name)) {
    m_muIsoSF_tool = asg::ToolStore::get<CP::MuonEfficiencyScaleFactors>(
      m_isoEffSF_tool_name);
  } else {
    m_muIsoSF_tool = new CP::MuonEfficiencyScaleFactors(m_isoEffSF_tool_name);
    RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                 m_muIsoSF_tool->setProperty("WorkingPoint",
                                             iso_WP),
                 "Failed to set Working Point property of MuonEfficiencyScaleFactors for iso efficiency SF");
    RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                 m_muIsoSF_tool->setProperty("CalibrationRelease",
                                             m_calibRelease),
                 "Failed to set calibration release property of MuonEfficiencyScaleFactors for iso efficiency SF");
    RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                 m_muIsoSF_tool->initialize(),
                 "Failed to properly initialize CP::MuonEfficiencyScaleFactors for iso efficiency SF");
    ...
    if (m_debug) {
      ...
      for (const auto& syst_it : affectSystsIso) Info("initialize()",
                                                      "MuonEfficiencyScaleFactors tool can be affected by iso efficiency systematic: %s",
                                                      (syst_it.name()).c_str());
    }
    ...
    // Use HelperFunctions::getListofSystematics() for this!
    //
    const CP::SystematicSet recSystsIso =
      m_muIsoSF_tool->recommendedSystematics();
    m_systListIso = HelperFunctions::getListofSystematics(recSystsIso,
                                                          m_systNameIso,
                                                          m_systValIso,
                                                          m_debug);

    Info("initialize()",
         "Will be using MuonEfficiencyScaleFactors tool iso efficiency systematic:");
  ...
  if (m_isMC && !m_useRandomRunNumber) {
    Warning("executeSF()",
            "m_useRandomRunNumber is set to false! This is not recommended!!!");

    if (m_YearsList.size() > 1) {
      Error("executeSF()",
            "In this case the list of years should contain only one element");
      return EL::StatusCode::FAILURE;
    }
  }
  ...
  for (auto yr : m_YearsList) {
    m_trigEffSF_tool_names[yr] = "MuonTriggerScaleFactors_" + yr +
                                 "_effSF_Trig_Reco" + m_WorkingPointRecoTrig +
                                 "_Iso" + m_WorkingPointIsoTrig;
  }
  ...
  for (auto yr : m_YearsList) {
    ...
    std::cout << "" << std::endl;
    Info("MuonEfficiencyCorrector::initialize()",
         " Initialising CP::MuonTriggerScaleFactors for TRIGGER efficiency SF...");
    std::cout << "" << std::endl;

    RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                 checkToolStore<CP::MuonTriggerScaleFactors>(
                   m_trigEffSF_tool_names[yr]),
                 "Failed to check whether tool already exists in asg::ToolStore");

    if (asg::ToolStore::contains<CP::MuonTriggerScaleFactors>(
          m_trigEffSF_tool_names[yr])) {
      m_muTrigSF_tools[yr] = asg::ToolStore::get<CP::MuonTriggerScaleFactors>(
        m_trigEffSF_tool_names[yr]);
    } else {
      m_muTrigSF_tools[yr] = new CP::MuonTriggerScaleFactors(
        m_trigEffSF_tool_names[yr]);

      if (m_AllowZeroSF) {
        Warning("MuonEfficiencyCorrector::initialize()",
                "m_AllowZeroSF is set to True. No errors will arise for runs missing required triggers!!!");
        RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                     m_muTrigSF_tools[yr]->setProperty("AllowZeroSF",
                                                       m_AllowZeroSF),
                     "Failed to set AllowZeroSF property of MuonTriggerScaleFactors");
      }

      RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                   m_muTrigSF_tools[yr]->setProperty("Isolation", iso_trig_WP),
                   "Failed to set Isolation property of MuonTriggerScaleFactors");
      RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                   m_muTrigSF_tools[yr]->setProperty("MuonQuality",
                                                     m_WorkingPointRecoTrig),
                   "Failed to set MuonQuality property of MuonTriggerScaleFactors");

      if (!yr.empty()) {
        RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                     m_muTrigSF_tools[yr]->setProperty("Year", yr),
                     "Failed to set Year property of MuonTriggerScaleFactors");
      }

      if (!m_MCCampaign.empty()) {
        RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                     m_muTrigSF_tools[yr]->setProperty("MC",
                                                       m_MCCampaign),
                     "Failed to set MC Campaign property of MuonTriggerScaleFactors");
      }
      RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                   m_muTrigSF_tools[yr]->initialize(),
                   "Failed to properly initialize CP::MuonTriggerScaleFactors for trigger efficiency SF");
    }
  }
  ...
  //  Add the chosen WP to the string labelling the vector<SF>/vector<eff>
  // decoration
  //
  m_outputSystNamesTrig = m_outputSystNamesTrig + "_Reco" +
                          m_WorkingPointRecoTrig + "_Iso" + m_WorkingPointIsoTrig;
  m_outputSystNamesTrigMCEff = m_outputSystNamesTrigMCEff + "_Reco" +
                               m_WorkingPointRecoTrig + "_Iso" +
                               m_WorkingPointIsoTrig;

  if (m_debug) {
    CP::SystematicSet affectSystsTrig =
      m_muTrigSF_tools[m_YearsList[0]]->affectingSystematics();

    for (const auto& syst_it : affectSystsTrig) Info("initialize()",
                                                     "MuonEfficiencyScaleFactors tool can be affected by trigger efficiency systematic: %s",
                                                     (syst_it.name()).c_str());
  }
  ...
  // Use HelperFunctions::getListofSystematics() for this!
  //
  const CP::SystematicSet recSystsTrig =
    m_muTrigSF_tools[m_YearsList[0]]->recommendedSystematics();
  m_systListTrig = HelperFunctions::getListofSystematics(recSystsTrig,
                                                         m_systNameTrig,
                                                         m_systValTrig,
                                                         m_debug);

  Info("initialize()",
       "Will be using MuonEfficiencyScaleFactors tool trigger efficiency systematic:");
  ...
  // 4.
  // initialize the CP::MuonEfficiencyScaleFactors Tool for track-to-vertex
  // association (TTVA) SF
  //

  m_TTVAEffSF_tool_name = "MuonEfficiencyScaleFactors_effSF_" +
                          m_WorkingPointTTVA;

  std::cout << "" << std::endl;
  Info("MuonEfficiencyCorrector::initialize()",
       " Initialising CP::MuonEfficiencyScaleFactors for TTVA efficiency SF...");
  std::cout << "" << std::endl;
  ...
  if (asg::ToolStore::contains<CP::MuonEfficiencyScaleFactors>(
        m_TTVAEffSF_tool_name)) {
    m_muTTVASF_tool = asg::ToolStore::get<CP::MuonEfficiencyScaleFactors>(
      m_TTVAEffSF_tool_name);
  } else {
    m_muTTVASF_tool = new CP::MuonEfficiencyScaleFactors(m_TTVAEffSF_tool_name);
    RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                 m_muTTVASF_tool->setProperty("WorkingPoint",
                                              m_WorkingPointTTVA),
                 "Failed to set Working Point property of MuonEfficiencyScaleFactors for TTVA efficiency SF");
    RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                 m_muTTVASF_tool->setProperty("CalibrationRelease",
                                              m_calibRelease),
                 "Failed to set calibration release property of MuonEfficiencyScaleFactors for TTVA efficiency SF");
    RETURN_CHECK("MuonEfficiencyCorrector::initialize()",
                 m_muTTVASF_tool->initialize(),
                 "Failed to properly initialize CP::MuonEfficiencyScaleFactors for TTVA efficiency SF");
    ...
    if (m_debug) {
      ...
      for (const auto& syst_it : affectSystsTTVA) Info("initialize()",
                                                       "MuonEfficiencyScaleFactors tool can be affected by TTVA efficiency systematic: %s",
                                                       (syst_it.name()).c_str());
    }
    ...
    // Use HelperFunctions::getListofSystematics() for this!
    //
    const CP::SystematicSet recSystsTTVA =
      m_muTTVASF_tool->recommendedSystematics();
    m_systListTTVA = HelperFunctions::getListofSystematics(recSystsTTVA,
                                                           m_systNameTTVA,
                                                           m_systValTTVA,
                                                           m_debug);

    Info("initialize()",
         "Will be using MuonEfficiencyScaleFactors tool TTVA efficiency systematic:");
  ...
  Info("initialize()",
       "MuonEfficiencyCorrector Interface succesfully initialized!");
...
EL::StatusCode MuonEfficiencyCorrector::execute()
{
  ...
  if (!m_isMC) {
    if (m_numEvent == 1) Info("execute()",
                              "Sample is Data! Do not apply any Muon Efficiency correction... ");
  ...
  const xAOD::EventInfo *eventInfo(nullptr);
  RETURN_CHECK("MuonEfficiencyCorrector::execute()",
               HelperFunctions::retrieve(eventInfo, m_eventInfoContainerName,
                                         m_event, m_store,
                                         m_verbose), "");
  ...
  // if m_inputAlgoSystNames = "" --> input comes from xAOD, or just running one
  // collection,
  // then get the one collection and be done with it

  // Declare a counter, initialised to 0
  // For the systematically varied input containers, we won't store again the
  // vector with efficiency systs in TStore ( it will be always the same!)
  //
  unsigned int countInputCont(0);

  if (m_inputAlgoSystNames.empty()) {
    // I might not want to decorate sys altered muons but for some events the
    // nominal container might not exist
    // if muons are only allowed in systematic instances, hence, we have to
    // check for the existence of the nominal container
    //
    if (m_store->contains<xAOD::MuonContainer>(m_inContainerName)) {
      RETURN_CHECK("MuonEfficiencyCorrector::execute()",
                   HelperFunctions::retrieve(inputMuons, m_inContainerName,
                                             m_event, m_store,
                                             m_verbose),
                   "");

      if (m_debug) Info("execute", "Number of muons: %i",
                        static_cast<int>(inputMuons->size()));

      // decorate muons w/ SF - there will be a decoration w/ different name for
      // each syst!
      //
      this->executeSF(eventInfo, inputMuons, countInputCont, true);
    }
  } else {
    // if m_inputAlgo = NOT EMPTY --> you are retrieving syst varied containers
    // from an upstream algo. This is the case of calibrators: one different SC
    // for each calibration syst applied

    // get vector of string giving the syst names of the upstream algo
    // m_inputAlgo (rememeber: 1st element is a blank string: nominal case!)
    //
    ...
    for (auto sysInput : m_sysNames) {
      ...
      RETURN_CHECK("MuonEfficiencyCorrector::execute()",
                   HelperFunctions::retrieve(it_systNames, sysInput, 0, m_store,
                                             m_verbose),
                   "");
      systNames.insert(systNames.end(), it_systNames->begin(),
                       it_systNames->end());
    }
    ...
    if (!m_sysNamesForParCont.empty())  {
      ...
      RETURN_CHECK("MuonEfficiencyCorrector::execute()",
                   HelperFunctions::retrieve(par_systNames, m_sysNamesForParCont,
                                             0, m_store,
                                             m_verbose),
                   "");
      ...
      for (auto sys : *par_systNames) {
        if (!sys.empty() &&
            !m_store->contains<xAOD::MuonContainer>(m_inContainerName + sys)) {
          const xAOD::MuonContainer *tmpMuons(nullptr);

          if (m_store->contains<xAOD::MuonContainer>(m_inContainerName)) {
            RETURN_CHECK("MuonEfficiencyCorrector::execute()",
                         HelperFunctions::retrieve(tmpMuons,
                                                   m_inContainerName,
                                                   m_event,
                                                   m_store,
                                                   m_verbose),
                         "");
            RETURN_CHECK("MuonEfficiencyCorrector::execute()",
                         (HelperFunctions::makeDeepCopy<xAOD::MuonContainer,
    ...
    for (auto systName : systNames) {
      ...
      if (m_store->contains<xAOD::MuonContainer>(m_inContainerName + systName)) {
        RETURN_CHECK("MuonEfficiencyCorrector::execute()",
                     HelperFunctions::retrieve(inputMuons,
                                               m_inContainerName + systName,
                                               m_event, m_store, m_verbose),
                     "");

        if (!m_store->contains<xAOD::MuonContainer>(m_outContainerName +
                                                    systName)) {
          RETURN_CHECK("MuonEfficiencyCorrector::execute()",
                       (HelperFunctions::makeDeepCopy<xAOD::MuonContainer,
                                                      xAOD::MuonAuxContainer,
        ...
        RETURN_CHECK("MuonEfficiencyCorrector::execute()",
                     HelperFunctions::retrieve(outputMuons,
                                               m_outContainerName + systName,
                                               m_event, m_store, m_verbose),
                     "");

        if (m_debug) {
          Info("execute", "Number of muons: %i",
               static_cast<int>(outputMuons->size()));
          Info("execute", "Input syst: %s", systName.c_str());
          unsigned int idx(0);

          for (auto mu : *(outputMuons)) {
            Info("execute", "Input muon %i, pt = %.2f GeV ", idx,
                 (mu->pt() * 1e-3));
            ++idx;
          }
        }

        // decorate muons w/ SF - there will be a decoration w/ different name
        // for each syst!
        //
        this->executeSF(eventInfo, outputMuons, countInputCont,
                        isNomMuonSelection);
    ...
    if (!m_outputAlgoSystNames.empty() &&
        !m_store->contains<std::vector<std::string> >(m_outputAlgoSystNames)) { //
                                                                                //
                                                                                //
                                                                                // might
                                                                                //
                                                                                ...
                                                                                // algo
      RETURN_CHECK("MuonEfficiencyCorrector::execute()",
                   m_store->record(vecOutContainerNames,
                                   m_outputAlgoSystNames),
...
EL::StatusCode MuonEfficiencyCorrector::executeSF(
  const xAOD::EventInfo     *eventInfo,
  const xAOD::MuonContainer *inputMuons,
  unsigned int               countSyst,
  bool                       isNomSel)
{
  //
  // In the following, every muon gets decorated with 2 vector<double>'s (for
  // reco/iso efficiency SFs),
  // and the event w/ 1 vector<double> (for trigger efficiency SFs)
  // Each vector contains the SFs, one SF for each syst (first component of each
  // vector will be the nominal SF).
  //
  // Additionally, we create these vector<string> with the SF syst names, so
  // that we know which component corresponds to.
  ...
  //
  // Firstly, loop over available systematics for this tool - remember: syst ==
  // EMPTY_STRING --> nominal
  ...
  if (!isToolAlreadyUsed(m_recoEffSF_tool_name)) {
    ...
    for (const auto& syst_it : m_systListReco) {
      if (m_decorateWithNomOnInputSys && !syst_it.name().empty() &&
          !isNomSel) continue;
      ...
      if (m_debug) Info("executeSF()",
                        "Muon reco efficiency SF sys name (to be recorded in xAOD::TStore) is: %s",
                        sfName.c_str());
      ...
      // apply syst
      //
      if (m_muRecoSF_tool->applySystematicVariation(syst_it) !=
          CP::SystematicCode::Ok) {
        Error("executeSF()",
              "Failed to configure MuonEfficiencyScaleFactors for systematic %s",
              syst_it.name().c_str());
        return EL::StatusCode::FAILURE;
      }

      if (m_debug) Info("executeSF()",
                        "Successfully applied systematic: %s",
                        syst_it.name().c_str());
      ...
      for (auto mu_itr : *(inputMuons)) {
        ...
        // a)
        // decorate directly the muon with reco efficiency (useful at all?), and
        // the corresponding SF
        //
        // if ( m_muRecoSF_tool->applyMCEfficiency( *mu_itr ) !=
        // CP::CorrectionCode::Ok ) {
        //  Warning( "executeSF()", "Problem in applyMCEfficiency for Reco");
        // }
        // if ( m_muRecoSF_tool->applyEfficiencyScaleFactor( *mu_itr ) !=
        // CP::CorrectionCode::Ok ) {
        //  Warning( "executeSF()", "Problem in applyEfficiencyScaleFactor for
        // Reco");
        // }
        ...
        //
        //  If SF decoration vector doesn't exist, create it (will be done only
        // for the 1st systematic for *this* muon)
        //

        SG::AuxElement::Decorator<std::vector<float> > sfVecReco(
        ...
        if (m_muRecoSF_tool->getEfficiencyScaleFactor(*mu_itr,
                                                      recoEffSF) !=
            CP::CorrectionCode::Ok) {
        ...
        // reco sys names are saved in a vector. Entries positions are
        // preserved!
        //
        SG::AuxElement::Decorator<std::vector<std::string> > sfVecReco_sysNames(
          m_outputSystNamesReco + "_sysNames");
        ...
        if (m_debug) {
          Info("executeSF()", "===>>>");
          Info("executeSF()", " ");
          Info("executeSF()", "Muon %i, pt = %.2f GeV ", idx,
               (mu_itr->pt() * 1e-3));
          Info("executeSF()", " ");
          Info("executeSF()",
               "Reco eff. SF decoration: %s",
               m_outputSystNamesReco.c_str());
          Info("executeSF()", " ");
          Info("executeSF()", "Systematic: %s",
               syst_it.name().c_str());
          Info("executeSF()", " ");

          // Info( "executeSF()", "Reco efficiency:");
          // Info( "executeSF()", "\t %f (from applyMCEfficiency())",
          // mu_itr->auxdataConst< float >( "mcEfficiency" ) );
          Info("executeSF()", "and its SF:");

          // Info( "executeSF()", "\t %f (from applyEfficiencyScaleFactor())",
          // mu_itr->auxdataConst< float >( "EfficiencyScaleFactor" ) );
          Info("executeSF()", "\t %f (from getEfficiencyScaleFactor())",
               recoEffSF);
          Info("executeSF()", "--------------------------------------");
        }
    ...
    //
    // NB: we need to make sure that this is not pushed more than once in
    // TStore!
    // This will be the case when this executeSF() function gets called for
    // every syst varied input container,
    // e.g. the different SC containers w/ calibration systematics upstream.
    //
    // Use the counter defined in execute() to check this is done only once per
    // event
    //
    if (countSyst == 0) RETURN_CHECK("MuonEfficiencyCorrector::executeSF()",
                                     m_store->record(sysVariationNamesReco,
                                                     m_outputSystNamesReco),
                                     "Failed to record vector of systematic names for muon reco efficiency SF");
  ...
  if (!isToolAlreadyUsed(m_isoEffSF_tool_name)) {
    ...
    for (const auto& syst_it : m_systListIso) {
      ...
      if (m_debug) Info("executeSF()",
                        "Muon iso efficiency SF sys name (to be recorded in xAOD::TStore) is: %s",
                        sfName.c_str());
      ...
      // apply syst
      //
      if (m_muIsoSF_tool->applySystematicVariation(syst_it) !=
          CP::SystematicCode::Ok) {
        Error("executeSF()",
              "Failed to configure MuonEfficiencyScaleFactors for systematic %s",
              syst_it.name().c_str());
      ...
      for (auto mu_itr : *(inputMuons)) {
        ...
        // a)
        // decorate directly the muon with iso efficiency (useful at all?), and
        // the corresponding SF
        //
        // if ( m_muIsoSF_tool->applyMCEfficiency( *mu_itr ) !=
        // CP::CorrectionCode::Ok ) {
        //  Warning( "executeSF()", "Problem in applyMCEfficiency for Iso");
        // }
        // if ( m_muIsoSF_tool->applyEfficiencyScaleFactor( *mu_itr ) !=
        // CP::CorrectionCode::Ok ) {
        //  Warning( "executeSF()", "Problem in applyEfficiencyScaleFactor for
        // Iso");
        // }
        ...
        // for the 1st systematic for *this* muon)
        //
        SG::AuxElement::Decorator<std::vector<float> > sfVecIso(
        ...
        if (m_muIsoSF_tool->getEfficiencyScaleFactor(*mu_itr,
                                                     IsoEffSF) !=
        CP::CorrectionCode::Ok) {
          Warning("executeSF()", "Problem in getEfficiencyScaleFactor");
          IsoEffSF = 1.0;
        ...
        if (m_debug) {
          ...
          Info("executeSF()", " ");
          Info("executeSF()",
               "Isolation SF decoration: %s",
               m_outputSystNamesIso.c_str());
          Info("executeSF()", " ");
          Info("executeSF()", "Systematic: %s",
               syst_it.name().c_str());
          Info("executeSF()", " ");
          ...
          // Info( "executeSF()", "\t %f (from applyIsoEfficiency())",
          // mu_itr->auxdataConst< float >( "ISOmcEfficiency" ) );
          Info("executeSF()", "and its SF:");
    ...
    //
    if (countSyst == 0) RETURN_CHECK("MuonEfficiencyCorrector::executeSF()",
      m_store->record(sysVariationNamesIso, m_outputSystNamesIso),
      "Failed to record vector of systematic names for muon iso efficiency SF");
  }

  // 3.
  // Trigger efficiency SF - this is in principle given by the MCP tool as a
  // per-EVENT weight
  //
  // To allow more freedom to the user, we calculate it as a per-muon weight
  // with a trick
  ...
  if (m_isMC && m_useRandomRunNumber) {
    ...
    // use the per-event random runNumber weighted by integrated luminosity got
    // from CP::PileupReweightingTool::getRandomRunNumber()
    // Source:
    ...
    //
    //
    // TEMP! Commenting this out b/c stupid AnaToolHandle does not work as it
    // should...
    //
    ...
    int randRunNumber =
    m_pileup_tool_handle->getRandomRunNumber(*eventInfo, true);

    // int randRunNumber =
    // asg::ToolStore::get<CP::PileupReweightingTool>("Pileup")->getRandomRunNumber(
    // *eventInfo, true );
    ...
    if ((runNumber >= 266904) && (runNumber <= 284484)) {
      ...
      if (!(std::find(m_YearsList.begin(), m_YearsList.end(),
      randYear) != m_YearsList.end())) {
        Error("executeSF()",
        "Random runNumber is 2015 but no corresponding MuonTriggerEfficiency tool has been initialized. Check ilumicalc config or extend m_Years!");
        return EL::StatusCode::FAILURE;
      }
      ...
      randYear) != m_YearsList.end())) {
        Error("executeSF()",
        "Random runNumber is 2016 but no corresponding MuonTriggerEfficiency tool has been initialized. Check ilumicalc config or extend m_Years!");
        return EL::StatusCode::FAILURE;
      }
    } else {
  ...
  if (m_isMC && !m_useRandomRunNumber) {
    ...
    if (m_YearsList[0] == "2015") rn = m_runNumber2015; else if (m_YearsList[0] == "2016") rn = m_runNumber2016; else {
  ...
  if (!isToolAlreadyUsed(m_trigEffSF_tool_names[randYear])) {
    for (const auto& trig_it : m_SingleMuTriggers) {
      ...
      // std::cout << "This is the new string: " <<
      // m_fullname_outputSystNamesTrig << std::endl;
      ...
      for (const auto& syst_it : m_systListTrig) {
        ...
        for (auto mu_itr : *(inputMuons)) {
          ...
          //  If SF decoration vector doesn't exist, create it (will be done
          // only for the 1st systematic for *this* muon)
          //
  ...
  if (!isToolAlreadyUsed(m_TTVAEffSF_tool_name)) {
    ...
    for (const auto& syst_it : m_systListTTVA) {
      ...
      // should...
      //
      // if ( m_muTTVASF_tool_handle->applySystematicVariation(syst_it) !=
      // CP::SystematicCode::Ok ) {
      ...
      for (auto mu_itr : *(inputMuons)) {
        ...
        // a)
        // decorate directly the muon with TTVA efficiency (useful at all?), and
        // the corresponding SF
        //
        // if ( m_muTTVASF_tool->applyMCEfficiency( *mu_itr ) !=
        // CP::CorrectionCode::Ok ) {
        //  Warning( "executeSF()", "Problem in applyMCEfficiency for TTVA");
        // }
        // if ( m_muTTVASF_tool->applyEfficiencyScaleFactor( *mu_itr ) !=
        // CP::CorrectionCode::Ok ) {
        //  Warning( "executeSF()", "Problem in applyEfficiencyScaleFactor for
        // TTVA");
        // }
        ...
        if (m_debug) {
          ...
          // Info( "executeSF()", "\t %f (from applyIsoEfficiency())",
          // mu_itr->auxdataConst< float >( "TTVAmcEfficiency" ) );
          Info("executeSF()", "and its SF:");

          // Info( "executeSF()", "\t %f (from applyEfficiencyScaleFactor())",
          // mu_itr->auxdataConst< float >( "TTVAEfficiencyScaleFactor" ) );

    ...
    if(m_infoSwitch.m_effSF && m_mc) {
        for(auto& PID : m_infoSwitch.m_PIDWPs) {
            for(auto& iso : m_infoSwitch.m_isolWPs) {
                if( !iso.empty() ) elec.IsoEff_SF[ PID + iso ] = (*m_IsoEff_SF) [ PID + iso ].at(idx);
                for(auto& trig : m_infoSwitch.m_trigWPs) {
...
} // ElectronContainer::updateParticle
...
void ElectronContainer::setBranches(TTree* tree) {
    ...
    if(m_infoSwitch.m_effSF && m_mc) {
        for(auto& PID : m_infoSwitch.m_PIDWPs) {
            for(auto& isol : m_infoSwitch.m_isolWPs) {
                if( !isol.empty() ) tree->Branch( (m_name + "_IsoEff_SF_" + PID + isol).c_str(), &(*m_IsoEff_SF)[ PID + isol ] );
                for(auto& trig : m_infoSwitch.m_trigWPs) {
...
void ElectronContainer::FillElectron(const xAOD::IParticle* particle, const xAOD::Vertex* primaryVertex) {
    ...
    if(m_infoSwitch.m_PID) {
        ...
        if( LHLooseAcc.isAvailable(*elec) ) {
            ...
            if(LHLooseAcc(*elec) == 1) ++m_n_LHLoose; }  else {m_LHLoose->push_back(-1);}
        ...
        if( LHLooseBLAcc.isAvailable(*elec) ) {
            ...
            if(LHLooseBLAcc(*elec) == 1) ++m_n_LHLooseBL; }  else {m_LHLooseBL->push_back(-1);}
        ...
        if( LHMediumAcc.isAvailable(*elec) ) {
            ...
            if(LHMediumAcc(*elec) == 1) ++m_n_LHMedium; }  else {m_LHMedium->push_back(-1);}

        static SG::AuxElement::Accessor<char> LHTightAcc("LHTight");
        if( LHTightAcc.isAvailable(*elec) ) {
            m_LHTight->push_back( LHTightAcc(*elec) );
            if(LHTightAcc(*elec) == 1) ++m_n_LHTight; } else {m_LHTight->push_back(-1);}
        ...
        if( EMLooseAcc.isAvailable(*elec) ) {
            ...
            if(EMLooseAcc(*elec) == 1) ++m_n_IsEMLoose; } else {m_IsEMLoose->push_back(-1);}
        ...
        if( EMMediumAcc.isAvailable(*elec) ) {
            ...
            if(EMMediumAcc(*elec) == 1) ++m_n_IsEMMedium; } else {m_IsEMMedium->push_back(-1);}
        ...
        if( EMTightAcc.isAvailable(*elec) ) {
            ...
            if(EMTightAcc(*elec) == 1) ++m_n_IsEMTight; } else {m_IsEMTight->push_back(-1);}
    }
    ...
    if(m_infoSwitch.m_effSF && m_mc) {
        ...
        if( accRecoSF.isAvailable(*elec) ) m_RecoEff_SF->push_back( accRecoSF(*elec) ); else m_RecoEff_SF->push_back(junkSF); if( accPIDSF_LHLooseAndBLayer.isAvailable(*elec) ) m_PIDEff_SF_LHLooseAndBLayer->push_back( accPIDSF_LHLooseAndBLayer(*elec) ); else m_PIDEff_SF_LHLooseAndBLayer->push_back(junkSF); if( accPIDSF_LHLoose.isAvailable(*elec) ) m_PIDEff_SF_LHLoose->push_back( accPIDSF_LHLoose(*elec) ); else m_PIDEff_SF_LHLoose->push_back(junkSF); if( accPIDSF_LHMedium.isAvailable(*elec) ) m_PIDEff_SF_LHMedium->push_back( accPIDSF_LHMedium(*elec) ); else m_PIDEff_SF_LHMedium->push_back(junkSF); if( accPIDSF_LHTight.isAvailable(*elec) ) m_PIDEff_SF_LHTight->push_back( accPIDSF_LHTight(*elec) ); else m_PIDEff_SF_LHTight->push_back(junkSF);
    }
} // ElectronContainer::FillElectron

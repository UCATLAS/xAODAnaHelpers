...
ElectronContainer::ElectronContainer(const std::string& name,
  ...
  : ParticleContainer(name, detailStr, units, mc, true)
{
  ...
  if (m_infoSwitch.m_effSF && m_mc) {
    ...
    if (!m_infoSwitch.m_PIDWPs.size()) m_infoSwitch.m_PIDWPs =
      { "LooseAndBLayerLLH", "MediumLLH", "TightLLH" };

    // default isolation working points if no user input
    if (!m_infoSwitch.m_isolWPs.size()) m_infoSwitch.m_isolWPs =
      { "", "isolGradient", "isolLoose", "isolTight" };

    // default trigger working points if no user input
    if (!m_infoSwitch.m_trigWPs.size()) m_infoSwitch.m_trigWPs =
    ...
    for (auto& PID : m_infoSwitch.m_PIDWPs) {
      for (auto& isol : m_infoSwitch.m_isolWPs) {
        for (auto& trig : m_infoSwitch.m_trigWPs) {
          Info("ElectronConainer()", "Used working points: %s_%s_%s",
               trig.c_str(), PID.c_str(), isol.c_str());
        }
      }
    }
  }
...
void ElectronContainer::setTree(TTree *tree)
{
  ...
  if (m_infoSwitch.m_isolation) {
    ...
    connectBranch<int>(tree,
                       "isIsolated_FixedCutTight",
                       &m_isIsolated_FixedCutTight);
    connectBranch<int>(tree,
    ...
    connectBranch<float>(tree, "etcone20",
                         &m_etcone20);
  ...
  if (m_infoSwitch.m_PID) {
    ...
    tree->SetBranchAddress(("n" + m_name + "_IsEMMedium").c_str(),
                           &m_n_IsEMMedium);
      ...
      for (auto& isol : m_infoSwitch.m_isolWPs) {
        if (!isol.empty()) {
          tree->SetBranchStatus(
            (m_name + "_IsoEff_SF_" + PID + "_" + isol).c_str(), 1);
          tree->SetBranchAddress(
            (m_name + "_IsoEff_SF_" + PID + "_" + isol).c_str(),
            &(*m_IsoEff_SF)[PID + isol]);
        }

        for (auto& trig : m_infoSwitch.m_trigWPs) {
          tree->SetBranchStatus((m_name + "_TrigEff_SF_" + trig + "_" + PID +
                                 (!isol.empty() ? "_" + isol : "")).c_str(), 1);
          tree->SetBranchAddress((m_name + "_TrigEff_SF_" + trig + "_" + PID +
                                  (!isol.empty() ? "_" + isol : "")).c_str(),
                                 &(*m_TrigEff_SF)[trig + PID + isol]);

          tree->SetBranchStatus((m_name + "_TrigMCEff_"  + trig + "_" + PID +
                                 (!isol.empty() ? "_" + isol : "")).c_str(), 1);
          tree->SetBranchAddress((m_name + "_TrigMCEff_"  + trig + "_" + PID +
                                  (!isol.empty() ? "_" + isol : "")).c_str(),
                                 &(*m_TrigMCEff)[trig + PID + isol]);
        }
      }
    }

    connectBranch<std::vector<float> >(tree,
                                       "RecoEff_SF",
                                       &m_RecoEff_SF);
    connectBranch<std::vector<float> >(tree,
                                       "PIDEff_SF_LHLooseAndBLayer",
                                       &m_PIDEff_SF_LHLooseAndBLayer);
    connectBranch<std::vector<float> >(tree,
                                       "PIDEff_SF_LHLoose",
                                       &m_PIDEff_SF_LHLoose);
    connectBranch<std::vector<float> >(tree,
                                       "PIDEff_SF_LHMedium",
                                       &m_PIDEff_SF_LHMedium);
    connectBranch<std::vector<float> >(tree,
                                       "PIDEff_SF_LHTight",
                                       &m_PIDEff_SF_LHTight);
  }
...
void ElectronContainer::updateParticle(uint idx, Electron& elec)
{
  ...
  // isolation
  if (m_infoSwitch.m_isolation) {
    elec.isIsolated_LooseTrackOnly =
    ...
    elec.isIsolated_GradientLoose =     m_isIsolated_GradientLoose->at(
      idx);
    elec.isIsolated_FixedCutLoose =     m_isIsolated_FixedCutLoose->at(
      idx);
    elec.isIsolated_FixedCutTight =     m_isIsolated_FixedCutTight->at(
      idx);
    elec.isIsolated_FixedCutTightTrackOnly =
      m_isIsolated_FixedCutTightTrackOnly->at(idx);
    elec.isIsolated_UserDefinedFixEfficiency =
      m_isIsolated_UserDefinedFixEfficiency->at(idx);
    elec.isIsolated_UserDefinedCut =
  ...
  if (m_infoSwitch.m_effSF && m_mc) {
    for (auto& PID : m_infoSwitch.m_PIDWPs) {
      for (auto& iso : m_infoSwitch.m_isolWPs) {
        if (!iso.empty()) elec.IsoEff_SF[PID +
                                         iso] =  (*m_IsoEff_SF)[PID + iso].at(idx);

        for (auto& trig : m_infoSwitch.m_trigWPs) {
          elec.TrigEff_SF[trig + PID +
                          iso]             = (*m_TrigEff_SF)[trig + PID + iso].at(idx);
          elec.TrigMCEff[trig + PID + iso] = (*m_TrigMCEff)[trig + PID + iso].at(
            idx);
        }
      }
    }
  ...
  if (m_infoSwitch.m_trackhitcont) {
    ...
    elec.trknInnermostPixLayHits = m_trknInnermostPixLayHits->at(idx); // not
                                                                       // available
                                                                       // in
                                                                       // DC14
    elec.trkPixdEdX = m_trkPixdEdX->at(idx);                           // not
                                                                       // available
                                                                       // in
                                                                       // DC14
  }
...
void ElectronContainer::setBranches(TTree *tree)
{
  ...
  if (m_infoSwitch.m_effSF && m_mc) {
    for (auto& PID : m_infoSwitch.m_PIDWPs) {
      for (auto& isol : m_infoSwitch.m_isolWPs) {
        if (!isol.empty()) tree->Branch(
            (m_name + "_IsoEff_SF_"  + PID + isol).c_str(),
            &(*m_IsoEff_SF)[PID + isol]);

        for (auto& trig : m_infoSwitch.m_trigWPs) {
          tree->Branch((m_name + "_TrigEff_SF_" + trig + "_" + PID +
                        (!isol.empty() ? "_" + isol : "")).c_str(),
                       &(*m_TrigEff_SF)[trig + PID + isol]);
          tree->Branch((m_name + "_TrigMCEff_"  + trig + "_" + PID +
                        (!isol.empty() ? "_" + isol : "")).c_str(),
                       &(*m_TrigMCEff)[trig + PID + isol]);
        }
      }
...
void ElectronContainer::clear()
{
  ...
  if (m_infoSwitch.m_effSF && m_mc) {
    for (auto& PID : m_infoSwitch.m_PIDWPs) {
      for (auto& isol : m_infoSwitch.m_isolWPs) {
        if (!isol.empty()) (*m_IsoEff_SF)[PID + isol].clear();
...
void ElectronContainer::FillElectron(const xAOD::Electron *elec,
                                     const xAOD::Vertex   *primaryVertex) {
  return FillElectron(static_cast<const xAOD::IParticle *>(elec), primaryVertex);
}

void ElectronContainer::FillElectron(const xAOD::IParticle *particle,
                                     const xAOD::Vertex    *primaryVertex)
{
  ...
  if (m_infoSwitch.m_kinematic) {
    float calo_eta =
      (elec->caloCluster()) ? elec->caloCluster()->etaBE(2) : -999.0;
  ...
  if (m_infoSwitch.m_trigger) {
    ...
    static SG::AuxElement::Accessor<std::map<std::string,
                                             char> > isTrigMatchedMapElAcc("isTrigMatchedMapEl");
    ...
    // if at least one match among the chains is found, say this electron is
    // trigger matched
    if (std::find(matches.begin(), matches.end(), 1) != matches.end()) m_isTrigMatched->push_back(1); else m_isTrigMatched->push_back(0);
  }

...
namespace xAOD {
  ...
  class JetContainer;
#endif // ifndef XAODJET_JETCONTAINER_H
#ifndef XAODJET_JET_H
  class Jet;
#endif // ifndef XAODJET_JET_H
}
  ...
  Algorithm(className)
{
  ...
  m_runNominal = true;              // set to false if you want to run met
                                    // systematics
...
EL::StatusCode METConstructor::setupJob(EL::Job& job)
{
  ...
  // StatusCode::enableFailure();// do not decomment this, maybe an unchecked
  // status code gives a crash...
...
EL::StatusCode METConstructor::initialize()
{
  ...
  if (m_tauSelTool->initialize().isFailure()) {
    Error("initialize()",
          "Failed to properly initialize tau selection tool. Exiting.");
    return EL::StatusCode::FAILURE;
  }

  Info("initialize()",
       "METConstructor Interface %s succesfully initialized!",
       m_name.c_str());

  // use the helper function getlistofsystematics:

  // run syst
  if (!m_runNominal && !m_systName.empty()) { //  m_systName is set by default
                                              // to m_systName= "All", do not
                                              // change it
    // get the syst from met syst tool
    const CP::SystematicSet recSyst = m_metSyst_handle->recommendedSystematics();
    sysList = HelperFunctions::getListofSystematics(recSyst,
                                                    m_systName,
                                                    m_systVal,
                                                    m_debug);
  } else {                                  // run nominal
    sysList.push_back(CP::SystematicSet()); // add empty systematic (Nominal
                                            // case)
  }
  ...
  RETURN_CHECK("METConstructor::initialize()",
  ...
  if (m_debug) Info("METConstructor::initialize()", "Is MC? %i",
                    static_cast<int>(m_isMC));
...
EL::StatusCode METConstructor::execute()
{
  ...
  // if (m_debug) cout<< "number of processed events now is : "<< m_numEvent
  // <<endl;


  const xAOD::MissingETContainer *coreMet(0);
  RETURN_CHECK("METConstructor::execute()",
               HelperFunctions::retrieve(coreMet,
                                         m_coreName.Data(), m_event, m_store,
                                         m_debug), "Failed retrieving MET Core.");

  const xAOD::MissingETAssociationMap *metMap = 0;
  RETURN_CHECK("METConstructor::execute()",
               HelperFunctions::retrieve(metMap,
                                         m_mapName.Data(), m_event, m_store,
                                         m_debug), "Failed retrieving MET Map.");
  ...
  if (!m_runNominal && !m_jetSystematics.empty()) {
    ...
    RETURN_CHECK("METConstructor::initialize()",
                 HelperFunctions::retrieve(sysJetsNames,
                                           m_jetSystematics,
                                           0,
                                           m_store,
                                           m_debug),
                 "Failed retrieving object systematics.");

    for (auto systName : *sysJetsNames) {
      if ((systName != "") &&
          !(std::find(sysList.begin(), sysList.end(),
                      CP::SystematicSet(systName)) !=
            sysList.end())) sysList.push_back(CP::SystematicSet(systName));
  ...
  if (!m_runNominal && !m_eleSystematics.empty()) {
    ...
    RETURN_CHECK("METConstructor::initialize()",
                 HelperFunctions::retrieve(sysElectronsNames,
                                           m_eleSystematics,
                                           0,
                                           m_store,
                                           m_debug),
  ...
  if (!m_runNominal && !m_muonSystematics.empty()) {
    ...
    RETURN_CHECK("METConstructor::initialize()",
                 HelperFunctions::retrieve(sysMuonsNames,
                                           m_muonSystematics,
                                           0,
                                           m_store,
                                           m_debug),
  ...
  if (!m_runNominal && !m_phoSystematics.empty()) {
    ...
    RETURN_CHECK("METConstructor::initialize()",
                 HelperFunctions::retrieve(sysPhotonsNames,
                                           m_phoSystematics,
                                           0,
                                           m_store,
                                           m_debug),
  ...
  for (sysListItr = sysList.begin(); sysListItr != sysList.end(); ++sysListItr) { //
                                                                                  ...
                                                                                  //
                                                                                  // systematics
    // this is kind of annoying, but applySystematicVariation only takes a
    // SystematicSet, but *sysListItr is a SystematicVariation.
    // We use the SystematicSet constructor which just takes a
    // SystematicVariation
    // CP::SystematicSet iSysSet( (*sysListItr).name());
    // tell the tool that we are using this SystematicSet (of one
    // SystematicVariation for now)
    // after this call, when we use applyCorrection, the given met term will be
    // adjusted with this systematic applied
    // assert(    m_metSyst_handle->applySystematicVariation(sysList) );
    //
    // info from
    // https://svnweb.cern.ch/trac/atlasoff/browser/Reconstruction/MET/METUtilities/trunk/util/example_METMaker_METSystematicsTool.cxx

    CP::SystematicSet iSysSet((*sysListItr).name()); // to pass from
                                                     // SystematicVariation to
                                                     // SystematicSet
    //
    std::string sysListItrString;                    // just for convenience, to
    ...
    // if the syst varied container exists take it, otherwise take the nominal
    // one
    ...
    if ((m_inputElectrons.Length() > 0) &&
        m_store->contains<xAOD::ElectronContainer>(m_inputElectrons.Data() +
                                                   sysListItrString)) {
      const xAOD::ElectronContainer *eleCont(0);

      if (m_store->contains<xAOD::ElectronContainer>(m_inputElectrons.Data() +
                                                     sysListItrString)) {
        RETURN_CHECK("METConstructor::execute()",
                     HelperFunctions::retrieve(eleCont,
                                               m_inputElectrons.Data() + sysListItrString, m_event, m_store, m_debug),
                     "Failed retrieving electron cont.");
    ...
    if ((m_inputPhotons.Length() > 0) && m_store->contains<xAOD::PhotonContainer>(m_inputPhotons.Data() + sysListItrString)) {
      ...
      if (m_doPhotonCuts) {
        ...
        for (const auto& ph : *phoCont) {
          ...
          // ATH_MSG_VERBOSE("Photon author = " << ph->author() << " test " <<
          // (ph->author()&20));
    ...
    ///// NOTE: for taus we are not applying systematics! since
    // "m_inputTaus.Data()+sysListItrString" is not in Tstore!
    ...
    std::string m_inputJets_Syst =  m_inputJets.Data() + sysListItrString; // just
                                                                           // for
                                                                           // convenience
    ...
    if (m_store->contains<xAOD::JetContainer>(m_inputJets.Data() + sysListItrString)) {
      ...
      // RETURN_CHECK( "METConstructor::execute()",
      // m_metSyst_handle->evtStore()->retrieve( jetCont,m_inputJets_Syst  ),
      // "");// is this necessary?
      ...
      // RETURN_CHECK( "METConstructor::execute()",
      // m_metSyst_handle->evtStore()->retrieve( jetCont, m_inputJets.Data() ),
      // "");// is this necessary?
    ...
    // the jet term and soft term(s) are built simultaneously using
    // METMaker::rebuildJetMET(...) or METMaker::rebuildTrackMET(...)
    ...
    // now tell the m_metSyst_handle that we are using this SystematicSet (of
    // one SystematicVariation for now)
    // after this call, when we use applyCorrection, the given met term will be
    // adjusted with this systematic applied
    ...
    /* something causes a crash down here
       ...
       RETURN_CHECK("METConstructor::execute()",
          HelperFunctions::retrieve(oldMet, m_referenceMETContainer.Data(),
          m_event, m_store, m_verbose) ,"");
       //xAOD::MissingETContainer::const_iterator
          final(oldMet->find("FinalClus"));
       //xAOD::MissingETContainer::const_iterator
          newfinal(newMet->find("FinalClus"));
       Info("execute()", ">>>>>>>>>>>>>>>>>>>>>>>>>>>>");
       if( m_inputElectrons.Length() > 0 ) Info("execute()", "RefEle:
              old=%8.f  new=%8.f", (*oldMet->find("RefEle"))->met(),
          (*newMet->find("RefEle"))->met());
       if( m_inputPhotons.Length() > 0 )   Info("execute()", "RefPhoton:
           old=%8.f  new=%8.f", (*oldMet->find("RefGamma"))->met(),
          (*newMet->find("RefGamma"))->met());
       if( m_inputTaus.Length() > 0 )      Info("execute()", "RefTau:
              old=%8.f  new=%8.f", (*oldMet->find("RefTau"))->met(),
          (*newMet->find("RefTau"))->met());
       if( m_inputMuons.Length() > 0 )     Info("execute()", "RefMuon:
             old=%8.f  new=%8.f", (*oldMet->find("Muons"))->met(),
          (*newMet->find("Muons"))->met());
       Info("execute()", "RefJet:       old=%8.f  new=%8.f",
          (*oldMet->find("RefJet"))->met(), (*newMet->find("RefJet"))->met());
       Info("execute()", "SoftClus:     old=%8.f  new=%8.f",
          (*oldMet->find("SoftClus"))->met(),
          (*newMet->find("SoftClus"))->met());
       Info("execute()", "PVSoftTrk:    old=%8.f  new=%8.f",
          (*oldMet->find("PVSoftTrk"))->met(),
          (*newMet->find("PVSoftTrk"))->met());
       Info("execute()", "  ");
       Info("execute()", "FinalClus:    old=%8.f  new=%8.f",
          (*oldMet->find("FinalClus"))->met(),
          (*newMet->find("FinalClus"))->met());
       Info("execute()", "       >>>>> R=%.3f",
                   (*oldMet->find("FinalClus"))->met()/
          (*newMet->find("FinalClus"))->met());
       // Info("execute()", "FinalTrk:     old=%8.f  new=%8.f",
          (*oldMet->find("FinalTrk"))->met(),
          (*newMet->find("FinalTrk"))->met());
       // Info("execute()", "       >>>>> R=%.3f",
                   (*oldMet->find("FinalTrk"))->met()/
          (*newMet->find("FinalTrk"))->met());
       }

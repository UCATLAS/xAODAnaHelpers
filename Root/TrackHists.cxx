...
StatusCode TrackHists::execute( const xAOD::TrackParticle* trk, const xAOD::Vertex *pvx, float eventWeight,  const xAOD::EventInfo* eventInfo ) {
  ...
  if(m_fillHitCounts){
    ...
    if(!trk->summaryValue(nBL,       xAOD::numberOfBLayerHits))       ATH_MSG_ERROR("TrackHists::execute()", "BLayer hits not filled");
    if(!trk->summaryValue(nPix,      xAOD::numberOfPixelHits))        ATH_MSG_ERROR("TrackHists::execute()", "Pix hits not filled");
    if(!trk->summaryValue(nPixDead,  xAOD::numberOfPixelDeadSensors)) ATH_MSG_ERROR("TrackHists::execute()", "Pix Dead not filled");
    if(!trk->summaryValue(nPixHoles, xAOD::numberOfPixelHoles))       ATH_MSG_ERROR("TrackHists::execute()", "Pix holes not filled");
    if(!trk->summaryValue(nSCT,      xAOD::numberOfSCTHits))          ATH_MSG_ERROR("TrackHists::execute()", "SCT hits not filled");
    if(!trk->summaryValue(nSCTDead,  xAOD::numberOfSCTDeadSensors))   ATH_MSG_ERROR("TrackHists::execute()", "SCT Dead not filled");
    if(!trk->summaryValue(nTRT,      xAOD::numberOfTRTHits))          ATH_MSG_ERROR("TrackHists::execute()", "TRT hits not filled");
    // if(!trk->summaryValue(nTRTHoles, xAOD::numberOfTRTHoles))         ATH_MSG_ERROR("TrackHists::execute()", "TRT holes not filled");
    // if(!trk->summaryValue(nTRTDead,  xAOD::numberOfTRTDeadStraws))    ATH_MSG_ERROR("TrackHists::execute()", "TRT Dead not filled");

...
EL::StatusCode MinixAOD::changeInput(bool firstFile)
{
  ...
  if (m_copyCutBookkeeper)
  {
    ...
    RETURN_CHECK("MinixAOD::fileExecute()",
                 wk()->xaodEvent()->retrieveMetaInput(inputCBKContainer,
                                                      "CutBookkeepers"),
                 "");
...
EL::StatusCode MinixAOD::fileExecute() {
  return EL::StatusCode::SUCCESS;
}
...
EL::StatusCode MinixAOD::initialize()
{
  ...
  RETURN_CHECK("MinixAOD::initialize()", m_event->writeTo(
                 file_xAOD), "Could not set output to file");

  if (m_copyFileMetaData) {
    m_fileMetaDataTool = new xAODMaker::FileMetaDataTool();

    if (m_verbose) RETURN_CHECK("MinixAOD::initialize()",
                                m_fileMetaDataTool->setProperty("OutputLevel",
                                                                MSG::VERBOSE),
                                "Could not set verbosity on fileMenuMetaDataTool");



    RETURN_CHECK("MinixAOD::initialize()",
                 m_fileMetaDataTool->initialize(),
                 "Could not initialize FileMetaDataTool");
  ...
  if (m_copyTriggerInfo) {
    ...
    if (m_verbose) RETURN_CHECK("MinixAOD::initialize()",
                                m_trigMetaDataTool->setProperty("OutputLevel",
                                                                MSG::VERBOSE),
                                "Could not set verbosity on TriggerMenuMetaDataTool");



    RETURN_CHECK("MinixAOD::initialize()",
                 m_trigMetaDataTool->initialize(),
                 "Could not initialize TriggerMenuMetaDataTool");

    if (m_debug) Info("initialize()", "TriggerMenuMetaDataTool initialized...");

    if (m_debug) std::cout <<
        "Adding xTrigDecision and TrigConfKeys to the list of keys copied from the input file."
                           << std::endl;
  ...
  while (std::getline(ss, token, ' ')) m_simpleCopyKeys_vec.push_back(token);

  // A B C D ... Z -> {A, B, C, D, ..., Z}
  ss.clear(); ss.str(m_storeCopyKeys);

  while (std::getline(ss, token, ' ')) m_copyFromStoreToEventKeys_vec.push_back(
      token);
  ...
  while (std::getline(ss, token, ' ')) {
    ...
    m_shallowCopyKeys_vec.push_back(std::pair<std::string,
                                              std::string>(token.substr(0, pos),
                                                           token.substr(pos +
                                                                        1)));
  }
  ...
  while (std::getline(ss, token, ' ')) {
    int pos = token.find_first_of('|');
    m_deepCopyKeys_vec.push_back(std::pair<std::string,
                                           std::string>(token.substr(0, pos),
                                                        token.substr(pos + 1)));
  }
  ...
  while (std::getline(ss, token, ' ')) {
    int pos = token.find_first_of('|');
    m_vectorCopyKeys_vec.push_back(std::pair<std::string,
                                             std::string>(token.substr(0, pos),
                                                          token.substr(pos + 1)));
  }

  if (m_debug) Info("initialize()",
                    "MinixAOD Interface succesfully initialized!");
...
EL::StatusCode MinixAOD::execute()
{
  ...
  RETURN_CHECK("BasicEventSelection::initialize()",
               HelperFunctions::retrieve(eventInfo,
                                         "EventInfo",
                                         m_event,
                                         m_store,
                                         m_verbose), "");
  ...
  if (m_copyCutBookkeeper)
  {
    ...
    if (eventInfo->eventType(xAOD::EventInfo::IS_SIMULATION)) eventWeight =
  ...
  for (const auto& key: m_simpleCopyKeys_vec) {
    RETURN_CHECK("MinixAOD::execute()", m_event->copy(key),
                 std::string(
                   "Could not copy " + key + " from input file.").c_str());

    if (m_debug) std::cout << "Copying " << key << " from input file" <<
        std::endl;
  }
  ...
  for (const auto& keypair: m_deepCopyKeys_vec) {
    ...
    RETURN_CHECK("MinixAOD::execute()",
                 HelperFunctions::retrieve(cont,
                                           in_key,
                                           nullptr,
                                           m_store,
                                           m_verbose),
                 std::string(
                   "Could not retrieve container " + in_key +
                   " from TStore. Enable m_verbose to find out why.").c_str());

    if (const xAOD::ElectronContainer *t_cont =
          dynamic_cast<const xAOD::ElectronContainer *>(cont)) {
      RETURN_CHECK("MinixAOD::execute()",
                   (HelperFunctions::makeDeepCopy<xAOD::ElectronContainer,
                                                  ...
                                                  xAOD::Electron>(m_store,
                                                                  ...
                                                                  t_cont)),
                   std::string(
                     "Could not deep copy " + in_key + " to " + out_key +
                     ".").c_str());
    } else if (const xAOD::JetContainer *t_cont =
                 dynamic_cast<const xAOD::JetContainer *>(cont)) {
      RETURN_CHECK("MinixAOD::execute()",
                   ...
                   std::string(
                     "Could not deep copy " + in_key + " to " + out_key +
                     ".").c_str());
    } else if (const xAOD::MissingETContainer *t_cont =
                 dynamic_cast<const xAOD::MissingETContainer *>(cont)) {
      RETURN_CHECK("MinixAOD::execute()",
                   (HelperFunctions::makeDeepCopy<xAOD::MissingETContainer,
                                                  xAOD::MissingETAuxContainer,
                                                  xAOD::MissingET>(m_store,
                                                                   out_key.c_str(),
                                                                   t_cont)),
                   std::string(
                     "Could not deep copy " + in_key + " to " + out_key +
                     ".").c_str());
    } else if (const xAOD::MuonContainer *t_cont =
                 dynamic_cast<const xAOD::MuonContainer *>(cont)) {
      RETURN_CHECK("MinixAOD::execute()",
                   ...
                   std::string(
                     "Could not deep copy " + in_key + " to " + out_key +
                     ".").c_str());
    } else if (const xAOD::PhotonContainer *t_cont =
                 dynamic_cast<const xAOD::PhotonContainer *>(cont)) {
      RETURN_CHECK("MinixAOD::execute()",
                   (HelperFunctions::makeDeepCopy<xAOD::PhotonContainer,
                   ...
                   std::string(
                     "Could not deep copy " + in_key + " to " + out_key +
                     ".").c_str());
    } else if (const xAOD::TauJetContainer *t_cont =
                 dynamic_cast<const xAOD::TauJetContainer *>(cont)) {
      RETURN_CHECK("MinixAOD::execute()",
                   (HelperFunctions::makeDeepCopy<xAOD::TauJetContainer,
      ...
      std::cout << "Could not identify what container " << in_key <<
        " corresponds to for deep-copying." << std::endl;
      return EL::StatusCode::FAILURE;
    }
    m_copyFromStoreToEventKeys_vec.push_back(out_key);

    if (m_debug) std::cout << "Deep-Copied " << in_key << " to " << out_key <<
        " to record to output file" << std::endl;
  }
  ...
  for (const auto& keypair: m_vectorCopyKeys_vec) {
    ...
    RETURN_CHECK("MinixAOD::execute()",
                 HelperFunctions::retrieve(vector,
                                           vectorName,
                                           nullptr,
                                           m_store,
                                           m_verbose),
                 std::string(
                   "Could not retrieve vector " + vectorName +
                   " from TStore. Enable m_verbose to find out why.").c_str());
    ...
    if (m_debug &&
        !parent.empty()) std::cout << "... along with their parent " << parent <<
        std::endl;
  }
  ...
  // all we need to do is retrieve it and figure out what type it is to record
  // it and we're done
  for (const auto& key: m_copyFromStoreToEventKeys_vec) {
    ...
    RETURN_CHECK("MinixAOD::execute()",
                 HelperFunctions::retrieve(cont,
                                           ...
                                           m_verbose),
                 std::string(
                   "Could not retrieve container " + key +
                   " from TStore. Enable m_verbose to find out why.").c_str());

    if (dynamic_cast<const xAOD::ElectronContainer *>(cont)) {
      RETURN_CHECK("MinixAOD::execute()",
                   (HelperFunctions::recordOutput<xAOD::ElectronContainer,
                                                  xAOD::ElectronAuxContainer>(
                      m_event, m_store, key)),
                   std::string(
                     "Could not copy " + key +
                     " from TStore to TEvent.").c_str());
    } else if (dynamic_cast<const xAOD::JetContainer *>(cont)) {
      RETURN_CHECK("MinixAOD::execute()",
                   (HelperFunctions::recordOutput<xAOD::JetContainer,
                                                  xAOD::JetAuxContainer>(m_event,
                                                                         m_store,
                                                                         key)),
                   std::string(
                     "Could not copy " + key +
      ...
      RETURN_CHECK("MinixAOD::execute()",
                   (HelperFunctions::recordOutput<xAOD::MissingETContainer,
                                                  xAOD::MissingETAuxContainer>(
                      m_event, m_store, key)),
                   std::string(
                     "Could not copy " + key +
      ...
      RETURN_CHECK("MinixAOD::execute()",
                   (HelperFunctions::recordOutput<xAOD::MuonContainer,
                                                  xAOD::MuonAuxContainer>(m_event,
                                                                          m_store,
                                                                          key)),
                   std::string(
                     "Could not copy " + key + " from TStore to TEvent").c_str());
    } else if (dynamic_cast<const xAOD::PhotonContainer *>(cont)) {
      RETURN_CHECK("MinixAOD::execute()",
                   (HelperFunctions::recordOutput<xAOD::PhotonContainer,
                                                  xAOD::PhotonAuxContainer>(
                      m_event, m_store, key)),
                   std::string(
                     "Could not copy " + key +
      ...
      RETURN_CHECK("MinixAOD::execute()",
                   (HelperFunctions::recordOutput<xAOD::TauJetContainer,
                                                  xAOD::TauJetAuxContainer>(
                      m_event, m_store, key)),
                   std::string(
                     "Could not copy " + key +
      ...
      std::cout << "Could not identify what container " << key <<
        " corresponds to for copying from TStore to TEvent." << std::endl;
      return EL::StatusCode::FAILURE;
    }

    if (m_debug) std::cout << "Copied " << key <<
      " and it's auxiliary container from TStore to TEvent" << std::endl;
  }
...
EL::StatusCode MinixAOD::postExecute() {
  return EL::StatusCode::SUCCESS;
}
...
EL::StatusCode MinixAOD::finalize() {
  ...
  if (m_copyCutBookkeeper)
  {
    RETURN_CHECK("MinixAOD::finalize()",
                 wk()->xaodEvent()->recordMeta(m_outputCBKContainer,
                                               "CutBookkeepers"),
    "");
    RETURN_CHECK("MinixAOD::finalize()",
                 wk()->xaodEvent()->recordMeta(m_outputCBKContainer_aux,
                                               "CutBookkeepersAux."),
    "");
    RETURN_CHECK("MinixAOD::finalize()",
                 wk()->xaodEvent()->recordMeta(m_outputInCBKContainer,
                                               "IncompleteCutBookkeepers"),
    "");
    RETURN_CHECK("MinixAOD::finalize()",
                 wk()->xaodEvent()->recordMeta(m_outputInCBKContainer_aux,
  ...
  // Close file
  TFile *file_xAOD = wk()->getOutputFile(m_outputFileName);
  RETURN_CHECK("MinixAOD::finalize()", m_event->finishWritingTo(
                 file_xAOD), "Could not finish writing to the output xAOD.");
